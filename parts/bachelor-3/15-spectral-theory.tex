...
(Square roots) Let $T$ be compact, self-adjoint, positive
($\langle x, Tx\rangle \geq 0$) on a Hilbert space $H$, with
spectral decomposition $Tx = \sum_n\mu_n\langle e_n,
x\rangle e_n$ ($\mu_n > 0$).
(a) Define $Sx = \sum_n\sqrt{\mu_n}\,\langle e_n, x\rangle
e_n$; show $S$ is compact, self-adjoint, positive, with $S^2
= T$.
(b) Prove uniqueness: any compact positive self-adjoint $R$
...
\emph{compact}, not necessarily self-adjoint.
(a) Show that $T^*T$ is compact, self-adjoint, positive; let
...
\cref{prop:b3:spectral:ideal}'s converse for Hilbert
...
$A$ be a compact self-adjoint \emph{positive} operator with
...
\begin{enumerate}[resume]
  ...
  \item (Weyl stability) For compact self-adjoint positive
        $A, B$, deduce from \cref{exo:b3:spectral:8} that
        \[
        \abs{\mu_n(A) - \mu_n(B)} \;\leq\; \vertiii{A - B}
        \qquad\text{for every } n :
        \]
        the full spectrum is $1$-Lipschitz in the operator
...
\begin{enumerate}[resume]
  \item Show that $\sum_n\lambda_n^2 =
        ...
        Hilbert basis, cf.\ \cref{exo:b3:spectral:5} --- and
        apply Parseval on the square; question 7 identifies
        the coefficients)}.
...
\begin{enumerate}[resume]
  \item Suppose $\nu \notin \{n^2\pi^2 : n \geq 1\}$. Show
        ...
        --- the explicit-coordinates form of the Fredholm
  ...
  \item (Synthesis) Compile the dictionary of this problem:
        ...
        Fredholm alternative $\leftrightarrow$ resonance;
...
\begin{enumerate}[resume]
  \item ($\zeta(6)$, for free) Question 6's Parseval
        ...
        at the cost of an iterated kernel --- Parseval on one
        well-chosen function is the cheaper route here.
  \item (The ground state is positive) Let $A$ be a compact
        self-adjoint positive operator on $L^2(\intcc01)$
        given by a continuous symmetric kernel $k > 0$ on
        ...
        both signs on sets of positive measure --- hence $u$
