...
Can every equation be solved by \omterm{def:b3:galois:radical}{radicals}, as the quadratic formula
...
answered --- negatively --- by a single idea of \'Evariste \omterm{def:b3:galois:galois}{Galois}:
...
the dictionary: \omterm{def:b3:galois:extension}{field extensions} and \omterm{def:b3:galois:extension}{degrees}, \omterm{thm:b3:galois:splitting}{splitting fields} and
\omterm{def:b3:galois:closure}{algebraic closures}, \omterm{thm:b3:galois:finitefields}{finite fields} (a complete theory --- and the
promised cyclicity of $\mathbb F_q^\times$), separability, then
the \omterm{thm:b3:galois:fundamental}{Galois correspondence} itself, with full proofs. We harvest:
...
\omterm{def:b3:galois:radical}{radicals} --- \cref{ch:b3:groups}'s simplicity of $A_5$ striking
its target.
...
element} over $K$; the monic generator $\pi_\alpha$ of the \omterm{def:b3:rings:ideal}{ideal}
$\{P : P(\alpha) = 0\}$ of $K[X]$ is its \emph{minimal
polynomial}\index{minimal polynomial (field theory)}, an
\omterm{def:b3:rings:divisibility}{irreducible} polynomial ($\pi = QR$ with $Q(\alpha) = 0$ forces $R$
constant by minimality of the \omterm{def:b3:galois:extension}{degree}). Otherwise $\alpha$ is
...
If $\alpha$ is \omterm{def:b3:galois:algebraic}{algebraic} over $K$ with $d = \deg\pi_\alpha$,
then
\[
...
$[L:K] < \infty$, every $\alpha \in L$ is \omterm{def:b3:galois:algebraic}{algebraic} of \omterm{def:b3:galois:extension}{degree}
...
\omterm{def:b3:rings:divisibility}{irreducible}, $K[X]/(\pi_\alpha)$ is a \emph{field}
(\cref{prop:b3:rings:primemaximal}: in the \omterm{def:b3:rings:pidufd}{PID} $K[X]$,
\omterm{def:b3:rings:divisibility}{irreducible} generates a \omterm{def:b3:rings:primemaximal}{maximal ideal}), so $K[\alpha]$ is a field
containing $K$ and $\alpha$: it equals $K(\alpha)$. The classes
of $1, X, \dots, X^{d-1}$ form a basis of the quotient (Euclidean
division), whence the basis and the \omterm{def:b3:galois:extension}{degree}. Conversely if $[L:K]
= n < \infty$: $1, \alpha, \dots, \alpha^n$ are dependent, giving
an annihilating polynomial; then $[K(\alpha):K] =
\deg\pi_\alpha$ divides $n$ by the \omterm{thm:b3:galois:tower}{tower law}.
\end{proof}

\begin{corollary}\label{cor:b3:galois:algclosed}
If $\alpha, \beta$ are \omterm{def:b3:galois:algebraic}{algebraic} over $K$, so are $\alpha \pm
\beta$, $\alpha\beta$, $\alpha/\beta$ ($\beta \ne 0$): the
elements of $L$ \omterm{def:b3:galois:algebraic}{algebraic} over $K$ form a subfield of $L$.
Moreover algebraicity is transitive: \omterm{def:b3:galois:algebraic}{algebraic} over \omterm{def:b3:galois:algebraic}{algebraic} is
\omterm{def:b3:galois:algebraic}{algebraic}.
\end{corollary}

\begin{proof}
$K(\alpha, \beta) = (K(\alpha))(\beta)$ is finite over
$K(\alpha)$ ($\beta$ \omterm{def:b3:galois:algebraic}{algebraic} over $K \subseteq K(\alpha)$) and
$K(\alpha)/K$ is finite: by the \omterm{thm:b3:galois:tower}{tower law} $[K(\alpha,\beta):K] <
\infty$, and every element of $K(\alpha, \beta)$ --- including
the four listed --- is \omterm{def:b3:galois:algebraic}{algebraic} (\cref{thm:b3:galois:simple}).
Transitivity: if $\beta$ is \omterm{def:b3:galois:algebraic}{algebraic} over $L$ and $L/K$ is
\omterm{def:b3:galois:algebraic}{algebraic}, the coefficients $c_0, \dots, c_{m-1}$ of
$\pi_{\beta/L}$ generate a finite extension $F = K(c_0, \dots,
c_{m-1})$ of $K$ (repeated \omterm{thm:b3:galois:tower}{tower law}), and $F(\beta)/F$ is
finite: $[F(\beta):K] < \infty$, so $\beta$ is \omterm{def:b3:galois:algebraic}{algebraic} over
$K$.
\end{proof}

\begin{example}\label{ex:b3:galois:degrees}
$[\Q(\sqrt2):\Q] = 2$, $[\Q(\sqrt[3]2):\Q] = 3$ ($X^3 - 2$ is
\omterm{def:b3:rings:divisibility}{irreducible}: \omterm{thm:b3:rings:criteria}{Eisenstein}), $[\Q(\zeta_p):\Q] = p - 1$ for
$\zeta_p = \eu^{2\iu\pi/p}$ ($\Phi_p$ is \omterm{def:b3:rings:divisibility}{irreducible},
\cref{ex:b3:rings:eisenstein}). The \omterm{thm:b3:galois:tower}{tower law} is already a
...
\begin{proof}
\emph{Existence}, by induction on $\deg P$: pick an \omterm{def:b3:rings:divisibility}{irreducible}
...
\omterm{def:b3:galois:extension}{degrees} multiply to at most $n(n-1)\cdots = n!$.
...
$L, L'$ \omterm{thm:b3:galois:splitting}{splitting fields} of $P, P^\sigma$; then $\sigma$ extends
...
$\alpha \in L \setminus K$ of an \omterm{def:b3:rings:divisibility}{irreducible} factor $Q$ of $P$
with $\deg Q \geq 2$; $Q^\sigma$ is an \omterm{def:b3:rings:divisibility}{irreducible} factor of
...
\omterm{thm:b3:galois:splitting}{splitting field} of $P$ over $K(\alpha)$, and $L'$ of $P^\sigma$
...
closure}\index{algebraic closure} of $K$ is an \omterm{def:b3:galois:algebraic}{algebraic}
extension $\bar K/K$ with $\bar K$ \omterm{def:b3:galois:algebraic}{algebraically} closed.
\end{definition}

\begin{theorem}[Steinitz]\label{thm:b3:galois:steinitz}
Every field $K$ has an \omterm{def:b3:galois:closure}{algebraic closure}, unique up to
$K$-isomorphism.
\end{theorem}
...
monic $f \in K[X]$, and $I$ the \omterm{def:b3:rings:ideal}{ideal} generated by all
$f(X_f)$. $I$ is proper: a relation $1 = \sum_{i=1}^r g_i\,
f_i(X_{f_i})$ involves finitely many polynomials; in a common
\omterm{thm:b3:galois:splitting}{splitting field} $E$ of $f_1\cdots f_r$ pick roots $\alpha_i$ of
...
m$: a \omterm{def:b3:galois:extension}{field extension} of $K$ in which every nonconstant $f \in
K[X]$ has a root, namely $\bar X_f$, and which is \omterm{def:b3:galois:algebraic}{algebraic} over
$K$ (it is generated by the $\bar X_f$, each \omterm{def:b3:galois:algebraic}{algebraic}).
...
its finitely many coefficients in some $K_n$; an \omterm{def:b3:rings:divisibility}{irreducible}
factor of $g$ over $K_n$ has a root in $K_{n+1} \subseteq
\Omega$: $\Omega$ is \omterm{def:b3:galois:closure}{algebraically closed}, and \omterm{def:b3:galois:algebraic}{algebraic} over
$K$ (each $K_n$ is, by transitivity,
\cref{cor:b3:galois:algclosed}): $\Omega$ is an \omterm{def:b3:galois:closure}{algebraic
closure}.

\emph{Uniqueness.} Let $\Omega, \Omega'$ be two \omterm{def:b3:galois:closure}{algebraic
...
over $\tau_0(E_0)$ having a root $\beta$ in the \omterm{def:b3:galois:closure}{algebraically
...
image, isomorphic to $\Omega$, is \omterm{def:b3:galois:closure}{algebraically closed}, and
$\Omega'$ is \omterm{def:b3:galois:algebraic}{algebraic} over it: for $x \in \Omega'$,
...
\omterm{def:b3:galois:algebraic}{algebraic} numbers $\bar\Q = \{z \in \C : z \text{ algebraic over
} \Q\}$ form an \omterm{def:b3:galois:closure}{algebraic closure} --- a subfield of $\C$ by
\cref{cor:b3:galois:algclosed}, \omterm{def:b3:galois:closure}{algebraically closed} because $\C$
is (d'Alembert--Gauss, proved by complex analysis in
\cref{ch:b3:holomorphic}) and roots of polynomials over $\bar\Q$
are \omterm{def:b3:galois:algebraic}{algebraic} over $\Q$ by transitivity.
\end{remark}
...
\begin{enumerate}
  \item A \omterm{thm:b3:galois:finitefields}{finite field} has cardinality a prime power, and for
        each $q$ there is exactly one field $\mathbb F_q$ with
        $q$ elements up to isomorphism: the \omterm{thm:b3:galois:splitting}{splitting field} of
...
\begin{proof}
(1) A \omterm{thm:b3:galois:finitefields}{finite field} $E$ has characteristic $p > 0$ and is a
...
hence is a \omterm{thm:b3:galois:splitting}{splitting field} of it over $\mathbb F_p$ ---
determining $E$ up to isomorphism
(\cref{thm:b3:galois:splitting}). Conversely, in a \omterm{thm:b3:galois:splitting}{splitting
...
elements since $X^q - X$ is \omterm{def:b3:galois:separable}{separable}: its derivative is
...
(3) If $\mathbb F_{p^m} \subseteq \mathbb F_{p^n}$, the \omterm{thm:b3:galois:tower}{tower law}
...
root in $\mathbb F_2$, hence is \omterm{def:b3:rings:divisibility}{irreducible}. Writing $\omega =
...
instance of the \omterm{thm:b3:galois:fundamental}{Galois correspondence}.
\end{example}
...
polynomial} if it has no repeated root in a \omterm{thm:b3:galois:splitting}{splitting field} ---
equivalently $\gcd(P, P') = 1$ (a repeated root is a common
root; conversely, over the \omterm{thm:b3:galois:splitting}{splitting field}, a common root is
repeated; and the gcd does not change under \omterm{def:b3:galois:extension}{field extension},
\cref{cor:b3:modules:descent}'s argument). An \omterm{def:b3:galois:algebraic}{algebraic element}
is separable if its \omterm{def:b3:galois:algebraic}{minimal polynomial} is; an extension $L/K$ is
separable if all its elements are.
\end{definition}

\begin{proposition}\label{prop:b3:galois:perfect}
An \emph{\omterm{def:b3:rings:divisibility}{irreducible}} $P \in K[X]$ is \omterm{def:b3:galois:separable}{separable} unless $P' = 0$,
which forces $\operatorname{char} K = p > 0$ and $P \in K[X^p]$.
Consequently every \omterm{def:b3:galois:algebraic}{algebraic} extension of a field of
characteristic $0$, and of a \omterm{thm:b3:galois:finitefields}{finite field}, is \omterm{def:b3:galois:separable}{separable} (such
...
Q(X^p)$. Over a \omterm{thm:b3:galois:finitefields}{finite field}, every element is a $p$-th power
(\omterm{thm:b3:galois:finitefields}{Frobenius} is onto), so $Q(X^p) = \sum b_k^p X^{pk} = (\sum
b_kX^k)^p$ is not \omterm{def:b3:rings:divisibility}{irreducible}: $P' = 0$ cannot happen for
\omterm{def:b3:rings:divisibility}{irreducible} $P$ there either.
\end{proof}

\begin{proposition}[Counting embeddings]\label{prop:b3:galois:embeddings}
Let $L = K(\alpha_1, \dots, \alpha_r)$ be finite over $K$, and
$\sigma \colon K \to \Omega$ an embedding into an \omterm{def:b3:galois:closure}{algebraically
closed field}. Then the number of extensions of $\sigma$ to $L$
is at most $[L:K]$, with equality if $L/K$ is \omterm{def:b3:galois:separable}{separable}. In
...
[K(\alpha):K]$, with equality iff $\pi_\alpha$ is \omterm{def:b3:galois:separable}{separable}
...
$\leq [L : K(\alpha_1)]$ ways by induction; multiply (\omterm{thm:b3:galois:tower}{tower
law}). In the \omterm{def:b3:galois:separable}{separable} case both counts are equalities: \omterm{def:b3:galois:algebraic}{minimal
polynomials} over the bigger field $K(\alpha_1)$ divide those
over $K$, hence remain \omterm{def:b3:galois:separable}{separable}.
\end{proof}

\begin{theorem}[Primitive element]\label{thm:b3:galois:primitive}
Every finite \emph{\omterm{def:b3:galois:separable}{separable}} extension is simple: $L =
...
($\Omega$ an \omterm{def:b3:galois:closure}{algebraic closure}). The polynomial
\[
...
if it is the \omterm{thm:b3:galois:splitting}{splitting field} of a \emph{\omterm{def:b3:galois:separable}{separable}} polynomial
...
If $L/K$ is \omterm{def:b3:galois:galois}{Galois}, then $\abs{\operatorname{Gal}(L/K)} =
[L:K]$; moreover $L/F$ is \omterm{def:b3:galois:galois}{Galois} for every intermediate field $K
\subseteq F \subseteq L$, and every $F$-embedding $L \to \Omega
\supseteq L$ has image $L$ (\emph{normality}).
\end{proposition}

\begin{proof}
Let $L$ split the \omterm{def:b3:galois:separable}{separable} $P$ over $K$, and fix an \omterm{def:b3:galois:closure}{algebraic
closure} $\Omega \supseteq L$. $L/K$ is \omterm{def:b3:galois:separable}{separable}: it is generated
...
of the \omterm{def:b3:galois:separable}{separable} $P$, whose \omterm{def:b3:galois:algebraic}{minimal polynomials} divide $P$)
yields exactly $[L:K]$ extensions of $K \hookrightarrow \Omega$
(in the inductive step, the \omterm{def:b3:galois:algebraic}{minimal polynomial} of a root of $P$
over an intermediate field still divides $P$, hence is
\omterm{def:b3:galois:separable}{separable}). Each such embedding $\tau \colon L \to \Omega$
...
intermediate $F$: $L$ is also the \omterm{thm:b3:galois:splitting}{splitting field} of $P$ over
$F$, and $P$ remains \omterm{def:b3:galois:separable}{separable}: $L/F$ is \omterm{def:b3:galois:galois}{Galois}; the same
...
Let $L/K$ be a \omterm{def:b3:galois:galois}{Galois extension} with group $G =
...
\begin{enumerate}
  ...
  \item $H \trianglelefteq G$ iff $L^H/K$ is \omterm{def:b3:galois:galois}{Galois}, and then
...
\alpha$. The \omterm{def:b3:galois:algebraic}{minimal polynomial} $\pi_\alpha$ over $K$ has \omterm{def:b3:galois:extension}{degree}
$\geq 2$ and is \omterm{def:b3:galois:separable}{separable} ($L/K$ \omterm{def:b3:galois:separable}{separable},
\cref{prop:b3:galois:galoisorder}), so it has another root
$\beta \neq \alpha$ in an \omterm{def:b3:galois:closure}{algebraic closure} $\Omega \supseteq L$.
...
(2) For a subgroup $H$: $L/L^H$ is \omterm{def:b3:galois:galois}{Galois}
...
$L/F$ \omterm{def:b3:galois:galois}{Galois} gives $L^{\operatorname{Gal}(L/F)} = F$ by (1)
applied to $L/F$. The two maps are mutually inverse; they
reverse inclusions evidently. \omterm{def:b3:galois:extension}{Degrees}: $[L:L^H] = \abs H$ just
...
remains to see $F/K$ is \omterm{def:b3:galois:galois}{Galois}: $F$ is \omterm{def:b3:galois:separable}{separable} over $K$
(inside the \omterm{def:b3:galois:separable}{separable} $L/K$), and $F = K(\gamma)$
...
coefficients
fixed by $G$, hence in $K$ by (1): it is a \omterm{def:b3:galois:separable}{separable polynomial}
of $K[X]$ split by $F$, and its roots generate $F$: $F/K$ is
\omterm{def:b3:galois:galois}{Galois}. Conversely, if $F = L^H$ with $F/K$ \omterm{def:b3:galois:galois}{Galois}, normality of
...
{\small The \omterm{thm:b3:galois:fundamental}{Galois correspondence} for the \omterm{thm:b3:galois:splitting}{splitting field} $L$ of
...
subextension $\Q(\iu\sqrt3)/\Q$ that is \omterm{def:b3:galois:galois}{Galois}; the three
...
\zeta_n^k\bigr)$, of \omterm{def:b3:galois:extension}{degree} $\varphi(n)$; grouping the roots of
...
$\Phi_n$ is \omterm{def:b3:rings:divisibility}{irreducible} over $\Q$; hence $[\Q(\zeta_n) : \Q] =
\varphi(n)$ and
\[
...
\]
The extension $\Q(\zeta_n)/\Q$ is thus \omterm{def:b3:galois:galois}{Galois} with \emph{abelian}
group.
\end{theorem}
...
\mid g(X^p)$ in $\Z[X]$ (\omterm{def:b3:galois:algebraic}{minimal polynomial}, then Gauss again).
Reduce mod $p$: $\bar g(X^p) = \bar g(X)^p$ (\omterm{thm:b3:galois:finitefields}{Frobenius} on
$\mathbb F_p[X]$: coefficientwise $a^p = a$, and freshman's
dream), so $\bar f \mid \bar g^{\,p}$: $\bar f$ and $\bar g$
share an \omterm{def:b3:rings:divisibility}{irreducible} factor, and $\bar\Phi_n = \bar f\bar g$ has
...
root of $f$: $f = \Phi_n$, \omterm{def:b3:rings:divisibility}{irreducible}. Consequently
$[\Q(\zeta_n):\Q] = \varphi(n)$, and $\Q(\zeta_n)$ is the
\omterm{thm:b3:galois:splitting}{splitting field} of the \omterm{def:b3:galois:separable}{separable} $X^n - 1$ (all roots are powers
of $\zeta_n$): \omterm{def:b3:galois:galois}{Galois}. An automorphism $\sigma$ sends $\zeta_n$
...
$z \in \C$ is \omterm{def:b3:galois:constructible}{constructible} iff there is a tower $\Q = F_0
\subseteq F_1 \subseteq \dots \subseteq F_r$ with $[F_{i+1} :
F_i] = 2$ and $z \in F_r$. In particular, a \omterm{def:b3:galois:constructible}{constructible number}
is \omterm{def:b3:galois:algebraic}{algebraic} of \omterm{def:b3:galois:extension}{degree} a power of $2$ over
...
too (adjoin $\iu$: one more quadratic step). The \omterm{def:b3:galois:extension}{degree}
consequence: $[\Q(z):\Q]$ divides $[F_r : \Q] = 2^r$ (\omterm{thm:b3:galois:tower}{tower
law}).

($\Leftarrow$) The \omterm{def:b3:galois:constructible}{constructible numbers} form a field: sums and
differences by parallelograms (parallels are \omterm{def:b3:galois:constructible}{constructible}:
...
therefore \omterm{def:b3:galois:constructible}{constructible} by induction on the tower: each step
...
\begin{enumerate}
  \item \emph{Duplication of the cube}: $\sqrt[3]2$ has \omterm{def:b3:galois:extension}{degree}
        $3$, not a power of $2$.
  \item \emph{Trisection of the angle}: trisecting $60^\circ$
        requires $\cos 20^\circ$, a root of the \omterm{def:b3:rings:divisibility}{irreducible} $8X^3
        - 6X - 1$: \omterm{def:b3:galois:extension}{degree} $3$.
...
\end{enumerate}
Also, the regular $n$-gon is \omterm{def:b3:galois:constructible}{constructible} iff
$\varphi(n)$ is a power of $2$ (Gauss--Wantzel; the ``if'' uses
the weekend problem's method, the ``only if'' is
\cref{thm:b3:galois:wantzel} applied to $\zeta_n$, of \omterm{def:b3:galois:extension}{degree}
$\varphi(n)$). For $n = 7$: $\varphi(7) = 6$: the regular
heptagon is impossible; for $n = 17$: $\varphi(17) = 16 = 2^4$:
\omterm{def:b3:galois:constructible}{constructible} --- the weekend problem constructs it.
\end{corollary}

\begin{proof}
(1) $X^3 - 2$ is \omterm{def:b3:rings:divisibility}{irreducible} (\omterm{thm:b3:rings:criteria}{Eisenstein}). (2) From $\cos
...
12, \pm\frac14, \pm\frac18$ fail), hence is \omterm{def:b3:rings:divisibility}{irreducible}: \omterm{def:b3:galois:extension}{degree}
$3$. A general $60^\circ$ angle is \omterm{def:b3:galois:constructible}{constructible}, so a trisector
would construct $c$. (3) If $\sqrt\pi$ were \omterm{def:b3:galois:constructible}{constructible} it
would be \omterm{def:b3:galois:algebraic}{algebraic}, hence also $\pi$. The $n$-gon statement: the
\omterm{def:b3:galois:extension}{degree} of $\zeta_n$ is $\varphi(n)$
(\cref{thm:b3:galois:cyclotomicirred}); necessity follows from
Wantzel; for sufficiency, the \omterm{def:b3:galois:galois}{Galois group}, abelian of order
...
radicals}\index{solvable by radicals} if its \omterm{thm:b3:galois:splitting}{splitting field} is
...
$K(\sqrt[n]a)/K$ is \omterm{def:b3:galois:galois}{Galois} with \emph{cyclic} group. Conversely
--- not needed below --- every cyclic extension of \omterm{def:b3:galois:extension}{degree} $n$ is
of this form. Moreover $K(\zeta_n)/K$ is \omterm{def:b3:galois:galois}{Galois} with
\emph{abelian} group, for any $K$ of characteristic $0$.
\end{lemma}

\begin{proof}
$X^n - a$ is \omterm{def:b3:galois:separable}{separable} ($\gcd$ with $nX^{n-1}$: $a \neq 0$) and
splits in $K(\alpha)$, $\alpha^n = a$: its roots are the
$\zeta^k\alpha \in K(\alpha)$. So $K(\alpha)/K$ is \omterm{def:b3:galois:galois}{Galois}; the
...
remark below). For $K(\zeta_n)$: it splits the \omterm{def:b3:galois:separable}{separable} $X^n -
...
Let $K$ be of characteristic $0$ and $P \in K[X]$ with \omterm{thm:b3:galois:splitting}{splitting
field} $L$. If $P$ is \omterm{def:b3:galois:radical}{solvable by radicals}, then
$\operatorname{Gal}(L/K)$ is a \omterm{def:b3:groups:derived}{solvable group}. (The converse is
also true; we shall not need it.)\index{Galois' solvability
theorem}
\end{theorem}

\begin{proof}
Step 1: \emph{enlarge the \omterm{def:b3:galois:radical}{radical} tower to a \omterm{def:b3:galois:galois}{Galois} one.} Let $L
\subseteq M$ with $M/K$ \omterm{def:b3:galois:radical}{radical}, with root exponents $n_1, \dots,
n_r$ and $n = n_1\cdots n_r$. First adjoin $\zeta_n$: the tower
$K \subseteq K(\zeta_n) \subseteq M(\zeta_n)$ is still \omterm{def:b3:galois:radical}{radical}
($\zeta_n$ is a root of unity: a \omterm{def:b3:galois:radical}{radical} step, $\zeta_n^n = 1$),
...
\omterm{def:b3:galois:closure}{algebraic closure}: $N$ is the \omterm{thm:b3:galois:splitting}{splitting field} of the product of
\omterm{def:b3:galois:algebraic}{minimal polynomials} of a generating set (characteristic $0$:
finite and \omterm{def:b3:galois:separable}{separable}), hence $N/K$ is \omterm{def:b3:galois:galois}{Galois}; and $N$ is
\omterm{def:b3:galois:radical}{radical} over $K$: each $\sigma(M(\zeta_n))$ is \omterm{def:b3:galois:radical}{radical} over $K$
(apply $\sigma$ to a \omterm{def:b3:galois:radical}{radical} tower), and a composite of \omterm{def:b3:galois:radical}{radical}
extensions is \omterm{def:b3:galois:radical}{radical} (concatenate the towers: if $F'/K$ is
\omterm{def:b3:galois:radical}{radical} with tower adjoining $\beta_j$, then $F''(\beta_j)$-type
steps remain \omterm{def:b3:galois:radical}{radical} over any bigger base).

Step 2: \emph{read \omterm{def:b3:groups:derived}{solvability} off the \omterm{def:b3:galois:galois}{Galois} tower.} So assume
$L \subseteq N$, $N/K$ \omterm{def:b3:galois:galois}{Galois} and \omterm{def:b3:galois:radical}{radical} with tower $K
...
\dots \supseteq G_s = \{e\}$. Each $E_{i+1}/E_i$ is \omterm{def:b3:galois:galois}{Galois} with
cyclic group (\cref{lem:b3:galois:cyclicsteps}), so by the
fundamental theorem applied to the \omterm{def:b3:galois:galois}{Galois extension} $N/E_i$
...
\omterm{def:b3:galois:galois}{Galois} with abelian group $G/G_0$
(\cref{lem:b3:galois:cyclicsteps}). The chain exhibits $G$ as
\omterm{def:b3:groups:derived}{solvable} (\cref{prop:b3:groups:derived}). Finally
$\operatorname{Gal}(L/K)$ is a \emph{quotient} of $G$: $L/K$ is
\omterm{def:b3:galois:galois}{Galois} ($P$ \omterm{def:b3:galois:separable}{separable} in characteristic $0$) and restriction $G
\to \operatorname{Gal}(L/K)$ is onto
(\cref{thm:b3:galois:fundamental}(3) with $H =
\operatorname{Gal}(N/L)$); quotients of \omterm{def:b3:groups:derived}{solvable groups} are
\omterm{def:b3:groups:derived}{solvable}.
\end{proof}

\begin{corollary}[Unsolvability of the quintic]
\label{cor:b3:galois:quintic}
There are polynomials of \omterm{def:b3:galois:extension}{degree} $5$ over $\Q$ that are not
\omterm{def:b3:galois:radical}{solvable by radicals}: for instance $X^5 - 4X + 2$, whose \omterm{def:b3:galois:galois}{Galois
group} is $S_5$ (\cref{exo:b3:galois:11}), a \omterm{def:b3:groups:derived}{non-solvable} group
(\cref{cor:b3:groups:snnotsolvable}). No general formula in
\omterm{def:b3:galois:radical}{radicals} can exist for \omterm{def:b3:galois:extension}{degree} $\geq 5$.\index{quintic,
unsolvability}
\end{corollary}

\begin{remark}\label{rem:b3:galois:converse}
The converse of \cref{thm:b3:galois:solvable} --- a \omterm{def:b3:groups:derived}{solvable}
\omterm{def:b3:galois:galois}{Galois group} implies \omterm{def:b3:groups:derived}{solvability} by \omterm{def:b3:galois:radical}{radicals} --- is proved by
descending the \omterm{def:b3:groups:derived}{derived series} and showing every cyclic extension
(with enough roots of unity) is \omterm{def:b3:galois:radical}{radical}, via \emph{Lagrange
resolvents}; it explains why \omterm{def:b3:galois:extension}{degrees} $2, 3, 4$ have formulas:
$S_2, S_3, S_4$ are \omterm{def:b3:groups:derived}{solvable}
...
\Q(\sqrt2, \sqrt3)$, and compute the \omterm{def:b3:galois:algebraic}{minimal polynomial} of
$\sqrt2 + \sqrt3$ over $\Q$.
\end{exercise}
...
not $\Q(\alpha)$), determine the \omterm{thm:b3:galois:splitting}{splitting field} $L$ of $X^3 -
2$ and $[L:\Q]$, and check $\operatorname{Aut}_\Q(\Q(\alpha)) =
\{\mathrm{id}\}$: for \omterm{def:b3:galois:galois}{non-Galois} extensions, the automorphism
group can be far smaller than the \omterm{def:b3:galois:extension}{degree}.
\end{exercise}

\begin{exercise}[$\star$]\label{exo:b3:galois:3}
Construct $\mathbb F_9$ as $\mathbb F_3[X]/(X^2+1)$ and find a
generator of $\mathbb F_9^\times$. List the monic \omterm{def:b3:rings:divisibility}{irreducible}
polynomials of \omterm{def:b3:galois:extension}{degrees} $1, 2, 3$ over $\mathbb F_2$, and verify
...
F_{p^{\operatorname{lcm}(m,n)}}$ inside a fixed \omterm{def:b3:galois:closure}{algebraic
...
Let $I_d(q)$ be the number of monic \omterm{def:b3:rings:divisibility}{irreducible} polynomials of
\omterm{def:b3:galois:extension}{degree} $d$ over $\mathbb F_q$. Prove
\[
...
\omterm{thm:b3:galois:splitting}{splitting field} of $(X^2-2)(X^2-3)(X^2-6)$ --- what do you
notice?
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:b3:galois:8}
(The cubic, solved by its group) Let $P = X^3 + pX + q \in
\Q[X]$ be \omterm{def:b3:rings:divisibility}{irreducible} with roots $x_1, x_2, x_3$ and \omterm{thm:b3:galois:splitting}{splitting
...
formulas drop out. Where did \omterm{def:b3:groups:derived}{solvability} of $S_3$ get used?
\end{exercise}
...
1}4$. Conclude that the regular pentagon is \omterm{def:b3:galois:constructible}{constructible}.
\end{exercise}
...
Let $P = X^5 - 4X + 2$ and $G$ its \omterm{def:b3:galois:galois}{Galois group} over $\Q$,
acting on the $5$ roots.
(a) Show $P$ is \omterm{def:b3:rings:divisibility}{irreducible}, and deduce $5 \mid \abs G$; conclude
...
\cref{thm:b3:galois:solvable}, that $P$ is not \omterm{def:b3:galois:radical}{solvable by
radicals}.
\end{exercise}

\begin{exercise}[$\star\star\star$]\label{exo:b3:galois:12}
(The dihedral quartic) Let $\alpha = \sqrt[4]2$ and $L =
\Q(\alpha, \iu)$, the \omterm{thm:b3:galois:splitting}{splitting field} of $X^4 - 2$ over $\Q$.
(a) Show $[L : \Q] = 8$ and that $G =
\operatorname{Gal}(L/\Q)$ is generated by $\sigma\colon
\alpha \mapsto \iu\alpha,\ \iu \mapsto \iu$ and complex
conjugation $\tau$, with $\sigma^4 = \tau^2 = e$ and
$\tau\sigma\tau = \sigma^{-1}$: $G \cong D_4$.
(b) List the subgroup lattice of $D_4$ (ten subgroups) and
match each to its fixed field; verify in particular that
$\Q(\sqrt2)$, $\Q(\iu)$, $\Q(\iu\sqrt2)$ are the three
quadratic subfields, and locate $\Q(\alpha)$,
$\Q(\iu\alpha)$, $\Q(\sqrt2, \iu)$.
(c) Which intermediate fields are \omterm{def:b3:galois:galois}{Galois} over $\Q$? Match
your answer against the \omterm{def:b3:groups:normal}{normal subgroups} of $D_4$, and
explain why $\Q(\alpha)/\Q$ fails while $\Q(\sqrt2)/\Q$
succeeds.
\end{exercise}
...
regular $17$-gon is \omterm{def:b3:galois:constructible}{constructible} --- the first progress on the
...
\begin{enumerate}
  ...
  \item Conclude \emph{a priori}, using
        ...
        $17$-gon --- is \omterm{def:b3:galois:constructible}{constructible}. The rest of the problem
...
\begin{enumerate}[resume]
  \item Where exactly did the argument use that $17$ is a
        ...
        a prime $p$, the regular $p$-gon is \omterm{def:b3:galois:constructible}{constructible} iff $p
        = 2^{2^t} + 1$ for some $t$ \emph{(if $p - 1 = 2^m$,
        show $m$ must itself be a power of $2$)}.
  \item Deduce the complete list of \omterm{def:b3:galois:constructible}{constructible} regular
...
reciprocity.} The periods of Part II hide a treasure. For an
odd prime $p$, the \emph{Legendre symbol} $\bigl(\frac
ap\bigr)$ is $+1$ if $a$ is a nonzero square mod $p$, $-1$ if
it is not, $0$ if $p \mid a$; \cref{exo:b3:galois:4}(b)
(Euler's criterion) gives $\bigl(\frac ap\bigr) \equiv
a^{(p-1)/2} \pmod p$, whence multiplicativity. Write $\zeta =
\eu^{2\iu\pi/p}$, $p^* = (-1)^{(p-1)/2}p$, and define the
\emph{Gauss sum}
\[
g \;=\; \sum_{a=1}^{p-1}\Bigl(\frac ap\Bigr)\zeta^a .
\]
\begin{enumerate}[resume]
  \item Show $\sum_{a=1}^{p-1}\bigl(\frac ap\bigr) = 0$
        (as many squares as nonsquares), and prove the
        alternative form $g = \sum_{a=0}^{p-1}\zeta^{a^2}$
        \emph{(each nonzero square is hit twice, and
        $\sum_{a}\zeta^a = 0$)}. For $p = 17$: relate $g$ to
        the periods of Part II --- show $g = \eta_0 - \eta_1$
        \emph{(the squares mod $17$ are exactly the even
        powers of the generator $3$)}.
  \item Prove $g^2 = p^*$: expand
        \[
        ...
        a)}p\Bigr)\zeta^{c}
        \]
        (set $b = c - a$), substitute $c - a = at$ to evaluate
        the inner sum as $\bigl(\frac{-1}p\bigr)(p - 1)$ for
        $c = 0$ and $-\bigl(\frac{-1}p\bigr)$ otherwise, and
        conclude with question 14. Check numerically: for $p =
        17$, $(\eta_0 - \eta_1)^2 = 17$ (Part II).
  \item Deduce $\sqrt{p^*} \in \Q(\zeta_p)$, and conclude
        that the \emph{unique} quadratic subfield of
        $\Q(\zeta_p)$ is $\Q(\sqrt{p^*})$ --- unique because
        $\operatorname{Gal}(\Q(\zeta_p)/\Q)$ is cyclic
        (\cref{thm:b3:galois:cyclotomicirred}) and a cyclic
        group has exactly one subgroup of index $2$. (Every
        quadratic field embeds in some cyclotomic field ---
        this is the first case of the Kronecker--Weber
        theorem, whose general form lies far ahead.)
  \item Now let $q \neq p$ be another odd prime. Working in
        the ring $\Z[\zeta]$ modulo $q$, prove
        \[
        ...
        \bigl(\frac qp\bigr)$)}.
  \item On the other hand, $g^q = g\,(g^2)^{(q-1)/2} =
        g\,(p^*)^{(q-1)/2}$; using Euler's criterion mod $q$,
        deduce $g^q \equiv \bigl(\frac{p^*}q\bigr)g
        \pmod{q\Z[\zeta]}$, then --- multiplying the two
        expressions for $g^q$ by $g$ and using $g^2 = p^*$,
        invertible mod $q$ --- conclude
        \[
        ...
        Intersect with $\Z$.)}
  \item Unfold $\bigl(\frac{p^*}q\bigr) =
        \bigl(\frac{-1}q\bigr)^{(p-1)/2}\bigl(\frac pq\bigr)$
        and $\bigl(\frac{-1}q\bigr) = (-1)^{(q-1)/2}$ to
        obtain the \emph{law of quadratic reciprocity}:
        \[
        \Bigl(\frac pq\Bigr)\Bigl(\frac qp\Bigr)
        = (-1)^{\frac{p-1}2\cdot\frac{q-1}2} .
        \]
        Verify it on $(p, q) = (17, 3)$ by listing the squares
        mod $17$ and mod $3$, and use it to decide in three
        lines whether $x^2 \equiv 219 \pmod{383}$ is \omterm{def:b3:groups:derived}{solvable}
        ($383$ is prime, $219 = 3\cdot73$).
\end{enumerate}

\medskip
\noindent\textbf{Part VI --- Counting \omterm{def:b3:rings:divisibility}{irreducible} polynomials:
the prime number theorem of $\mathbb F_q[X]$.} Fix a prime power
$q$ and let $N_q(n)$ be the number of \emph{monic \omterm{def:b3:rings:divisibility}{irreducible}}
polynomials of \omterm{def:b3:galois:extension}{degree} $n$ over $\mathbb F_q$; recall from
\cref{exo:b3:galois:6} the factorization of $X^{q^n} - X$ and
the identity $q^n = \sum_{d\mid n}d\,N_q(d)$, which we now
invert, reinterpret, and exploit.
\begin{enumerate}[resume]
  \item (Words) Call a word $w \in \mathbb F_q^n$
        \emph{primitive} if it is not a power $u^{n/d} =
        u\cdots u$ of a strictly shorter word $u$, and let
        $A(d)$ be the number of primitive words of length $d$.
        Show that every word of length $n$ is uniquely a power
        of a primitive word of some length $d \mid n$, so that
        $q^n = \sum_{d \mid n}A(d)$; comparing with
        \cref{exo:b3:galois:6}, conclude $A(d) = d\,N_q(d)$
        for every $d$, and explain this coincidence by an
        explicit bijection: an element $\alpha \in
        \mathbb F_{q^d}$ of \omterm{def:b3:galois:extension}{degree} $d$ has \omterm{thm:b3:galois:finitefields}{Frobenius} \omterm{def:b3:groups:action}{orbit}
        $(\alpha, \alpha^q, \dots, \alpha^{q^{d-1}})$ of
        exactly $d$ distinct elements, and elements of \omterm{def:b3:galois:extension}{degree}
        $d$ correspond $d$-to-one to \omterm{def:b3:rings:divisibility}{irreducibles} of \omterm{def:b3:galois:extension}{degree}
        $d$.
  \item Prove the \emph{M\"obius inversion formula}: if $f(n) =
        \sum_{d\mid n}g(d)$ for all $n$, then $g(n) =
        \sum_{d\mid n}\mu(d)\,f(n/d)$, where $\mu$ is the
        M\"obius function ($\mu(m) = (-1)^{\#\text{prime
        factors}}$ if $m$ is squarefree, $0$ otherwise)
        ...
        \]
  \item Show $N_q(n) \geq \frac1n\bigl(q^n - 2q^{n/2}\bigr) > 0$
        for every $n \geq 1$: a new proof that $\mathbb F_{q^n}$
        exists for all $n$. Interpret the leading term: a random
        monic polynomial of \omterm{def:b3:galois:extension}{degree} $n$ is \omterm{def:b3:rings:divisibility}{irreducible} with
        probability $\sim \frac1n$ --- the \omterm{prop:b3:galois:perfect}{perfect} analogue of
        the prime number theorem, with $\log x$ traded for $n$;
        verify numerically for $q = 2$, $n \leq 4$
        (\cref{exo:b3:galois:6} lists the counts).
  \item Prove the multiplicative companion of question 21:
        \[
        \prod_{\substack{\pi \text{ monic irred.}\\ \deg\pi =
        n}}\pi \;=\; \prod_{d \mid n}
        \bigl(X^{q^d} - X\bigr)^{\mu(n/d)}
        \]
        \emph{(M\"obius inversion in the abelian group of
        nonzero rational functions)}; verify it by hand for $q
...
\begin{enumerate}[resume]
  \item (The second supplement) Part V's method also computes
        $\bigl(\frac2q\bigr)$. Let $\omega = \eu^{2\iu\pi/8}$
        and $g = \omega + \omega^{-1}$. Show $g^2 = 2$
        \emph{($\omega^2 = \iu$)}; then, for an odd prime $q$,
        prove in $\Z[\omega]$ modulo $q$ that
        \[
        g^q \equiv \omega^q + \omega^{-q} \pmod{q\Z[\omega]},
        \]
        and that the right side equals $g$ if $q \equiv \pm1
        \pmod 8$ and $-g$ if $q \equiv \pm3 \pmod 8$. Comparing
        with $g^q = g\,(g^2)^{(q-1)/2} \equiv
        \bigl(\frac2q\bigr)g$ as in question 18, conclude
        \[
        \Bigl(\frac2q\Bigr) = (-1)^{(q^2-1)/8},
        \]
        checking that $(q^2 - 1)/8$ is even exactly when $q
        \equiv \pm1 \pmod 8$. Verify: $2$ is a square mod $7$
        and mod $17$ ($3^2$ and $6^2$), not mod $3$ nor mod
        $5$.
  \item (The zeta function of $\mathbb F_q[X]$) Prove the
        identity of formal power series in $t$:
        \[
        \prod_{n \geq 1}\bigl(1 - t^n\bigr)^{-N_q(n)}
        = \frac1{1 - qt}
        \]
        \emph{(unique factorization into monic \omterm{def:b3:rings:divisibility}{irreducibles}:
        expand each factor as a geometric series and count
        monic polynomials of \omterm{def:b3:galois:extension}{degree} $n$)}. Recover the identity
        $q^m = \sum_{d \mid m}d\,N_q(d)$ of
        \cref{exo:b3:galois:6} by taking logarithms. Check the
        coefficient of $t^2$ by hand for $q = 2$, and use
        question 21's formula to compute $N_2(6) = 9$,
        verifying $2^6 = 1\cdot2 + 2\cdot1 + 3\cdot2 +
        6\cdot9$.
\end{enumerate}
\end{problem}
