...
A Hilbert space is a Banach space whose norm comes from an
inner product --- and that single extra structure restores, in
infinite dimension, almost all of Euclidean geometry:
orthogonal projections exist, every continuous functional is an
inner product against a fixed vector (Riesz), and orthonormal
bases expand every vector in a convergent series with
Pythagorean bookkeeping (Parseval). The chapter's climax is a
debt honored: the trigonometric system is an orthonormal basis
of $L^2$, so Parseval's identity holds for \emph{every}
square-integrable function --- the statement Year 2 could only
...
inner-product space complete for this norm. Examples: $\ell^2$
(\cref{pb:b3:banach:1}) and, the fundamental one, $L^2(\mu)$
with $\langle f, g\rangle = \int\bar fg\,\dd\mu$ ---
complete by Riesz--Fischer (\cref{thm:b3:lp:complete}); the
...
the Hilbert space $H$ and $x \in H$. There is a unique $p_C(x)
...
F^{\,\perp}$ (continuity of the inner product), so
$(F^\perp)^\perp = \bar F$ by the closed case; density iff
$\bar F = H$ iff $F^\perp = 0$.
\end{proof}

\begin{theorem}[Riesz representation]\label{thm:b3:hilbert:riesz}
For every continuous linear functional $\varphi \in H'$ there
...
\begin{enumerate}
  ...
  \item The following are equivalent: (i) $(e_n)$ is a Hilbert
  ...
  \item If $(e_n)$ is a Hilbert basis, $x \mapsto (c_n(x))_n$
        is an isometric isomorphism $H \to \ell^2$
        (\emph{every} infinite-dimensional separable Hilbert
...
\perp$ each $e_k$ (continuity), hence $\perp F$: by uniqueness
...
(x_1, \dots, x_n)$. Consequently every separable Hilbert space
(one with a countable dense subset) has a Hilbert
...
\eu^{\iu nt}$, $n \in \Z$, is a Hilbert basis. Consequently,
for \emph{every} $f \in L^2$ --- in particular every piecewise
continuous $2\pi$-periodic $f$ --- with $c_n(f) =
...
\]
This proves, in full generality, the Parseval identity that
...
coefficients vanish. Continuous $2\pi$-periodic functions are
dense in $L^2(\intcc{-\pi}\pi)$: indeed $\mathcal
C_c(\intoo{-\pi}\pi)$ is dense
(\cref{thm:b3:lp:density}(2)) and such functions extend
periodically and continuously. Trigonometric polynomials are
$\norm\cdot_\infty$-dense among continuous periodic functions
...
Let $H$ be a real Hilbert space and $a \colon H\times H \to
\R$ bilinear, \emph{continuous} ($\abs{a(u,v)} \leq M\norm
...
\begin{proof}
For fixed $u$, $v \mapsto a(u, v)$ is continuous linear: Riesz
...
parallelogram law: these norms come from no inner product.
\end{exercise}
...
two places where the chapter used this continuity.
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:b3:hilbert:4}
Apply Gram--Schmidt to $1, x, x^2$ in $L^2(\intcc{-1}1)$
(Lebesgue measure): obtain the first three normalized
...
Apply Parseval (\cref{thm:b3:hilbert:fourier}) to $f(t) = t$
and $f(t) = t^2$ on $\intcc{-\pi}\pi$ --- now legitimately for
these (continuous, but previously the identity needed
...
attained at exactly one point. \emph{(Complete the square:
...
(Orthogonal projections, characterized) Let $H$ be a Hilbert
...
Let $I \subseteq \R$ be an interval and $w > 0$ a continuous
\emph{weight} on the interior of $I$ such that $\int_I
...
the \emph{orthogonal polynomials} $(p_n)$ for $w$ (monic
...
\begin{enumerate}
  ...
  \item (Roots) Show that $p_n$ has $n$ \emph{distinct} roots,
        all interior to $I$. \emph{(Let $t_1 < \dots < t_m$ be
        the interior sign changes of $p_n$; if $m < n$,
...
\begin{enumerate}[resume]
  \item Show $\deg P_n = n$ with leading coefficient
        ...
        normalization) the orthogonal polynomials for $w = 1$.
  ...
  \item Show that the normalized Legendre polynomials form a
        Hilbert basis of $L^2(\intcc{-1}1)$
        \emph{(Weierstrass, \cref{cor:b3:complete:weierstrass},
        plus density of $\mathcal C$ in $L^2$)}, and expand
...
\begin{enumerate}[resume]
  ...
  \item Show that the weights are positive
        ...
        converges: $Q_n(f) \to \int_I fw$ for every continuous
        $f$ on a compact $I$.
...
\begin{enumerate}[resume]
  \item Show that $T_n(\cos\theta) = \cos n\theta$ defines a
        ...
        \]
        the $T_n$ are the orthogonal polynomials for this
...
\begin{enumerate}[resume]
  ...
  \item (Jacobi matrix) Let $J_n$ be the $n\times n$
        ...
        the interlacing above) tying orthogonal polynomials
...
behind the weights.} Here $I$ is compact and $f \in \mathcal
C^{2n}(I)$.
\begin{enumerate}[resume]
