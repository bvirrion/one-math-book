...
What happens to a holomorphic function near a point where it
is \emph{not} defined? The answer is a complete trichotomy ---
removable point, pole, or essential singularity --- read off a
two-sided power series, the Laurent expansion. One coefficient
of that expansion, the \emph{residue}, controls every contour
integral around the singularity: the residue theorem converts
...
closed paths $\gamma_1, \dots, \gamma_m$; integrals and
...
\end{cases}
\]
\emph{$g$ is continuous}: off the diagonal, clear. Near a
...
the radius): the sum is continuous.

Set $h(z) = \frac1{2\iu\pi}\int_\Gamma g(z, w)\,\dd w$ on
$\Omega$: continuous (uniform continuity of $g$ on compacts),
and holomorphic --- by Morera
...
$z \mapsto g(z, w)$ is holomorphic on $\Omega$ (at $z = w$ the
singularity is removable: $g$ is continuous there and
holomorphic elsewhere --- the extension argument of
\cref{thm:b3:holomorphic:formula}'s proof).

On the open set $\Omega' = \{z \notin \operatorname{im}\Gamma
: \operatorname{Ind}_\Gamma(z) = 0\}$, define $h_1(z) =
\frac1{2\iu\pi}\int_\Gamma\frac{f(w)}{w - z}\dd w$:
holomorphic on $\Omega'$ (Morera or differentiation under the
...
Let $f$ be holomorphic on the annulus $A = \{r < \abs{z - a} <
...
half-series converging normally on compact subannuli. The
expansion is unique.
\end{theorem}
...
$\int_\Gamma$ over a null-index cycle of the holomorphic
...
If $f$ is holomorphic on a punctured disc $D(a, R)\setminus
\{a\}$, expand by Laurent ($r = 0$). Three exclusive
cases:\index{isolated singularity}
\begin{itemize}
  \item all $c_n = 0$ for $n < 0$: \emph{removable
  ...
  \item $c_n \neq 0$ for finitely many, at least one, $n < 0$:
        ...
        equivalently $f = g/(z-a)^m$, $g$ holomorphic, $g(a)
...
$\operatorname{Res}(f, a) = c_{-1}$. A function holomorphic on
...
\delta$ near $a$, so $g = 1/(f - b)$ is holomorphic and
...
$\int_{S_R} \to 0$ (ML), and the residue theorem with the
poles $\eu^{\iu\pi/4}, \eu^{3\iu\pi/4}$ (simple, residues
...
($a > 1$) becomes a residue count inside the unit circle
(\cref{exo:b3:residues:2}).
(d) \emph{Series}: pair $f$ with $\pi\cot(\pi z)$, whose poles
are the integers with residue $1$: the weekend problem sums
...
the residue theorem counts the two enclosed poles (blue). The
...
Let $f$ be meromorphic on $\Omega$, with zeros $z_j$ (orders
$m_j$) and poles $p_k$ (orders $\mu_k$), and $\gamma$ a closed
path in $\Omega$ avoiding them all, with
...
number of the image path $f\circ\gamma$ around $0$.
\end{theorem}

\begin{proof}
Near a zero of order $m$: $f = (z-a)^mg$, $g(a) \neq 0$, so
$\frac{f'}f = \frac m{z - a} + \frac{g'}g$ with the second
term holomorphic near $a$: a simple pole of residue $m$. Near
a pole of order $\mu$: $f = (z-a)^{-\mu}g$ gives residue
$-\mu$. Elsewhere $\frac{f'}f$ is holomorphic. (The zeros and
poles with nonzero index lie in a compact region encircled by
...
Let $f, g$ be holomorphic on $\Omega$, and $\gamma$ a closed
path with $\operatorname{Ind}_\gamma \in \{0,1\}$, zero off
...
continuous in $t$ (the integrand is jointly continuous, the
...
A nonconstant holomorphic function on a connected open set is
an open map. In particular (again) the maximum principle
holds, and a holomorphic bijection has holomorphic
...
(isolated zeros) and let $\delta = \min_{\abs{z - a} =
...
principle: an interior maximum of $\abs f$ is impossible for
a nonconstant $f$, its image around $f(a)$ containing points
of larger modulus. Inverse: a holomorphic bijection $f$ is
open, so $f^{-1}$ is continuous; the zero of $f - f(a)$ at
...
Classify the singularity at $0$ and compute the residue:
\[
\frac{\sin z}{z},\qquad
...
\frac1{\sin z} .
\]
Also give the residue of the third at $z = 1$ and of the last
at $z = \pi$.
\end{exercise}
...
Expand $f(z) = \dfrac1{(z-1)(z-2)}$ in Laurent series in each
...
\emph{not} a residue of $f$ at $0$ ($f$ has no singularity
there), and compute the actual residues of $f$, at $1$ and at
$2$.
\end{exercise}
...
punctured neighborhood of $0$ yet $\eu^{1/z}$ has no pole:
which limit fails?
\end{exercise}
...
(Hurwitz) Let $f_n \to f$ uniformly on compacts, $f_n \in
\mathcal H(\Omega)$, $\Omega$ connected, $f \not\equiv 0$.
...
\emph{all} residues of $f$ is zero \emph{(integrate over
...
pole $z = -1$ has residue $\eu^{\iu\pi(a - 1)}$. Deduce also
...
with residues}]\label{pb:b3:residues:1}
The residue theorem sums series: pairing a rational function
with $\pi\cot(\pi z)$, whose poles sit at the integers, turns
$\sum_{n}f(n)$ into a residue count. We prove the method and
...
\begin{enumerate}
  \item Show that $\pi\cot(\pi z)$ is meromorphic on $\C$ with
        simple poles exactly at $z = n \in \Z$, each of
        residue $1$ \emph{(compute
  ...
  \item Compute the beginning of the Laurent expansion at $0$:
        ...
        is holomorphic and nonzero near $0$, so its
        reciprocal is holomorphic; identify coefficients
        through order $3$).
...
\begin{enumerate}[resume]
  \item Let $f$ be rational, holomorphic at the integers,
        with $\deg(\text{denom}) \geq \deg(\text{num}) + 2$.
        Using the residue theorem on $C_N$ and the bound of
        question 3, prove:
        \[
...
\begin{enumerate}[resume]
  \item Fix $w \in \C\setminus\Z$ and apply the method of
        ...
        simple poles at $\pm w$, whose residues must join the
...
\begin{enumerate}[resume]
  \item Differentiate question 9's expansion term by term
        ...
        obtain, normally on compacts of $\C\setminus\Z$,
        \[
  ...
  \item Verify the duplication identity $\tan\theta =
        ...
        \]
        normally on compacts avoiding $\frac12 + \Z$.
  ...
  \item Verify $\frac1{\sin\theta} = \cot\frac\theta2 -
        ...
        \]
        Check the signs against the residues of
