...
(a) Continuity of the distribution makes ties null events
...
slots is uniform and independent of that ordering
(exchangeability of the $n$-tuple). Hence $R_n$ (the event
``$X_n$ takes the top slot'') is independent of the whole
record history $(R_1, \dots, R_{n-1})$, which is a function
of the relative ordering of the first $n - 1$ variables.
Induction gives full independence with $\P(R_n = 1) =
\frac1n$.

(c) $\sum\P(R_n = 1) = \sum\frac1n = \infty$ with
independence: the second Borel--Cantelli half gives records
...
= 1) = \frac1{n(n+1)}$ (independence), and
\[
...
independent, each all-heads with probability $2^{-\ell}
...
intersecting countably many full-measure events:
...
is independent of $S_n - S_k$, a function of $(Z_{k+1},
...
its independence from the second block is coalition
independence --- pairwise independence of the $Z_i$ only
...
\]
The events increase with $m$; continuity from below gives
...
independent (Borel functions of independent variables,
...
independent sequence $(\varepsilon_n)$, so Kolmogorov's
zero--one law (\cref{thm:b3:probability:zeroone}) forces
...
the expectation is $1$ when the indices pair off (all four
equal, or two distinct pairs, the latter in $3$
arrangements) and $0$ otherwise (an unpaired sign has zero
mean and factors out by independence). Hence
\[
...
sure, and with question 14 the dichotomy is complete.
...
formula for independent variables give
\[
...
only $X_1 \in L^1$ and pairwise independence, and delivers
no rate; Hoeffding asks boundedness and full independence,
...
measurable integrand with values in $\intcc01$. Question
11's strong law promised convergence with no finite-$n$
guarantee; a deterministic grid with $k$ points per axis
costs $k^d$ evaluations, exponential in $d$. Concentration
is what makes Monte Carlo a \emph{method} rather than a
hope.

\textbf{21.} Independence and the product formula:
...
\]
and the symmetric bound for $-S_n$ (same law) doubles the
...
Borel--Cantelli cannot conclude. The lower half of the law
of the iterated logarithm applies the second
Borel--Cantelli lemma to the independent increments
...
{n^\alpha}$ are independent, centered, bounded, with
...
zero--one law) with a deterministic verdict all the same.
\end{solution}
