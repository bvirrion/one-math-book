...
\omterm{def:b3:holomorphic:holo}{holomorphic} $\mathbb D \to \mathbb D$ with $g(0) = 0$, so
...
exchanged). \omterm{def:b3:holomorphic:holo}{Holomorphic} self-maps of the disc are
...
\intoo0\pi$), \omterm{def:b3:holomorphic:holo}{holomorphic} with nonvanishing derivative and
\omterm{def:b3:holomorphic:holo}{holomorphic} inverse (principal $\log$).
(b) $z \mapsto z^2$ doubles arguments: the open quadrant
$\{0 < \arg z < \frac\pi2\}$ maps \omterm{def:b3:conformal:conformal}{conformally} onto $\mathbb
...
+ z}{1 - z}\bigr)^2$.
(d) The \omterm{thm:b3:conformal:autdisc}{Blaschke factor} $\varphi_{1/2}(z) = \frac{z -
...
(b) The square is convex, hence \omterm{def:b3:conformal:simplyconnected}{simply connected}, and proper:
\omterm{def:b3:conformal:conformal}{conformally} $\mathbb D$ (\cref{thm:b3:conformal:rmt}). The cut
plane $\C\setminus\intoc{-\infty}0$ is star-shaped about $1$
(segments from $1$ avoid the cut), hence \omterm{def:b3:conformal:simplyconnected}{simply connected},
and proper: \omterm{def:b3:conformal:conformal}{conformally} $\mathbb D$. The punctured disc: if
...
also $g(w)$ for some $w \neq 0$; two disjoint \omterm{def:b3:topology:topology}{neighborhoods}
of $0$ and $w$ have images that are open and share the value
$\tilde g(0)$, hence share \emph{other} values too (\omterm{def:b3:topology:topology}{open
...
\to A$ conformal. $F$ is zero-free on the \omterm{def:b3:conformal:simplyconnected}{simply connected}
$\mathbb D$, so $F = \eu^L$ for \omterm{def:b3:holomorphic:holo}{holomorphic} $L$
(\cref{def:b3:conformal:simplyconnected}). Let $\sigma$ be
the circle $\abs z = \frac32$ in $A$ and $\gamma =
F^{-1}\circ\sigma$, a closed \omterm{def:b3:topology:pathconnected}{path} in $\mathbb D$; then
\[
...
0\}$ \omterm{def:b3:conformal:conformal}{conformally} onto $\mathbb D$ (Cayley rotated: $\abs{w -
1} < \abs{w + 1}$ iff $\operatorname{Re}w > 0$), with $T(1) =
0$. For $f \in \mathcal F$, $g = T\circ f\colon \mathbb D \to
\mathbb D$ is \omterm{def:b3:holomorphic:holo}{holomorphic} with $g(0) = 0$: Schwarz gives
$\abs{g(z)} \leq \abs z$.
...
\emph{Herglotz extremals}, \omterm{def:b3:conformal:conformal}{conformal maps} onto the right
half-plane.
\end{solution}

\begin{solution}{exo:b3:conformal:7}
(i) Simple connectivity enters exactly twice, through the
existence of \omterm{def:b3:holomorphic:holo}{holomorphic} square roots of zero-free functions
...
bounded maps (Liouville). (iii) \omterm{def:b3:topology:connected}{Connectedness} is used whenever
...
Failure for the annulus: not \omterm{def:b3:conformal:simplyconnected}{simply connected} --- the
square-root construction breaks (e.g.\ $z$ itself, zero-free
on $A$, has no \omterm{def:b3:holomorphic:holo}{holomorphic} square root: the same index
...
\qquad u(0) = \frac12 :
\]
the \omterm{ex:b3:groups:actions}{center} sees exactly the average of the boundary data ---
...
u(sz)$ is harmonic on a \omterm{def:b3:topology:topology}{neighborhood} of $\bar{\mathbb D}$,
...
\frac{1+r}{1-r}\,u(0).
\]
Let $s \to 1^-$ at fixed $r\eu^{\iu\varphi}$ (\omterm{def:b3:topology:continuity}{continuity} of
...
Locally, $u = \operatorname{Re}F$ with $F$ \omterm{def:b3:holomorphic:holo}{holomorphic}
(\cref{prop:b3:conformal:harmonicholo}), so $u\circ f =
\operatorname{Re}(F\circ f)$ is harmonic wherever defined:
harmonicity is \omterm{def:b3:conformal:conformal}{conformally} invariant. On $\mathbb H$: the
principal logarithm gives $\log z = \ln\abs z + \iu\arg z$
\omterm{def:b3:holomorphic:holo}{holomorphic} on $\mathbb H$, so $u = \frac1\pi\arg z =
...
the \omterm{ex:b3:conformal:mobius}{Cayley map} (which sends $\mathbb D \to \mathbb H$ after
...
\cref{exo:b3:conformal:8}(c); evaluating at the \omterm{ex:b3:groups:actions}{center}
...
(b) $h(z) = f(z)/z$ (\omterm{thm:b3:residues:riemanncw}{removable singularity} at $0$) is
\omterm{def:b3:holomorphic:holo}{holomorphic} on $\mathbb D$ with $\abs h \leq 1$ (Schwarz);
$\abs h < 1$ everywhere, else the maximum principle
(\omterm{def:b3:topology:interior}{interior} maximum of $\abs h$) would make $h$ a unimodular
constant, i.e.\ $f$ a rotation --- excluded. On the \omterm{def:b3:topology:compact}{compact}
...
\omterm{def:b3:groups:action}{orbits} tend to $0$; quantitatively $f(z) = \frac z2(1 + z)$
gives $\abs{f(z)} \leq \frac{3}{4}\abs z$ on $\abs z \leq
\frac12$, and once the \omterm{def:b3:groups:action}{orbit} is small, $\abs{f(z)} \approx
...
-u_{yy} = u_{xx} = \partial_x(u_x)$). On a star-shaped \omterm{def:b3:topology:topology}{open
set} the Poincar\'e lemma (\cref{thm:b3:forms:poincare}; or
the primitive construction of
\cref{thm:b3:holomorphic:cauchy} applied to the \omterm{def:b3:holomorphic:holo}{holomorphic}
$u_x - \iu u_y$) provides $v$ with $\dd v = \omega$, i.e.\
the Cauchy--Riemann system: $u + \iu v$ is \omterm{def:b3:holomorphic:holo}{holomorphic}. Two
conjugates differ by a function with vanishing gradient: a
constant (\omterm{def:b3:topology:connected}{connectedness}).
...
exists --- a conjugate would be a \omterm{def:b3:topology:continuity}{continuous} determination
...
\textbf{1.} $g$ is injective and \omterm{def:b3:holomorphic:holo}{holomorphic}; on $C_\rho$
...
\textbf{3.} $f(z)/z = 1 + a_2z + \cdots$ is \omterm{def:b3:holomorphic:holo}{holomorphic} and
zero-free on $\mathbb D$ ($f$ vanishes only at $0$, simply:
injectivity), hence so is $f(z^2)/z^2$, which has a
\omterm{def:b3:holomorphic:holo}{holomorphic} square root $\varphi$ with $\varphi(0) = 1$
...
with $w = \frac{1 + z}{1 - z}$, a \omterm{def:b3:conformal:conformal}{conformal map} of $\mathbb D$
onto the right half-plane; $w^2$ maps that half-plane
\omterm{def:b3:conformal:conformal}{conformally} onto $\C\setminus\intoc{-\infty}0$; then
$\frac{\cdot - 1}4$ gives
$\C\setminus\intoc{-\infty}{-\frac14}$: injective at each
stage, image as claimed.

\textbf{6.} $F = \frac{cf}{c - f}$: since $c \notin f(\mathbb
D)$, the denominator never vanishes: $F$ is \omterm{def:b3:holomorphic:holo}{holomorphic}, and
...
$\gamma = f^{-1}\circ[0, f(z)]$ is a $\mathcal C^1$ \omterm{def:b3:topology:pathconnected}{path}
from $0$ to $z$ in $\mathbb D$ ($f^{-1}$ \omterm{def:b3:holomorphic:holo}{holomorphic},
...
\textbf{18.} The \omterm{thm:b3:holomorphic:analytic}{Cauchy estimates} on the circle $\abs z =
...
segment $[0, w]$ is \omterm{def:b3:topology:connected}{connected}, meets $U$ (at $0$) and its
...
the \omterm{def:b3:topology:topology}{open set} $f(\mathbb D)$ --- is at distance $\geq
...
and the \omterm{def:b3:topology:connected}{connectedness} argument of question 19 keeps it in
...
\textbf{22.} The single principle is \omterm{thm:b3:hilbert:parseval}{Parseval}: univalence
...
lemma. One \omterm{def:b3:topology:interior}{interior} inequality, plus rigidity of its
...
f$ locally uniformly: $f$ is \omterm{def:b3:holomorphic:holo}{holomorphic} with $f_n' \to f'$
...
injective: $f \in \mathcal S$. A \omterm{def:b3:topology:continuity}{continuous} functional (such
as $f \mapsto \abs{a_2} = \frac{\abs{f''(0)}}2$) on a
\omterm{def:b3:topology:compact}{compact} class \emph{attains} its supremum: extremal
