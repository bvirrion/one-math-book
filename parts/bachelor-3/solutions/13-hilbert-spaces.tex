...
\operatorname{im}P \oplus \ker P$ (algebraically, from $P^2
...
0$. For $x$ in the closure: given $\varepsilon$, pick $x' =
...
\int_0^1f$ in $L^2(\R/\Z)$: the orbit averages of an
...
$D(s_j) = 0$ for some interior $s_j$, that zero is double
...
n^2$. At interior points, $\abs{T_n'(t)} \leq
\frac{n}{\sqrt{1 - t^2}} = O(n)$: the quadratic blow-up
lives only at the edges (Bernstein's interior bound versus
Markov's global one).
...
\;\leq\; \frac{M\,h_n}{(2n)!} ,
\]
and the intermediate value theorem applied to the continuous
