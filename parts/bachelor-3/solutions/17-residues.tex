...
\to 0$ ($a > 0$). Residue: at $z = -1 = \eu^{\iu\pi}$,
...
normally on $\bar D(0, R)$, with holomorphic partial sums
...
holomorphic there; $R$ being arbitrary, $P$ is entire. On
...
\]
On a compact $K \subseteq \C\setminus\Z$: $P_N \to P$ and
$P_N' \to P'$ uniformly
...
$P(z) = (z - m)\,\psi(z)$ with $\sigma, \psi$ holomorphic
and nonvanishing at $m$ (factor the power series): $Q =
\sin(\pi z)/P = \sigma/\psi$ extends holomorphically and
...
hence everywhere by continuity: $Q$ is constant. As $z \to
...
the limit by continuity of $\exp$. The double series
\[
...
\textbf{16.} On a compact $K \subseteq \C\setminus\Z$ the
...
\]
the convergence normal on compacts of $\C\setminus\Z$
(terms $O(n^{-2})$).
...
\]
normally on compacts avoiding $\frac12 + \Z$ (terms
$O(m^{-2})$).
...
\]
The moral: each kernel is meromorphic with poles on an
arithmetic progression and prescribed residues. The
cotangent puts residue $1$ at every integer and sums
...
integer poles but alternates the residues $(-1)^n$, hence
