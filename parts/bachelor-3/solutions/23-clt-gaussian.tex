...
\varphi_{(a+b)X_1}(\xi)$ (independence and
...
$\sqrt n$ for the Gaussian ($\alpha = 2$), $n$ itself for
...
Gaussian basin, and the Cauchy basin is reserved for laws
...
(c) For continuous $F$, the variables $U_k = F(X_k)$ are
...
are unchanged), and the second because a continuous $F$,
...
of Kolmogorov's distribution) tests \emph{any} continuous
model against data.
\end{solution}
...
$\theta$. Taking expectations in the $i$-th swap, the
...
question 3's bound for the last swap ($i = n$, no Gaussian
left):
\[
\bigl|\E f(T_n) - \E f(G_n)\bigr| \leq
...
$n^{-1/8}$ --- the Gaussian half of the hybrid did the
extra smoothing.
...
0$ (independence factors them as in question 3), so only
...
n^{-(k+1)/2}$; the hidden-Gaussian bound charges
...
already imitates the Gaussian. The saturation is
structural: swapping adds $n$ swap errors \emph{in
absolute value}, renouncing all cancellation between
swaps. The Fourier proof compares characteristic functions,
...
independent Gaussians, hence Gaussian
(\cref{exo:b3:clt:3}): $N = CZ$ is a Gaussian vector; its
...
Gaussian with moments of all orders
...
Gaussian vector with mean $0$ and covariance $\Sigma$, and
...
independence and centering), so $\P(\norm{T_n} > A) \leq
\operatorname{tr}\Sigma/A^2$, and likewise for $N$:
tightness. Given a bounded continuous $f$ and $\varepsilon
...
continuous with compact support, so its two-dimensional
...
\to \E f(N)$ for every bounded continuous $f \colon \R^2
...
\textbf{17.} For bounded continuous $g \colon \R \to \R$,
the map $x \mapsto g(\langle t, x\rangle)$ is bounded
continuous on $\R^2$, so question 16 gives $\E g(\langle
...
L^6$. Question 16 yields the displayed joint Gaussian
limit, and \cref{thm:b3:clt:gaussianvector}(2): the two
limit coordinates are independent exactly when the
...
(distribution functions converge at the continuity points
...
affine rule for characteristic functions); Slutsky's sum
...
Gaussian (a ``chi-squared'' law) --- when the first
derivative dies, the second-order term of Taylor dictates
a non-Gaussian limit. (b) Here $\sqrt n(\hat p_n - p)
...
independent of the pair $(X_i, Y_i)$ (coalitions). For $A
...
Gaussian makes the swap errors $o(\frac1n)$ each: sums go
Gaussian --- with Taylor as the local comparison tool.
...
the Gaussian and Poisson limits are its two oldest
dividends.
...
the Gaussian to third order, and the rate improves (Part
IV's matched-moment question). Numerically: $\P(S = 0) =
0.9^{20} = 0.1216$, while the Gaussian
