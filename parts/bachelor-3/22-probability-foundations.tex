...
Among $n$ people with independent, uniform birthdays over
...
argument the product formula of independence makes
...
$\binom n2$ pair-collision events are not independent, yet
...
(Records) Let $(X_n)_{n\geq1}$ be i.i.d.\ with continuous
...
\medskip
\noindent\textbf{Part VI --- What full independence buys:
maximal inequalities and random series.} Etemadi spends
only pairwise independence; the remaining parts exploit the
full (mutual) version. Let $(Z_n)$ be independent centered
...
\begin{enumerate}[resume]
  \item (Kolmogorov's maximal inequality) For $\varepsilon
        ...
        independence of the coalitions $(Z_1, \dots, Z_k)$
        and $(Z_{k+1}, \dots, Z_n)$,
        \cref{thm:b3:probability:independence})}. Point out
        the step where pairwise independence would no
        longer suffice.
...
inequality.} The strong law says $\frac{S_n}n \to m$;
...
\begin{enumerate}[resume]
  \item (Hoeffding's lemma)
        ...
        its chord, take expectations, and study $\varphi(t)
  ...
  \item (Hoeffding's inequality) Let $X_1, \dots, X_n$ be
        independent with $a_i \leq X_i \leq b_i$ and $S_n =
        ...
        independence and question 17, then optimize over
        $\lambda > 0$)}.
  \item (The strong law, bounded case, with a rate) Let the
        ...
        Borel--Cantelli: a second proof of the strong law
        for bounded variables --- no truncation, an
        exponential rate at every finite $n$, but bounded
        summands and full independence. Compare the
        hypotheses with Etemadi's.
  \item (Monte Carlo, guaranteed at fixed $n$) Let $g
        \colon \intcc01^d \to \intcc01$ be measurable and
...
\begin{enumerate}[resume]
  ...
  \item Along the doubling subsequence $n_j = 2^j$, show
        ...
        \]
        and reflect: the \emph{law of the iterated
        ...
        applied to independent blocks; both halves are
