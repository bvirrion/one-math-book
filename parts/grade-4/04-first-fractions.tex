...
\omterm{def:g3:division:half}{Half} an apple, a \omterm{def:g3:division:half}{quarter} of an hour, three \omterm{def:g3:division:half}{quarters} of the class:
\omterm{def:g4:fractions:def}{fractions} name the pieces of things. This chapter introduces the
writing $\frac{a}{b}$, reads \omterm{def:g4:fractions:def}{fractions} on pictures and number lines,
and compares the simplest ones. (\omterm{def:g4:fractions:def}{Fractions} grow up in
\cref{ch:g6:fractions}.)

\section{Naming the pieces}

\begin{definition}[Fraction]\label{def:g4:fractions:def}
Cut a whole into equal parts and take some --- that is a
\emph{fraction}\index{fraction}:
\[
\frac{3}{4}
\quad
\begin{array}{l}
3 \text{: how many parts are taken (the \emph{numerator}\index{numerator});}\\
4 \text{: how many equal parts the whole is cut into (the
\emph{denominator}\index{denominator}).}
\end{array}
\]
$\frac12$ is a \emph{\omterm{def:g3:division:half}{half}}, $\frac13$ a \emph{third}, $\frac14$ a
\emph{\omterm{def:g3:division:half}{quarter}}, $\frac{1}{10}$ a \emph{tenth}.
\end{definition}
...
{\small Reading \omterm{def:g4:fractions:def}{fractions} on pictures. The parts must be
...
steps. If $a$ is bigger than $b$, the walk passes $1$: \omterm{def:g4:fractions:def}{fractions} can
...
{\small The line cut in \omterm{def:g3:division:half}{quarters}: $\frac24$ lands on the same point
as $\frac12$; $\frac54$ is one \omterm{def:g3:division:half}{quarter} past $1$; $\frac{11}{4}$ is
$2$ and $\frac34$.}
\end{omfigure}

\begin{example}[Whole numbers hiding in fractions]\label{ex:g4:fractions:whole}
$\frac44 = 1$ (four \omterm{def:g3:division:half}{quarters} make the whole); $\frac82 = 4$;
$\frac{12}{3} = 4$. And $\frac74$ is $1 + \frac34$: one whole and
three \omterm{def:g3:division:half}{quarters} more.
\end{example}

\section{Comparing and adding simple fractions}

\begin{proposition}[Same denominator]\label{prop:g4:fractions:compare}
With the same \omterm{def:g4:fractions:def}{denominator}, the pieces have the same size --- so just
count them:
\[
\frac{2}{8} < \frac{5}{8},
\qquad
...
each \omterm{def:g3:division:half}{half} into two makes \omterm{def:g3:division:half}{quarters}; into five, tenths. (The general
rule is in \cref{ch:g6:fractions}.)
\end{example}

\begin{example}[Quarters of an hour]\label{ex:g4:fractions:hour}
An hour has $60$ minutes, so a \omterm{def:g3:division:half}{quarter} of an hour is
$60 \div 4 = 15$ minutes, and three \omterm{def:g3:division:half}{quarters} of an hour is
...
Write the \omterm{def:g4:fractions:def}{fraction} shown: a pizza cut in $6$ with $5$ slices left;
a chocolate bar of $8$ squares with $3$ eaten (\omterm{def:g4:fractions:def}{fraction} eaten); a
...
\omterm{def:g3:division:half}{quarter}).
\end{exercise}

\begin{exercise}[$\star$]\label{exo:g4:fractions:4}
Place on a number line cut in thirds: $\frac13$; \ $\frac33$; \
$\frac53$; \ $2$. Which \omterm{def:g4:fractions:def}{fraction} lands exactly on $2$?
\end{exercise}
...
Write as a whole number plus a \omterm{def:g4:fractions:def}{fraction} smaller than $1$:
$\frac54$; \quad $\frac73$; \quad $\frac{9}{2}$.
\end{exercise}

\begin{exercise}[$\star$]\label{exo:g4:fractions:8}
How many minutes are: \omterm{def:g3:division:half}{half} an hour; a \omterm{def:g3:division:half}{quarter} of an hour; three
\omterm{def:g3:division:half}{quarters} of an hour; $\frac13$ of an hour?
\end{exercise}
...
pizza. What \omterm{def:g4:fractions:def}{fraction} did they eat together? What \omterm{def:g4:fractions:def}{fraction} is left?
Who ate more?
\end{exercise}
...
What must one always know before comparing two \omterm{def:g4:fractions:def}{fractions} of
something?
\end{exercise}
