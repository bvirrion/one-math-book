...
Grade 3 divided with the tables and small remainders
...
\begin{enumerate}
  \item take the leftmost digit, $7$ (hundreds): $6$ goes $1$ time
        into $7$; write $1$ in the quotient, subtract
        $6$, remainder $1$;
  \item \emph{bring down} the next digit, $4$: now divide $14$
        (tens): $6 \times 2 = 12$; write $2$, remainder $2$;
  \item bring down the $9$: divide $29$: $6 \times 4 = 24$; write
        $4$, remainder $5$;
  \item no digit left: the quotient is $124$, the remainder $5$.
\end{enumerate}
...
\begin{tikzpicture}[scale=0.9]
  \node[font=\normalsize, anchor=west] at (0,2.1) {$7\ 4\ 9$};
  \draw[thick] (1.75,0.1) -- (1.75,2.45);
  \draw[thick] (1.75,2.0) -- (3.6,2.0);
  \node[font=\normalsize, anchor=west] at (2.0,2.25) {$6$};
  \node[font=\normalsize, omDef, anchor=west] at (2.0,1.5)
    {$1\ 2\ 4$};
  \node[font=\small, omThm, anchor=west] at (0.0,1.5)
    {$-6\downarrow$};
  \node[font=\small, anchor=west] at (0.28,0.95) {$1\ 4$};
  \node[font=\small, anchor=west] at (0.28,0.4) {$\ \ 2\ 9$};
  \node[font=\small, anchor=west] at (0.28,-0.15) {$\quad\ \ 5$};
  \node[right, font=\small, omExo] at (4.3,1.5)
    {quotient $124$};
  \node[right, font=\small, omExo] at (4.3,0.9)
    {remainder $5$};
  \node[right, font=\small, omExo] at (4.3,0.3)
    {check: $6 \times 124 + 5 = 749$};
\end{tikzpicture}

{\small The long division of $749$ by $6$, step by step: divide the
hundreds, bring down the tens, divide, bring down the units,
divide.}
\end{omfigure}
...
remainder $1$; units, bring down the $8$: $18 \div 3 = 6$. Quotient
$206$, remainder $0$. Forgetting the middle zero (writing $26$)
...
$6 \times 100 = 600$ and $6 \times 200 = 1\,200$, so the quotient is
...
\begin{enumerate}
  \item ``how many full \dots'' or ``how many each'': the quotient;
  \item ``how many left'': the remainder;
  \item ``how many containers needed for all'': the quotient, plus
        one if the remainder is not zero.
\end{enumerate}
\end{method}
...
$92 \div 4 = 23$ exactly (remainder $0$). Each pays $23$. When the
remainder is zero, the division ``comes out even'' and the sharing
is perfectly fair.
\end{example}
...
Frame each quotient as in \cref{ex:g4:division:size} (between which
two ``round'' numbers?), then compute the long division:
...
Careful with the zero (see \cref{ex:g4:division:zero}):
...
Find the dividend: the division by $7$ gives quotient $58$ and
remainder $3$.
\end{exercise}
...
In a division by $8$, the quotient equals the remainder. What are
the possible dividends? List them all. (The remainder must stay
below $8$.)
\end{exercise}
