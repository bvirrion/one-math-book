...
première opération, avec ses deux signes célèbres $+$ et $=$.

\section{Mettre ensemble}

\begin{definition}[Addition]\label{def:g1:addition:def}
\emph{Ajouter}, c'est mettre ensemble et compter le tout. On écrit
\[
3 + 2 = 5
\]
et on lit~: \guillemotleft{} trois plus deux égale cinq
\guillemotright{}. Le résultat d'une addition s'appelle la
...
{\small Une addition en images~: les deux groupes réunis font un
groupe de cinq.}
\end{omfigure}

\begin{example}[L'ordre n'a pas d'importance]\label{ex:g1:addition:order}
$2 + 3$ et $3 + 2$ rassemblent les mêmes billes~: les deux font
...
\begin{example}[Compléments à 10]\label{ex:g1:addition:friends}
...
pliés sont l'ami. Connaître les amis de $10$ par cœur rend
...
{\small Le cadre de $10$ montre d'un coup d'œil chaque paire
...
\begin{tikzpicture}[scale=0.62]
  ...
  \node[below, font=\small] at (7.2,0.55) {$3$ débordent};
\end{tikzpicture}
...
{\small $8 + 5$ sur la droite numérique~: d'abord un saut jusqu'à
$10$, puis le reste.}
\end{omfigure}
...
\guillemotleft{} Léa a $7$ autocollants. Elle en reçoit $6$ de
plus. Combien en a-t-elle maintenant~? \guillemotright{} Recevoir
plus, c'est ajouter~: $7 + 6 = 13$ (en passant par $10$~:
$7 + 3 = 10$, puis $+3$). Léa a $13$ autocollants. Toujours finir
...
Récite les doubles, puis calcule en s'en servant~: $7 + 8$~; \quad
$5 + 6$~; \quad $9 + 8$.
\end{exercise}
...
\guillemotright{} Écris l'addition et la phrase réponse.
\end{exercise}
...
Trois dés montrent $4$, $6$ et $5$. Quel est le total~? (Ajoute
d'abord deux dés --- choisis les deux qui forment un ami de $10$~!)
\end{exercise}
