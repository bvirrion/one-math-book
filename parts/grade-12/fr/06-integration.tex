...
\section{Problème~: Archimède contre la machine}

\begin{problem}[{Devoir du week-end --- l'aire sous la parabole,
calculée de trois façons à vingt-deux siècles d'écart, et l'identité
secrète du logarithme comme aire}]
\label{pb:g12:integ:1}
Vers 240 avant notre ère, Archimède calcula l'aire exacte d'un
segment de \omterm{def:g11:quad:parabola}{parabole} --- sans \omterm{def:g10:coordgeom:system}{coordonnées}, sans limites, sans
algèbre. Dix-neuf siècles plus tard, les rectangles de Riemann
refirent le calcul par encadrement brutal~; et le théorème
fondamental de l'analyse (\cref{thm:g12:integ:ftc}) le fait
désormais en une ligne. Ce problème dispute les trois matchs, puis
emploie la même machine pour révéler ce qu'est vraiment le
\omterm{def:g12:exp:ln}{logarithme}~: une aire munie d'une symétrie d'échelle.

\medskip
\noindent\textbf{Partie I --- Aisance.}
\begin{enumerate}
  \item Calculer $\displaystyle\int_0^1 (3x^2 - 2x + 1)\,\dd x$,
        \ $\displaystyle\int_1^{\eu} \frac{\dd x}{x}$, \ et
        $\displaystyle\int_0^{\pi/2} \cos x\,\dd x$.
  \item Reconnaître les formes $u'u$
        (\cref{met:g12:integ:uprime})~:
        $\displaystyle\int_0^1 x\,\eu^{x^2}\dd x$ et
        $\displaystyle\int_0^1 \frac{2x}{x^2 + 1}\,\dd x$.
  \item Par parties (\cref{thm:g12:integ:ibp})~:
        $\displaystyle\int_0^1 x\,\eu^x \dd x$ et
        $\displaystyle\int_1^{\eu} \ln x\,\dd x$.
  \item Calculer la \omterm{def:g12:integ:mean}{valeur moyenne} (\cref{def:g12:integ:mean}) de
        $\sin$ sur $\intcc{0}{\pi}$ --- et remarquer qu'elle ne vaut
        \emph{pas} $\frac12$.
  \item Calculer l'aire comprise entre la droite $y = x$ et la
        \omterm{def:g11:quad:parabola}{parabole} $y = x^2$ sur $\intcc{0}{1}$
        (\cref{met:g12:integ:areabetween}).
\end{enumerate}

\medskip
\noindent\textbf{Partie II --- La \omterm{def:g11:quad:parabola}{parabole}, de trois façons.}
\begin{enumerate}[resume]
  \item Le dispositif de Riemann pour l'aire sous $y = x^2$ sur
        $\intcc{0}{1}$~: écrire la somme inférieure $L_n$ et la
        somme supérieure $U_n$ sur $n$ rectangles égaux (comme à
        l'\cref{exo:g12:integ:8}).
  \item Démontrer par récurrence (la machine du
        \cref{pb:g12:seq:1}) la formule de la somme des carrés~:
        \[
        1^2 + 2^2 + \dots + n^2 = \frac{n(n+1)(2n+1)}{6} .
        \]
  \item En déduire des expressions closes de $U_n$ et $L_n$,
        calculer leur limite commune, et conclure~: l'aire vaut
        $\frac13$.
  \item Passons à la machine~: calculer $\int_0^1 x^2\,\dd x$ par
        le théorème fondamental, en une ligne. Comparer les
        efforts.
  \item Archimède l'énonçait autrement~: \emph{un segment de
        \omterm{def:g11:quad:parabola}{parabole} vaut $\frac43$ du triangle qui y est inscrit}. Pour
        le segment découpé dans $y = x^2$ par la corde joignant
        $(-1, 1)$ à $(1, 1)$~: calculer l'aire du segment par une
        \omterm{def:g12:integ:area}{intégrale}, l'aire du triangle inscrit (de sommet le point
        $(0, 0)$), et vérifier le rapport du maître.
  \item La méthode d'Archimède lui-même~: remplir le segment par le
        grand triangle $T$, puis par deux triangles totalisant
        $\frac T4$, puis par quatre totalisant $\frac{T}{16}$, et
        ainsi de \omterm{def:g12:seq:sequence}{suite}. Sommer la série géométrique et retrouver
        $\frac43 T$ --- la tablette de chocolat indéfiniment croquée
        du volume précédent, mangée par un géomètre grec.
  \item Une phrase pour chacune~: l'exhaustion (Archimède),
        l'encadrement (Riemann), la primitivation
        (Newton--Leibniz) --- de quoi chacune a-t-elle besoin, et
        que donne chacune~?
\end{enumerate}

\medskip
\noindent\textbf{Partie III --- Le \omterm{def:g12:exp:ln}{logarithme} est une aire.}
Pour $x > 0$, on pose
$A(x) = \displaystyle\int_1^x \frac{\dd t}{t}$.
\begin{enumerate}[resume]
  \item Donner $A(1)$ et $A'(x)$ (\cref{thm:g12:integ:ftc}), et
        conclure que $A$ est exactement le \omterm{def:g12:exp:ln}{logarithme népérien} de la
        \cref{def:g12:exp:ln}.
  \item Le miracle de l'échelle~: fixons $a > 0$ et étudions
        $g(x) = A(ax) - A(x)$. Calculer $g'$ (dérivation des
        composées), en déduire que $g$ est constante, évaluer la
        constante --- et conclure l'\omterm{def:g10:algebra:equation}{équation} fonctionnelle
        \[
        \ln(ab) = \ln a + \ln b ,
        \]
        démontrée par pure analyse~: l'aire de $1$ à $ab$ se scinde
        en copies dilatées.
  \item Déduire de la question 14 que $\ln(a^n) = n\ln a$ et
        $\ln\frac1a = -\ln a$.
  \item L'\cref{exo:g12:integ:8} lit
        $\frac{1}{n+1} + \frac{1}{n+2} + \dots + \frac{1}{2n}$
        comme des rectangles sous $\frac{1}{1 + t}$~: calculer cette
        somme pour $n = 10$ (trois décimales) et la comparer à
        $\ln 2$. Quelles sommes de saveur harmonique, divergentes
        terme à terme, convergent ici vers une aire~?
\end{enumerate}

\medskip
\noindent\textbf{Partie IV --- Accumulation.}
\begin{enumerate}[resume]
  \item Une voiture accélère avec la vitesse $v(t) = 3t^2$ m/s pour
        $t \in \intcc{0}{10}$. Calculer la distance parcourue, la
        vitesse \omterm{def:g11:stat:mean}{moyenne}, et l'instant où la vitesse instantanée
        égale la vitesse \omterm{def:g11:stat:mean}{moyenne}.
  \item Une tige de $4$ mètres a pour masse linéique
        $\rho(x) = 2 + x$ kg/m. Calculer sa masse totale et son
        centre de masse
        $\dfrac{\int_0^4 x\,\rho(x)\,\dd x}{\int_0^4
        \rho(x)\,\dd x}$ --- le point d'équilibre du triangle de
        carton du volume précédent, enfin calculé avec des poids qui
        varient.
  \item Les \omterm{def:g12:integ:area}{intégrales} de Wallis (\cref{exo:g12:integ:9})~:
        calculer $W_0$, $W_1$, puis
        $W_2 = \int_0^{\pi/2} \sin^2 t\,\dd t$ à l'aide de la
        linéarisation du \cref{pb:g12:trigo:1}. (Leur échelle
        infinie mène, dans les volumes universitaires, à une formule
        de produit pour $\pi$ et à la normalisation de la courbe en
        cloche.)
  \item Pour finir --- les trois visages de l'intégration~: une
        \emph{aire} par définition, une \emph{accumulation} dans le
        monde (distance, masse), une \emph{\omterm{def:g12:integ:primitive}{primitive}} par le
        théorème fondamental~; et son histoire en trois noms.
        Terminer par la perle du chapitre~: quelle touche
        quotidienne de la calculatrice est secrètement l'aire sous
        $\frac1t$ --- et quelle identité cette aire a-t-elle
        démontrée~?
\end{enumerate}
\end{problem}
