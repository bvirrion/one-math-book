...
La fonction exponentielle est l'unique fonction égale à sa propre dérivée
et valant $1$ en $0$. Elle convertit les sommes en produits~; son inverse,
le logarithme népérien, convertit les produits en sommes. Ensemble, ils
...
Il existe une unique fonction dérivable $f \colon \R \to \R$ telle que
\[
...
D'abord, une telle fonction ne s'annule jamais. En effet, soit
$g(x) = f(x)f(-x)$~; alors
\[
...
L'\emph{existence} est admise à ce niveau (on peut l'obtenir via la suite
de l'\cref{exo:g12:seq:10}, ou comme inverse du logarithme construit par
...
Son numérateur et son dénominateur, comme fonctions de $x$, sont tous deux
...
donne la convexité. Par convexité, $\eu^x \geq 1 + x$ (tangente en $0$,
...
{\small L'exponentielle~: strictement croissante, convexe, avec
$\lim_{-\infty} \eu^x = 0$. Elle est au-dessus de sa tangente en $0$~:
$\eu^x \geq 1 + x$.}
\end{omfigure}
...
L'exponentielle est continue et strictement croissante de $\R$ sur
$\intoo{0}{+\infty}$~; par le théorème de la bijection
(\cref{thm:g12:limcont:bijection}), pour tout $y > 0$ l'équation
...
{\small Les courbes de $\exp$ et $\ln$ sont images miroir l'une de l'autre
...
La fonction $\ln$ est dérivable sur $\intoo{0}{+\infty}$ avec
\[
(\ln x)' = \frac{1}{x},
\]
strictement croissante, concave, et
...
\begin{proof}
La dérivabilité de la fonction inverse est admise à ce niveau~; en l'accordant,
...
bijection. Pour la comparaison de croissances, substituer $x = \eu^{t}$~:
...
Les deux fonctions sont strictement croissantes, donc on peut les appliquer
à (ou les retirer de) les deux membres d'une équation ou d'une inégalité
...
les ensembles de définition d'abord} ($\ln$ exige des arguments positifs).
Pour des équations du type $a^x = b$ avec $a>0$, $a \neq 1$, réécrire
$a^x = \eu^{x\ln a}$ et résoudre $x = \frac{\ln b}{\ln a}$.
\end{method}

\begin{example}\label{ex:g12:exp:halflife}
Un échantillon radioactif se désintègre suivant $N(t) = N_0\,\eu^{-\lambda t}$.
...
Étudier la fonction $f(x) = x\,\eu^{-x}$ sur $\R$~: variations, limites,
extremum~; montrer que sa courbe a un point d'inflexion et donner ses
coordonnées.
\end{exercise}
...
\begin{enumerate}
  ...
  \item Comparer avec l'approximation «~$70$ divisé par le taux en
...
\begin{enumerate}
  \item Étudier les variations de $f$ sur $\intoo{0}{+\infty}$ et donner
        son minimum.
  \item Pour quelles valeurs de $k$ l'équation $\eu^x = kx$ a-t-elle $0$,
