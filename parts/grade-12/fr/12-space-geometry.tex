...
\section{Problème~: trancher le cube}

\begin{problem}[{Devoir du week-end --- un plan coupe le cube en un
hexagone parfait, un tétraèdre régulier se cache dans les coins, et
l'angle de la diagonale façonne tout diamant}]
\label{pb:g12:space:1}
Tranchez un cube et vous attendez des carrés et des rectangles ---
et pourtant une coupe célèbre produit un \emph{hexagone régulier}, et
quatre coupes de coin bien choisies laissent derrière elles un
\emph{tétraèdre régulier}. Ces deux surprises, ainsi que l'angle de
$109.5^\circ$ que vénèrent les atomes de carbone, cèdent à la
trousse de ce chapitre~: droites paramétrées, \omterm{def:g10:algebra:equation}{équations} de plans et
\omterm{def:g12:space:dot}{produit scalaire} (\cref{thm:g12:space:planeeq},
\cref{prop:g12:space:distance}). On utilisera partout le cube de
l'\cref{exo:g12:space:8}~: côté $1$, $A(0,0,0)$, $B(1,0,0)$,
...
\begin{enumerate}
  \item Écrire l'\omterm{def:g10:algebra:equation}{équation} du plan passant par $B(1,0,0)$ et de
        \omterm{def:g12:space:normal}{vecteur normal} $(1,1,1)$, une représentation paramétrique de
        la droite passant par $A$ et dirigée par $(1,1,1)$, puis
        leur point d'\omterm{def:g10:numbers:interunion}{intersection}.
  \item Calculer la distance de $A$ à ce plan
        (\cref{prop:g12:space:distance}).
  \item Les \omterm{def:g11:vect:vector}{vecteurs} $(1,-1,0)$ et $(1,1,-2)$ sont-ils
        \omterm{def:g11:scal:orthogonal}{orthogonaux}~?
  \item Position relative des plans $x + y + z = 1$ et
        $2x + 2y + 2z = 5$~? Et de la droite passant par $A$ et
        dirigée par $(1,1,0)$ par rapport au premier plan~?
  \item Énumérer les milieux des six arêtes
        $[BC], [CD], [DH], [HE], [EF], [FB]$ du cube, et vérifier
        que tous les six vérifient l'\omterm{def:g10:algebra:equation}{équation}
        $x + y + z = \frac32$.
\end{enumerate}

\medskip
\noindent\textbf{Partie II --- La section hexagonale.}
Soit $P$ le plan d'\omterm{def:g10:algebra:equation}{équation} $x + y + z = \frac32$.
\begin{enumerate}[resume]
  \item Montrer que $P$ est perpendiculaire à la grande diagonale
        $(AG)$ et passe par le centre du cube.
  \item Calculer les distances entre milieux consécutifs de la
        question 5 le long de la section~: que trouve-t-on~?
  \item Calculer l'angle du polygone de section en l'un de ses
        sommets (deux \omterm{def:g11:vect:vector}{vecteurs} d'arêtes consécutives et un \omterm{def:g12:space:dot}{produit
        scalaire}). Conclure~: la section est un \emph{hexagone
        régulier}.
  \item Calculer le périmètre et l'aire de l'hexagone, et comparer
        cette aire à celle d'une face du cube. (Pour mémoire~: la
        plus grande section plane du cube unité est le rectangle
        diagonal $1 \times \sqrt2$, d'aire $\sqrt2$ --- l'hexagone
        régulier décroche l'argent.)
  \item En quel point la diagonale $(AG)$ perce-t-elle $P$~?
        Vérifier que ce point de percée est le \omterm{prop:g10:coordgeom:midpoint}{milieu} de $[AG]$.
  \item Faire glisser la coupe~: décrire les sections
        $x + y + z = c$ lorsque $c$ croît de $0$ à $3$ --- calculer
        la section pour $c = \frac12$ (quel polygone, de quelle
        taille~?), et raconter la métamorphose qui passe par votre
        hexagone en $c = \frac32$.
  \item Pourquoi aucun plan ne peut-il découper dans un cube un
        polygone à \emph{sept} côtés~? (Où doit se trouver chaque
        côté de la section~?)
\end{enumerate}

\medskip
\noindent\textbf{Partie III --- Le tétraèdre caché.}
\begin{enumerate}[resume]
  \item Calculer le volume du tétraèdre de coin $ABDE$ (base,
        hauteur, et la formule du tiers admise dans le volume
        précédent --- l'\cref{exo:g12:space:9} calcule de tels
        volumes en général).
  \item Les quatre sommets $B$, $D$, $E$, $G$~: calculer les six
        distances deux à deux et en conclure que $BDEG$ est un
        tétraèdre \emph{régulier}. Puis obtenir son volume en
        retranchant du cube les tétraèdres de coin.
  \item Calculer la distance du centre du cube au plan $(BDE)$.
  \item L'angle du diamant~: calculer l'angle entre les diagonales
        $\vect{AG}$ et $\vect{BH}$ du cube. Son supplémentaire,
        environ $109.5^\circ$, est l'angle entre les liaisons dans
        le diamant et le méthane --- pourquoi quatre doublets
        électroniques autour d'un atome de carbone choisissent-ils
        les sommets du tétraèdre de la question 14~?
\end{enumerate}

\medskip
\noindent\textbf{Partie IV --- Authentiquement tridimensionnel.}
\begin{enumerate}[resume]
  \item Déterminer en quel point la droite passant par $B(1,0,0)$
        et dirigée par $(0,1,1)$ perce le plan $P$ de l'hexagone.
  \item Montrer que les droites $(AB)$ et $(EG)$ ne se coupent pas
        et ne sont pas parallèles~: elles sont \emph{non
        \omterm{def:g12:space:collinear}{coplanaires}}, phénomène impossible dans un plan.
  \item Calculer le projeté \omterm{def:g11:scal:orthogonal}{orthogonal} de $B$ sur $P$ (cheminer
        depuis $B$ le long de la normale jusqu'au plan), et vérifier
        que le point obtenu satisfait l'\omterm{def:g10:algebra:equation}{équation} de $P$.
  \item Pour finir --- la trousse de l'arpenteur de l'espace~: les
        droites paramétrées pour percer, les \omterm{def:g12:space:normal}{vecteurs normaux} pour
        les angles et les distances, les \omterm{def:g10:algebra:equation}{équations} de plans pour les
        sections~; et le cube comme terrain de jeu où elles ont
        produit un hexagone, un tétraèdre régulier, l'angle du
        diamant et deux droites non \omterm{def:g12:space:collinear}{coplanaires}. Une phrase pour
        chacune.
\end{enumerate}
\end{problem}
