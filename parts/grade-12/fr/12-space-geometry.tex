...
La géométrie en dimension trois devient calculatoire une fois les vecteurs
et les coordonnées disponibles~: les droites admettent des représentations
paramétriques, les plans des équations cartésiennes, et le produit scalaire
mesure les angles et les distances. Ce chapitre construit cette boîte à
outils et l'utilise pour résoudre les problèmes classiques d'intersection
et de distance.

\section{Vecteurs de l'espace}

Les vecteurs de l'espace obéissent aux mêmes règles que dans le plan~: ils
s'additionnent, se multiplient par un scalaire, et $\vect{AB} = \vect{CD}$
exactement lorsque $ABDC$ est un parallélogramme. Dans un repère
$(O; \vec\imath, \vec\jmath, \vec k)$, tout vecteur a des coordonnées
$(x, y, z)$.

\begin{definition}[Colinéarité, coplanarité]\label{def:g12:space:collinear}
Deux vecteurs sont \emph{colinéaires} si l'un est un multiple de l'autre.
Trois vecteurs $\vec u, \vec v, \vec w$ sont \emph{coplanaires}\index{coplanaire}
si l'un d'eux peut s'écrire comme combinaison des deux autres, disons
$\vec w = a\vec u + b\vec v$.
\end{definition}

\begin{proposition}\label{prop:g12:space:basis}
Si $\vec u, \vec v, \vec w$ ne sont pas coplanaires, tout vecteur de
l'espace se décompose de façon unique en $x\vec u + y\vec v + z\vec w$.
\end{proposition}

\begin{proof}[Idée de la preuve]
Par l'extrémité du vecteur à décomposer, mener la droite parallèle à
$\vec w$~; elle rencontre le plan de $\vec u, \vec v$ en un unique point,
qui scinde le vecteur en une composante dans ce plan (uniquement
...
vecteurs en fonction des deux autres, contredisant la non-coplanarité.
\end{proof}

\section{Droites et plans}

\begin{definition}[Représentation paramétrique d'une droite]\label{def:g12:space:line}
La droite passant par $A(x_A, y_A, z_A)$ de vecteur directeur
...
Le plan passant par $A$ dirigé par deux vecteurs non colinéaires
...
peuvent être sécantes, strictement parallèles, confondues, ou
\emph{non coplanaires} (gauches).
\end{proposition}

\begin{proof}[Esquisse]
Ce sont des énoncés d'incidence~; chaque analyse de cas se réduit, en
coordonnées, à résoudre un système linéaire et à compter ses solutions ---
...
- \norm{\vec u}^2 - \norm{\vec v}^2\right),
\]
où $\norm{\vec u}$ est la longueur de $\vec u$. Si les deux vecteurs sont
non nuls, $\vec u \cdot \vec v = \norm{\vec u}\,\norm{\vec v}\cos\theta$
où $\theta$ est l'angle entre eux~; et dans un repère \emph{orthonormé},
\[
\vec u \cdot \vec v = xx' + yy' + zz' .
\]
Deux vecteurs sont \emph{orthogonaux} si leur produit scalaire vaut $0$.
\end{definition}
...
\begin{proof}
Dans un repère orthonormé, les trois propriétés sont immédiates sur la
...
vecteur non nul orthogonal à tout vecteur situé dans $\mathcal P$ (il
suffit qu'il soit orthogonal à deux directions non colinéaires de
$\mathcal P$).
\end{definition}
...
{\small Un vecteur normal $\vec n$ du plan $\mathcal P$ est orthogonal à
...
Dans un repère orthonormé, le plan passant par $A$ de vecteur normal
$\vec n(a, b, c)$ a pour équation
\[
a x + b y + c z + d = 0,
\]
où $d = -(ax_A + by_A + cz_A)$. Réciproquement, toute telle équation avec
$(a,b,c) \neq (0,0,0)$ définit un plan de vecteur normal $(a, b, c)$.
\end{theorem}

\begin{proof}
$M \in \mathcal P \iff \vect{AM} \perp \vec n \iff
a(x - x_A) + b(y - y_A) + c(z - z_A) = 0$, qui se développe en l'équation.
Réciproquement, prendre un point solution $A$ quelconque de l'équation~; le
...
\begin{itemize}
  \item \emph{Droite $\cap$ plan}~: substituer les coordonnées paramétriques
        de la droite dans l'équation du plan~; résoudre en $t$ (une
        solution~: un point~; $0 = $ non nul~: parallèle~; $0 = 0$~: droite
        contenue).
  \item \emph{Plan $\cap$ plan}~: résoudre le système des deux équations,
        en paramétrant par une coordonnée libre~; la solution est une droite
        (ou les plans sont parallèles lorsque les vecteurs normaux sont
        colinéaires).
  \item \emph{Contrôles d'orthogonalité}~: droite $\perp$ plan ssi sa
        direction est colinéaire à la normale~; deux plans sont
        perpendiculaires ssi leurs normales sont orthogonales.
\end{itemize}
\end{method}

\begin{proposition}[Distance d'un point à un plan]\label{prop:g12:space:distance}
\index{distance!point à un plan}
Dans un repère orthonormé, la distance de $M_0(x_0, y_0, z_0)$ au plan
...
\begin{proof}
Soit $H$ la projection orthogonale de $M_0$ sur $\mathcal P$~: le pied de
la perpendiculaire, donc $\vect{HM_0}$ est colinéaire à la normale unitaire
$\frac{\vec n}{\norm{\vec n}}$, et la distance est
$\abs{\vect{HM_0}\cdot \frac{\vec n}{\norm{\vec n}}}$. Pour tout
$H \in \mathcal P$,
$\vect{HM_0}\cdot\vec n = ax_0 + by_0 + cz_0 - (ax_H + by_H + cz_H)
= ax_0 + by_0 + cz_0 + d$ (en utilisant l'équation en $H$), d'où la formule
...
Déterminer une équation cartésienne du plan passant par $A(1, 1, 0)$ de
vecteur normal $\vec n(2, -1, 3)$. Le point $B(0, 2, 1)$ lui
appartient-il~?
\end{exercise}
...
\begin{enumerate}
  ...
  \item Déterminer les coordonnées de la projection orthogonale $H$ de
        $M_0$ sur $\mathcal P$, et vérifier la distance $M_0H$.
\end{enumerate}
\end{exercise}

\begin{exercise}[$\star\star\star$]\label{exo:g12:space:8}
Le cube $ABCDEFGH$ a pour arête $1$~; placer les coordonnées de sorte que
$A(0,0,0)$, $B(1,0,0)$, $D(0,1,0)$, $E(0,0,1)$ (avec
$C = B + D$, $F = B + E$, $G = B + D + E$, $H = D + E$ comme vecteurs issus
de $A$).
\begin{enumerate}
  \item Montrer que la diagonale $(AG)$ est orthogonale au plan $(BDE)$.
...
\begin{enumerate}
  \item Vérifier que $\vect{AB}$, $\vect{AC}$, $\vect{AD}$ ne sont pas
        coplanaires (les points forment un vrai tétraèdre).
  \item Trouver un vecteur $\vec n$ orthogonal à la fois à $\vect{BC}$ et
        $\vect{BD}$, et en déduire une équation cartésienne du plan
        $(BCD)$.
