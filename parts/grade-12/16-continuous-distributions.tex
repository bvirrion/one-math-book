...
\section{Problem: Waiting for the bus, measuring the crowd}

\begin{problem}[{Weekend problem --- three densities run the
world: total ignorance, memoryless waiting, and the bell of many
small causes; plus the paradox that makes your bus always late}]
\label{pb:g12:contdist:1}
Why does your bus always seem to take longer than the timetable
promises? Why is the class an average student sits in bigger
than the average class? Why do your friends have more friends
than you? One sly piece of mathematics --- length-biased
sampling --- answers all three, and it lives in this chapter,
between the uniform, exponential and \omterm{def:g12:contdist:normal}{normal distributions}
(\cref{def:g12:contdist:uniform}, \cref{def:g12:contdist:expo},
\cref{def:g12:contdist:normal}). This final problem of the
volume puts the three densities to work and ends where the whole
series points: the bell curve.

\medskip
\noindent\textbf{Part I --- Three characters.}
\begin{enumerate}
  \item A bus arrives at a uniformly random time in the next
        $30$ minutes. Compute $\P(\text{wait} \leq 10)$, the
        expected wait, and $\sigma$
        (\cref{prop:g12:contdist:uniformmoments}).
  \item Check that $f(x) = 2x$ on $\intcc{0}{1}$ is a \omterm{def:g12:contdist:density}{density},
        and compute $\E(X)$ and $\P\left(X > \frac12\right)$.
  \item Phone calls reach a helpline as a memoryless stream:
        the wait for the next call is exponential with \omterm{def:g11:stat:mean}{mean}
        $10$ minutes. Compute $\P(T > 15)$ and the \omterm{def:g11:stat:median}{median} wait
        --- why is the \omterm{def:g11:stat:median}{median} \emph{less} than the \omterm{def:g11:stat:mean}{mean}?
  \item You have already waited $20$ minutes. What is
        $\P(T > 35 \mid T > 20)$
        (\cref{thm:g12:contdist:memoryless})? Interpret in one
        sentence.
  \item Match each wait to its right model --- uniform,
        exponential, or neither: (a) the click of a Geiger
        counter; (b) a metro running every $8$ minutes, your
        arrival unsynchronized; (c) the next call at the
        helpline; (d) the failure of a light bulb that
        \emph{wears out}. Justify (d) with the memorylessness
        theorem.
\end{enumerate}

\medskip
\noindent\textbf{Part II --- The bell at work.}
(Use the normal landmarks: $68\,\%$ within $\sigma$, $95\,\%$
within $2\sigma$, $99.7\,\%$ within $3\sigma$.)
\begin{enumerate}[resume]
  \item Adult heights follow roughly
        $\mathcal N(172,\ 8)$ (cm). What fractions lie in
        $\intcc{164}{180}$, $\intcc{156}{188}$,
        $\intcc{148}{196}$?
  \item Estimate the proportion taller than $190$ cm (\omterm{pb:g11:stat:1}{z-score}
        first; table or calculator).
  \item IQ scores are calibrated to $\mathcal N(100, 15)$.
        What fraction scores above $130$ --- and roughly one
        person in how many?
  \item A machine cuts bolts with lengths
        $\mathcal N(50,\ 0.1)$ mm and the specification is
        $\intcc{49.8}{50.2}$. What share is rejected? Industry
        celebrates ``six sigma'' processes, whose
        specifications sit at $\pm 6\sigma$: what does that
        buy, and why do factories chase it?
  \item De Moivre--Laplace (\cref{thm:g12:contdist:tcl}): for
        $100$ fair coin flips, approximate
        $\P(45 \leq \text{heads} \leq 55)$ (\omterm{def:g12:limcont:continuity}{continuity}:
        $\pm 0.5$; $\sigma = 5$). Which wooden machine of
        \cref{pb:g11:binom:1} does this theorem smooth into a
        curve?
  \item From \cref{exo:g12:contdist:8}: a $\pm 2$-point margin
        needs about $n = 2\,400$. Compute the \omterm{def:g10:proba:sample}{sample} needed for
        $\pm 0.5$ points --- and conclude, in one sentence, why
        a race separated by one point cannot be honestly called
        by any poll.
\end{enumerate}
...
\begin{enumerate}[resume]
  \item Buses arrive as a memoryless stream with \omterm{def:g11:stat:mean}{mean} gap $10$
        minutes. You reach the stop at an arbitrary moment.
        Intuition says the expected wait is $5$ minutes
        (half a gap). What does the memorylessness theorem say
        instead?
  \item Diagnose the intuition with a toy timetable: gaps
        alternate $5$ and $15$ minutes (\omterm{def:g11:stat:mean}{mean} gap $10$).
        Compute the \omterm{def:g10:proba:distribution}{probability} that a uniformly random arrival
        lands in a long gap, then the true expected wait ---
        and name the culprit: arrivals \omterm{def:g10:proba:sample}{sample} gaps with
        \omterm{def:g10:proba:distribution}{probability} proportional to their \emph{length}.
  \item In the memoryless stream, the gap you land in has
        expected length $20$ minutes --- twice the typical gap
        (the time back to the previous bus and forward to the
        next are \emph{both} exponential with \omterm{def:g11:stat:mean}{mean} $10$).
        Reconcile this with question 12 and state the
        \emph{inspection paradox} in one sentence.
  \item Same sting on land: a university runs nine classes of
        $10$ students and one of $110$. Compute the average
        class size, then the class size experienced by the
        \emph{average student}. Which number will the brochure
        quote, and which is the students' lived truth?
  \item The friendship paradox --- ``your friends have, on
        average, more friends than you'' --- is the same
        mathematics. Say in one sentence what plays the role of
        the long gap.
\end{enumerate}
...
\begin{enumerate}[resume]
  \item Simulation, the practitioners' bridge: if $U$ is
        uniform on $\intoo{0}{1}$, show that
        $T = -10\ln U$ is exponential with \omterm{def:g11:stat:mean}{mean} $10$ (compute
        $\P(T > t)$). Every random-waiting simulation in
        industry runs on this one-line trick.
  \item An old game programmers' recipe builds a bell: add
        twelve \omterm{def:g12:sums:indep}{independent} uniforms on $\intcc{0}{1}$ and
        subtract $6$. Give the \omterm{def:g11:stat:mean}{mean} and \omterm{def:g12:randvar:exp}{variance} of the sum,
        and explain --- Galton board in hand --- why the
        result is nearly normal.
  \item Markets crash harder than the bell allows: the 1987
        crash was described as a ``$25\sigma$ \omterm{def:g11:prob:model}{event}'', which
        under normality has \omterm{def:g10:proba:distribution}{probability} around $10^{-137}$.
        What is the correct conclusion --- about the world, or
        about the model? (One sentence, the statistician's
        catechism.)
  \item Finale, and farewell to the volume: the three
        characters in one line each --- uniform (ignorance
        within bounds), exponential (hazard without memory),
        normal (the sum of many small causes); the inspection
        paradox as the chapter's sting; and the arc of the
        whole series --- from counting cartons in grade 6 to
        the bell curve that measures every crowd --- with the
        university volumes waiting where the limits, the
        \omterm{def:g12:integ:area}{integrals} and the central limit theorem take over.
\end{enumerate}
\end{problem}
