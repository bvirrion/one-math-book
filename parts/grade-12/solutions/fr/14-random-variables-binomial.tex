...
\emph{1.} Les $10$ questions sont des épreuves de Bernoulli indépendantes
...
Essais identiques indépendants~: $X \sim \mathcal B(400,\ 0.05)$, donc
...
\frac{25}{6}$ avec probabilités $\frac46, \frac16, \frac16$~:
\[
\V(G) = \E(G^2)
...
ssi $k \leq (n+1)p - 1$. Donc les probabilités croissent strictement tant
que $k + 1 \leq (n+1)p$ et décroissent ensuite~: le maximum est atteint en
...
probabilité $\left(\frac12\right)^{k-1}\cdot\frac12 = 2^{-k}$, pour
$1 \leq k \leq 10$. Probabilité totale de gagner
...
célèbre \emph{paradoxe de Saint-Pétersbourg}, montrant que l'espérance
