...
even cleaner than the real one --- \omterm{def:b2:hermitian:adjoint}{Hermitian endomorphisms} have real
\omterm{def:b2:reduction:eigen}{eigenvalues}, \omterm{def:b2:hermitian:adjoint}{unitary} ones have unimodular \omterm{def:b2:reduction:eigen}{eigenvalues}, and both
...
\]
on \omterm{def:b2:metric:continuity}{continuous} functions, $\langle f, g\rangle = \int_a^b \conj
...
$x, y$ are linearly dependent; $\norm\cdot$ is a \omterm{def:b2:nvs:norm}{norm}. Moreover
...
and separation are immediate, so $\norm\cdot$ is a \omterm{def:b2:nvs:norm}{norm}.
...
\]
Its \omterm{def:b2:nvs:norm}{norm} is $\sqrt{\frac14 + \frac14} = \frac{1}{\sqrt2}$:
...
A^{\dagger}$ (conjugate \omterm{def:b2:linalg:transpose}{transpose}) --- indeed, if $B =
...
\omterm{def:b2:reduction:eigen}{Eigenvalues} of a \omterm{def:b2:hermitian:adjoint}{Hermitian endomorphism} are \emph{real};
\omterm{def:b2:reduction:eigen}{eigenvalues} of a \omterm{def:b2:hermitian:adjoint}{unitary} endomorphism have \emph{modulus $1$}; in
both cases \omterm{def:b2:reduction:eigen}{eigenspaces} for distinct \omterm{def:b2:reduction:eigen}{eigenvalues} are orthogonal.
\end{proposition}

\begin{proof}
\omterm{def:b2:hermitian:adjoint}{Hermitian}, $u(x) = \lambda x$, $x \neq 0$:
\[
\lambda \norm x^2 = \langle x, u(x)\rangle = \langle u(x), x\rangle
= \conj{\langle x, u(x)\rangle} = \conj\lambda\,\norm x^2 ,
\]
so $\lambda \in \R$. \omterm{def:b2:hermitian:adjoint}{Unitary}: $\norm{u(x)} = \norm x$ (from $u^*u =
\mathrm{id}$), so $\abs\lambda\norm x = \norm x$. Orthogonality
(\omterm{def:b2:hermitian:adjoint}{Hermitian} case): $\lambda\langle x, y\rangle = \langle u(x),
y\rangle = \langle x, u(y)\rangle = \mu\langle x, y\rangle$ for
\omterm{def:b2:reduction:eigen}{eigenvectors} with real $\lambda \neq \mu$. \omterm{def:b2:hermitian:adjoint}{Unitary} case, in
...
$A^\dagger = A^{\mathsf T} = -A$: \omterm{def:b2:hermitian:adjoint}{skew-Hermitian} (also real
antisymmetric --- over $\R$ it has no \omterm{def:b2:reduction:eigen}{eigenvalues} at all).
\omterm{def:b2:reduction:charpoly}{Characteristic polynomial} $X^2 + 4$: \omterm{def:b2:reduction:eigen}{eigenvalues} $\pm2\iu$,
purely imaginary, as \cref{exo:b2:hermitian:9} predicts in
general. \omterm{def:b2:reduction:eigen}{Eigenvectors}: $(A - 2\iu I)v = 0$ gives $v_1 =
...
(v_1\ v_2)$ \omterm{def:b2:hermitian:adjoint}{unitary}. Closing insight: $H = -\iu A =
\begin{pmatrix} 0 & 2\iu\\ -2\iu & 0\end{pmatrix}$ is \omterm{def:b2:hermitian:adjoint}{Hermitian}
with the real \omterm{def:b2:reduction:eigen}{spectrum} $\{\pm2\}$ and the \emph{same}
\omterm{def:b2:reduction:eigen}{eigenvectors} --- the bijection $u \mapsto \iu u$ between
\omterm{def:b2:hermitian:adjoint}{Hermitian} and \omterm{def:b2:hermitian:adjoint}{skew-Hermitian} endomorphisms
(\cref{exo:b2:hermitian:9}), seen matrix by matrix; over $\R$
the same $A$ is a rotation-scaling with no \omterm{def:b2:reduction:eigen}{eigenvectors} at
...
Every \omterm{def:b2:hermitian:adjoint}{Hermitian endomorphism} of a \omterm{def:b2:hermitian:def}{Hermitian space} has an orthonormal
basis of \omterm{def:b2:reduction:eigen}{eigenvectors} (with real \omterm{def:b2:reduction:eigen}{eigenvalues}): $A^\dagger = A$
implies $A = U D U^{\dagger}$ with $U \in U(n)$ and $D$ real
diagonal.\index{spectral theorem!Hermitian}
\end{theorem}

\begin{proof}
Over $\C$, the \omterm{def:b2:reduction:charpoly}{characteristic polynomial} splits: an \omterm{def:b2:reduction:eigen}{eigenvector}
$e_1$ exists (\cref{ch:b2:reduction}) --- no \omterm{def:b2:metric:compact}{compactness} argument
...
\]
($\lambda_1$ real). The restriction is \omterm{def:b2:hermitian:adjoint}{Hermitian}; induct on the
dimension and concatenate. In detail: the restriction $u|_F$ is
an endomorphism of the \omterm{def:b2:hermitian:def}{Hermitian space} $F$ (dimension $n - 1$)
with $\langle u|_F(x), y\rangle = \langle x, u|_F(y)\rangle$
inherited from $u$; the induction hypothesis provides an
orthonormal basis $(e_2, \dots, e_n)$ of $F$ of \omterm{def:b2:reduction:eigen}{eigenvectors},
and $(e_1, e_2, \dots, e_n)$ is orthonormal in $E$ ($e_1 \perp
F$) and made of \omterm{def:b2:reduction:eigen}{eigenvectors} of $u$. Matrix translation: the
columns of $U$ are the $e_i$, $U^\dagger U = I$ expresses their
orthonormality, and $AU = UD$ collects the \omterm{def:b2:reduction:eigen}{eigenvalue}
...
$A = \begin{pmatrix} 0 & -\iu\\ \iu & 0\end{pmatrix}$ is \omterm{def:b2:hermitian:adjoint}{Hermitian}
($A^\dagger = A$): \omterm{def:b2:reduction:eigen}{eigenvalues} from $\chi_A = X^2 - 1$: $\pm 1$
(real, as promised), with orthonormal \omterm{def:b2:reduction:eigen}{eigenvectors}
$\frac{1}{\sqrt2}(1, \iu)^{\mathsf T}$ and $\frac{1}{\sqrt2}(1,
-\iu)^{\mathsf T}$. (Physicists know $A$ as a Pauli matrix; the
reality of \omterm{def:b2:hermitian:adjoint}{Hermitian} spectra is why quantum observables are modeled
by \omterm{def:b2:hermitian:adjoint}{Hermitian} operators.)
\end{example}
...
\omterm{def:b2:hermitian:adjoint}{Hermitian}, since the diagonal is real and the off-diagonal
entries are conjugates. \omterm{def:b2:reduction:charpoly}{Characteristic polynomial}:
\[
(2-\lambda)(3-\lambda) - \abs{1-\iu}^2
= \lambda^2 - 5\lambda + 4
= (\lambda - 1)(\lambda - 4) :
\]
\omterm{def:b2:reduction:eigen}{spectrum} $\{1, 4\}$, real and positive --- $A$ is positive
definite. \omterm{def:b2:reduction:eigen}{Eigenvectors}: for $\lambda = 4$, the system $(A -
...
\]
Normalizing ($\norm{v_4}^2 = 2 + 4 = 6$, $\norm{v_1}^2 = 2 + 1
= 3$) gives the \omterm{def:b2:hermitian:adjoint}{unitary} $U = \bigl(\frac{v_4}{\sqrt6}\
...
attained at the two \omterm{def:b2:reduction:eigen}{eigenvectors}; this is the $n = 2$ germ of
the Courant--Fischer theory built in the weekend problem.
Spot-check that the form is real off the \omterm{def:b2:reduction:eigen}{eigenvectors} too: at
$x = (1, \iu)$,
\[
...
\end{pmatrix}$ (\omterm{def:b2:hermitian:adjoint}{unitary} by \cref{exo:b2:hermitian:2}). Its
\omterm{def:b2:reduction:charpoly}{characteristic polynomial} is $\bigl(X - \frac{1}{\sqrt2}\bigr)^2
+ \frac12$, with roots
\[
\lambda_\pm = \frac{1 \pm \iu}{\sqrt2} = \eu^{\pm\iu\pi/4},
\]
unimodular as \cref{prop:b2:hermitian:eigenvalues} promised, and
orthonormal \omterm{def:b2:reduction:eigen}{eigenvectors} $\frac{1}{\sqrt2}(1, \pm1)$. So $U =
...
no real \omterm{def:b2:reduction:eigen}{eigenvectors}, but over $\C$ it splits into two
unimodular scalars. Closing insight: \omterm{def:b2:hermitian:adjoint}{Hermitian} spectra live on
the real line, \omterm{def:b2:hermitian:adjoint}{unitary} spectra on the unit circle; both are
...
1 & 0\end{pmatrix}$ (\omterm{def:b2:hermitian:adjoint}{Hermitian}, \omterm{def:b2:reduction:eigen}{spectrum} $\{1, -1\}$,
orthonormal \omterm{def:b2:reduction:eigen}{eigenvectors} $\frac{1}{\sqrt2}(1, \pm1)$). In the
...
\iu I)(H + \iu I)^{-1}$ is the \omterm{def:b2:hermitian:adjoint}{unitary} with \omterm{def:b2:reduction:eigen}{eigenvalues}
$\mp\iu$ on those same \omterm{def:b2:reduction:eigen}{eigenvectors}:
\[
...
\omterm{def:b2:reduction:eigen}{eigenvalue} by \omterm{def:b2:reduction:eigen}{eigenvalue}, the Cayley transform is the
...
wrong silently. \emph{(ii) Complex \omterm{def:b2:quadratic:def}{polarization} is stronger:}
...
already forces $u$ \omterm{def:b2:hermitian:adjoint}{Hermitian}. \emph{(iii) Real normal is not
\omterm{def:b2:reduction:diag}{diagonalizable}:} the matrix of
\cref{ex:b2:hermitian:skewexample} is normal but has no real
\omterm{def:b2:reduction:eigen}{eigenvalue}; \omterm{def:b2:hermitian:adjoint}{unitary} diagonalization is a theorem \emph{over}
$\C$, and over $\R$ one only gets block reductions.
\emph{(iv) Checking unitarity:} $U^\dagger U = I$ means the
\emph{columns} are orthonormal for the \omterm{def:b2:hermitian:adjoint}{Hermitian} product ---
...
\omterm{def:b2:nvs:norm}{Norm} preservation looks weaker than unitarity, but over $\C$ it
is not: if $\norm{u(x)} = \norm x$ for all $x$, then $u^*u =
\mathrm{id}$. Indeed $v = u^*u - \mathrm{id}$ is \omterm{def:b2:hermitian:adjoint}{Hermitian} and
satisfies $\langle x, v(x)\rangle = \norm{u(x)}^2 - \norm x^2 =
0$ for every $x$; by complex \omterm{def:b2:quadratic:def}{polarization} (pitfall (ii)
above), a map with identically vanishing ``diagonal'' is zero:
$v = 0$. Concretely, the \omterm{def:b2:quadratic:def}{polarization} runs
\[
...
for all $x, y$. Closing insight: this is why ``\omterm{def:b2:hermitian:adjoint}{unitary}'' can
...
The \omterm{def:b2:hermitian:adjoint}{Hermitian} machinery built here is consumed almost
immediately. The Fourier chapter \emph{is} \omterm{def:b2:hermitian:adjoint}{Hermitian} geometry
...
\omterm{def:b2:reduction:eigen}{eigenvalue} stability that the differential-equations chapter
...
A^\dagger$, orthogonal $\leftrightarrow$ \omterm{def:b2:hermitian:adjoint}{unitary}, Rayleigh
real in both).
\end{remark}
...
covering \omterm{def:b2:hermitian:adjoint}{Hermitian}, \omterm{def:b2:hermitian:adjoint}{unitary} and \omterm{def:b2:hermitian:adjoint}{skew-Hermitian} maps at once. The
...
\omterm{def:b2:hermitian:adjoint}{Hermitian} spectral theory is the mathematics of quantum
mechanics: observables are modeled by \omterm{def:b2:hermitian:adjoint}{Hermitian} operators
(real spectra = measurable values), time evolution by \omterm{def:b2:hermitian:adjoint}{unitary}
ones (\omterm{def:b2:nvs:norm}{norm} preservation = conservation of probability). Within
this book, the Fourier chapter rests on the orthonormality of
the exponentials --- a \omterm{def:b2:hermitian:adjoint}{Hermitian} inner-product statement --- and
...
\omterm{def:b2:reduction:eigen}{eigenvalues} (Courant--Fischer, Weyl, interlacing), the daily
bread of numerical analysis and mathematical physics; the Year
3 volume extends it to \omterm{def:b2:metric:compact}{compact} \omterm{def:b2:hermitian:adjoint}{self-adjoint} operators on
Hilbert spaces.
\end{remark}
...
Which are \omterm{def:b2:hermitian:adjoint}{Hermitian}? \omterm{def:b2:hermitian:adjoint}{unitary}? normal?
\[
...
H + \iu K$ with $H, K$ \omterm{def:b2:hermitian:adjoint}{Hermitian} \emph{(the ``real and imaginary
...
Prove that $U(n)$ is \omterm{def:b2:metric:compact}{compact}, and that the \omterm{def:b2:reduction:eigen}{eigenvalue} map is
onto: every unimodular $\lambda$ arises for some \omterm{def:b2:hermitian:adjoint}{unitary} matrix.
Prove that $\det U \in \mathbb{U}$ (the unit circle) for $U \in
U(n)$.
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:b2:hermitian:6}
(Cayley transform) Let $H$ be \omterm{def:b2:hermitian:adjoint}{Hermitian}. Prove that $H + \iu I$ is
invertible and that $U = (H - \iu I)(H + \iu I)^{-1}$ is \omterm{def:b2:hermitian:adjoint}{unitary},
...
For $A$ \omterm{def:b2:hermitian:adjoint}{Hermitian} positive definite ($\langle x, Ax\rangle > 0$ for
$x \neq 0$), prove that $\operatorname{Sp}(A) \subseteq
\intoo{0}{\infty}$, that $A = B^2$ for a \omterm{def:b2:hermitian:adjoint}{Hermitian} positive
...
\omterm{def:b2:hermitian:def}{Hermitian space}.
\begin{enumerate}
  \item Prove $\norm{u(x)} = \norm{u^*(x)}$ for all $x$, and deduce
        $\ker(u - \lambda) = \ker(u^* - \conj\lambda)$.
  \item Prove that \omterm{def:b2:reduction:eigen}{eigenspaces} of $u$ for distinct \omterm{def:b2:reduction:eigen}{eigenvalues} are
        orthogonal, and that the orthogonal complement of an
        \omterm{def:b2:reduction:eigen}{eigenspace} is $u$-stable.
  \item Conclude by induction that $u$ is unitarily \omterm{def:b2:reduction:diag}{diagonalizable};
        and conversely.
\end{enumerate}
\end{exercise}

\begin{exercise}[$\star$]\label{exo:b2:hermitian:9}
An endomorphism is \emph{\omterm{def:b2:hermitian:adjoint}{skew-Hermitian}} when $u^* = -u$. Prove
that its \omterm{def:b2:reduction:eigen}{eigenvalues} are purely imaginary, that $u \mapsto \iu
u$ is a bijection from \omterm{def:b2:hermitian:adjoint}{Hermitian} to \omterm{def:b2:hermitian:adjoint}{skew-Hermitian}
endomorphisms, and that \omterm{def:b2:hermitian:adjoint}{skew-Hermitian} endomorphisms are
unitarily \omterm{def:b2:reduction:diag}{diagonalizable} (\cref{exo:b2:hermitian:8}).
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:b2:hermitian:10}
(The finite Fourier transform) Let $S$ be the \omterm{def:b2:structures:generated}{cyclic} shift of
$\C^n$: $S(x_0, x_1, \dots, x_{n-1}) = (x_{n-1}, x_0, \dots,
x_{n-2})$, and $\omega = \eu^{2\iu\pi/n}$.
\begin{enumerate}
  \item Show that $S$ is \omterm{def:b2:hermitian:adjoint}{unitary}, and that the vectors $f_k =
        \frac{1}{\sqrt n}\bigl(1, \omega^k, \omega^{2k}, \dots,
        \omega^{(n-1)k}\bigr)$, $0 \leq k < n$, form an
        orthonormal basis of \omterm{def:b2:reduction:eigen}{eigenvectors}: $Sf_k = \omega^{-k}
        f_k$.
  \item Deduce that every \emph{circulant} matrix $C =
        \sum_{j=0}^{n-1} c_jS^j$ is normal, diagonalized by the
        same basis, with \omterm{def:b2:reduction:eigen}{eigenvalues} $\widehat c(k) = \sum_j
        c_j\,\omega^{-jk}$.
\end{enumerate}
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:b2:hermitian:11}
Let $P$ be an idempotent ($P^2 = P$) endomorphism of a \omterm{def:b2:hermitian:def}{Hermitian
...
(Spectral projectors by interpolation) Let $A$ be \omterm{def:b2:hermitian:adjoint}{Hermitian} with
\emph{distinct} \omterm{def:b2:reduction:eigen}{eigenvalues} $\lambda_1, \dots, \lambda_p$ and
\omterm{def:b2:reduction:eigen}{eigenspace} decomposition $E = \bigoplus_i E_i$. Define the
...
The \omterm{def:b2:reduction:eigen}{eigenvalues} of a \omterm{def:b2:hermitian:adjoint}{Hermitian} matrix are not just roots of a
polynomial: they are solutions of \emph{optimization problems}.
That variational point of view --- \omterm{pb:b2:hermitian:1}{Rayleigh quotients} and the
\emph{Courant--Fischer min-max theorem}\index{Courant--Fischer
theorem} --- makes \omterm{def:b2:reduction:eigen}{eigenvalues} comparable, stable, and
...
and the \omterm{def:b2:reduction:eigen}{spectrum} of the discrete Laplacian. Throughout, $A, B,
E$ are \omterm{def:b2:hermitian:adjoint}{Hermitian} on $E = \C^n$ with \omterm{def:b2:reduction:eigen}{eigenvalues} listed in
...
\medskip
\noindent\textbf{Part I --- \omterm{pb:b2:hermitian:1}{Rayleigh quotients} and min-max.}
...
\begin{enumerate}
  ...
  \item Show that the critical points of $R_A$ are exactly the
        \omterm{def:b2:reduction:eigen}{eigenvectors} of $A$ \emph{(expand $t \mapsto R_A(x +
...
\begin{enumerate}[resume]
  ...
  \item Define $\vertiii{E}_2 = \max_{\norm x = 1}\norm{Ex}$
        ...
        \omterm{def:b2:hermitian:adjoint}{Hermitian} $E$. Deduce \emph{Weyl's perturbation
        theorem}:
        \[
        ...
        \qquad (1 \leq k \leq n) :
        \]
        each \omterm{def:b2:reduction:eigen}{eigenvalue} is a $1$-Lipschitz function of the
        matrix.
  \item (Rank-one perturbations) Let $P$ be \omterm{def:b2:hermitian:adjoint}{Hermitian} positive
        ...
        new \omterm{def:b2:reduction:eigen}{eigenvalues} \emph{interlace} the old ones.
  \item Check question 8 numerically: $A = \begin{pmatrix} 2 &
        \iu\\ -\iu & 2\end{pmatrix}$
        (\cref{exo:b2:hermitian:4}: \omterm{def:b2:reduction:eigen}{spectrum} $\{3, 1\}$) and
        $E = \begin{pmatrix} 0 & 1\\ 1 & 0\end{pmatrix}$
        (\omterm{def:b2:reduction:eigen}{spectrum} $\{1, -1\}$): compute the \omterm{def:b2:reduction:eigen}{spectrum} of $A +
...
\begin{enumerate}[resume]
  ...
  \item Verify questions 11 and 13 on
        ...
        2+\sqrt2\}\bigr)
        \]
        against its leading $2\times2$ block (\omterm{def:b2:reduction:eigen}{spectrum} $\{1,
        3\}$) and its diagonal.
\end{enumerate}
...
part are \omterm{def:b2:hermitian:adjoint}{Hermitian}.
\begin{enumerate}[resume]
  ...
  \item Prove that the square root \emph{is} monotone: $0 \leq
        ...
        $\mu$ be an \omterm{def:b2:reduction:eigen}{eigenvalue} of $\sqrt B - \sqrt A$ with unit
        \omterm{def:b2:reduction:eigen}{eigenvector} $v$; compute $\langle v, (B - A)v\rangle =
  ...
  \item Let $A, B$ be positive definite. Show that the
        \omterm{def:b2:reduction:eigen}{eigenvalues} of $AB$ (not \omterm{def:b2:hermitian:adjoint}{Hermitian} in general!) are
...
\begin{enumerate}[resume]
  ...
  \item (A potential) For a real diagonal $D =
        ...
        \omterm{def:b2:reduction:eigen}{spectrum}: for every $k$,
        \[
  ...
  \item Check Cauchy interlacing between $T_3$ and $T_2$
        ...
        of the path removed.
  \item Show the extreme \omterm{def:b2:reduction:eigen}{eigenvalues} satisfy, as $n \to
        \infty$:
        \[
  ...
  \item Synthesis. In one sentence each: (i) why the
        variational characterization, not the \omterm{def:b2:reduction:charpoly}{characteristic
        polynomial}, is what makes \omterm{def:b2:reduction:eigen}{eigenvalues} stable
        ...
        min-max principle for \omterm{def:b2:metric:compact}{compact} \omterm{def:b2:hermitian:adjoint}{self-adjoint}
        operators).
\end{enumerate}
\end{problem}
