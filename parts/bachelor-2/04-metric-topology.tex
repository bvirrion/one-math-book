...
any metric space, with balls in place of intervals.
\end{example}
...
the presence of such ``phantom limits''; completeness of $\R$
...
< 1,\ y > x^3\}$ is open --- it is $g^{-1}(\intoo{-\infty}{1})
\cap h^{-1}(\intoo{0}{+\infty})$ for the continuous $g(x,y) =
x^2 + y^2$ and $h(x, y) = y - x^3$, an intersection of two open
preimages. In $\bigl(C(\intcc01), d_\infty\bigr)$: the set of
functions with $f(0) = f(1)$ and $\int_0^1 f = 0$ is closed ---
the preimage of $\{(0,0)\}$ under the continuous map $f \mapsto
\bigl(f(0) - f(1),\ \int_0^1 f\bigr)$ into $\R^2$ (each
coordinate is $1$-Lipschitz, as in \cref{exo:b2:metric:3}). The
method never draws a picture: exhibit a continuous map, read the
...
map $f \mapsto \abs{f(x) - f(y)} - \abs{x - y}$ is continuous
...
On the complete $X = \intcc{0}{1}$, the map $f = \cos$ sends $X$
...
dress. Suppose $f \colon X \to Y$ continuous on the compact $X$
...
mechanism is visible: the local Lipschitz constant $\abs{x + y}$
is bounded on a compact and unbounded on $\R$ --- Heine's
theorem is exactly the statement that compactness caps such
...
\emph{Inheritance:} a closed subset of a known compact space is
compact; a finite union or a product of compacts is compact; a
continuous image of a compact is compact. (3) \emph{Bare
...
Let $K$ be compact, $F$ closed, $K \cap F = \emptyset$ in a
metric space. Then
\[
d(K, F) = \inf\,\{d(x, y) : x \in K,\ y \in F\} > 0 :
\]
the function $x \mapsto d(x, F)$ is continuous
(\cref{exo:b2:metric:11}) and positive on $K$ ($d(x, F) = 0$
would put $x \in \overline F = F$), so it attains a positive
minimum on the compact $K$
(\cref{thm:b2:metric:compactprops}~(3)). Compactness is not
...
axis). Escape to infinity is exactly what compactness forbids.
\end{example}
...
problem). \emph{Connectedness}: it globalizes local statements
...
(i) ``Closed and bounded implies compact'' is a theorem about
$\R^n$, not about metric spaces: an infinite set with the
discrete metric is closed and bounded in itself yet not compact
(\cref{exo:b2:metric:4}), and the closed unit ball of
$C(\intcc01)$ fails too (\cref{ch:b2:nvs}). (ii) Completeness is
...
(\cref{exo:b2:metric:1}). (iii) A continuous bijection need not
be a homeomorphism --- the circle parametrization of
\cref{exo:b2:metric:7}; compactness of the source repairs it.
(iv) Banach's theorem needs $k < 1$ \emph{uniformly}: the
condition $d(f(x), f(y)) < d(x,y)$ alone guarantees nothing on a
non-compact space (\cref{exo:b2:metric:5}). (v) Connected does
not imply path-connected in general --- but for the open
subsets of normed spaces met in this book, the two agree
(\cref{ch:b2:nvs}).
\end{remark}

\begin{remark}[Where this chapter is used]
Everywhere in the analysis half. Completeness of
...
local Picard--Lindel\"of theorem already); compactness gives the
equivalence of norms in finite dimension (\cref{ch:b2:nvs}) and
the existence of extrema in \cref{ch:b2:diffcalc}; connectedness
...
the Year 3 volume, compactness in function spaces (the
...
(\cref{exo:b2:metric:8}): each level deletes the open middle
third of every segment. The intersection $C = \bigcap_n C_n$ is
compact, has empty interior and length zero, yet is equipotent
...
Every continuous $f \colon \intcc01 \to \intcc01$ has a fixed
point --- no contraction hypothesis, no iteration. Consider
$g(x) = f(x) - x$, continuous on the connected $\intcc01$:
\[
g(0) = f(0) \geq 0,
\qquad
g(1) = f(1) - 1 \leq 0 ,
...
Connectedness is a topological fingerprint. Suppose $h \colon
\R^2 \to \R$ were a homeomorphism (a continuous bijection with
continuous inverse). Remove one point $a \in \R^2$: the
restriction $h \colon \R^2\setminus\{a\} \to
\R\setminus\{h(a)\}$ is still a homeomorphism. But $\R^2$ minus
a point is path-connected --- join any two points by a segment,
detouring along a second segment through an auxiliary point if
$a$ blocks the direct one --- hence connected
(\cref{thm:b2:metric:connectedness}~(3)); while $\R$ minus a
point splits into two nonempty open half-lines: not connected.
Connectedness is preserved by continuous maps: contradiction.
...
\]
is well defined, continuous, equal to $0$ exactly on $A$ and to
$1$ exactly on $B$ --- a continuous ``switch'' separating any two
disjoint closed sets.
\end{exercise}

\begin{exercise}[$\star\star\star$]\label{exo:b2:metric:12}
(Baire) Let $X$ be a complete metric space and $(U_n)_{n\geq1}$ a
sequence of dense open subsets. Prove that $\bigcap_n U_n$ is
dense in $X$ \emph{(inside any ball, build nested closed balls
$\overline B(x_n, r_n) \subseteq U_n$ with $r_n \to 0$ and use
completeness)}. Deduce that $\R$ is not a countable union of
...
Completeness plus contraction is a solving machine: feed it an
...
continuous dependence on the data, Kepler's equation, and the
...
continuous function on the rectangle $R = \intcc{t_0 - a}{t_0 +
...
\medskip
\noindent\textbf{Part I --- The complete stage.}
\begin{enumerate}
  \item Prove the two statements quoted in
        \cref{def:b2:metric:complete}: a closed subset of a
        complete metric space is complete, and a complete subset
        of any metric space is closed. Deduce that every closed
        subset of $\bigl(C(I), d_\infty\bigr)$ is a complete
        metric space.
  ...
  \item (Fixed points move less than the maps) Let $g \colon X
        \to X$ be a $k$-contraction of a metric space with fixed
        ...
        g(x)\bigr)}{1 - k}.
        \]
  \item (The iterate trick) Let $X$ be complete nonempty and $g
        \colon X \to X$ a map --- not assumed continuous ---
...
\begin{enumerate}[resume]
  \item Show that a function $y \colon I \to \intcc{y_0 - b}{y_0
        ...
        $I$ if and only if it is continuous and satisfies the
        integral equation
        \[
  ...
  \item Let $X_h = \{y \in C(I) : \abs{y(t) - y_0} \leq b
        ...
        a nonempty closed subset of $C(I)$, hence complete, and
...
\begin{enumerate}[resume]
  \item (Lipschitz fails, uniqueness fails) For $y' =
        ...
        2\sqrt{\abs y}$ fail to be Lipschitz?
  ...
  \item (Completeness is not decor) On $X = \Q \cap
  ...
  \item (Error bars) For a $k$-contraction $g$ on a complete
...
\medskip
\noindent\textbf{Part IV --- Continuous dependence.} In this
...
\begin{enumerate}[resume]
  ...
  \item (Parameters) If a family $f_\lambda$ of fields is
        ...
        $\lambda \mapsto y_\lambda$ is Lipschitz from the
...
\begin{enumerate}[resume]
  \item (Perturbation of the identity) Let $X$ be a complete
  ...
  \item (The Cantor set is a fixed point) Let $S_1(x) = \frac
        ...
        why no \emph{other} nonempty compact set satisfies this
        equation (the map $A \mapsto S_1(A) \cup S_2(A)$ is a
        contraction for a distance between compact sets --- the
        Hausdorff distance, made honest in the Year 3 volume).
  \item (Connectedness globalizes uniqueness) Let $f$ be locally
        Lipschitz in $y$ on an open set, and let $y, z$ be two
