...
is \emph{not} \omterm{def:b2:multint:exact}{exact} --- consistently, $P_y = 2y \neq 1 = Q_x$, so
...
f(x, y) = x^2y + xy^3 + y
\]
is a \omterm{def:b2:multint:exact}{potential} ($\R^2$ is star-shaped, so a \omterm{def:b2:multint:exact}{potential} had to
...
\]
(The \omterm{pb:b2:curves:1}{astroid} fits in the unit disk of \omterm{def:b2:multint:domain}{area} $\pi$; three eighths of
...
$x^2 + y^2 \leq z$, of \omterm{def:b2:multint:domain}{area} $\pi z$:
\[
...
is \omterm{def:b2:metric:continuity}{continuous} --- the integral over the shells $\varepsilon \leq r
...
$\mathcal{C}^1$ in $(x, y)$, \omterm{def:b2:metric:continuity}{continuous} in $t$, with partial
derivatives \omterm{def:b2:metric:continuity}{continuous} on $[0,1] \times U$; differentiation under
the integral sign (\cref{ch:b2:integration}, applied on the
\omterm{def:b2:metric:compact}{compact} $t$-interval $[0,1]$, domination being automatic there)
gives
\[
f_x(x, y)
...
\mapsto e^{-xy}\sin x$ is not \omterm{def:b2:series:def}{absolutely} integrable up to $y =
...
\cref{ch:b2:integration} by differentiating a \omterm{thm:b2:integration:continuity}{parameter integral};
here Fubini does the work instead.
\end{solution}

\begin{solution}{exo:b2:multint:10}
Parametrize by \omterm{def:b2:curves:length}{arc length} $s \in [0, 2\pi]$, so $x'^2 + y'^2 = 1$,
...
\omterm{def:b2:funcseq:def}{pointwise}), which forces $y = a\sin s - b\cos s + c$: the curve is
the unit circle (suitably centered). Since a curve of \omterm{def:b2:curves:length}{length} $L$
rescales to \omterm{def:b2:curves:length}{length} $2\pi$, the general statement is $A \leq
\frac{L^2}{4\pi}$: among all closed curves of given perimeter, the
circle encloses the largest \omterm{def:b2:multint:domain}{area}.
\end{solution}
...
radius $r$, of \omterm{def:b2:multint:domain}{area} $4\pi r^2$.
\end{solution}
...
the correct \omterm{def:b2:multint:domain}{area}.
\end{solution}
...
\omterm{def:b2:funcseq:def}{pointwise}). Now $2W_5 = 2\cdot\frac45\cdot\frac23 =
...
$n$; question 11 turns this back into the two \omterm{def:b2:multint:exact}{closed forms} of
question 6.
...
of radius $r$ carries \omterm{def:b2:multint:domain}{area} $nV_nr^{n-1}$, and the Gaussian
...
v_n(R)$: \omterm{def:b2:multint:domain}{area} is the radial derivative of volume.
...
of $B_n(1)$ has \omterm{def:b2:nvs:norm}{norm} close to $1$ (question 17: radial
...
have \omterm{def:b2:nvs:norm}{norm} of order $1$. High-dimensional volume concentrates
where all coordinates share the \omterm{def:b2:nvs:norm}{norm} budget equally ---
...
$(z, w)$-plane, of \omterm{def:b2:multint:domain}{area} $\pi(1 - x^2 - y^2)$. In polar
coordinates:
\[
...
in each coordinate (homogeneity), and the \omterm{pb:b2:multint:1}{Wallis integrals}
...
coordinates exist in every dimension, and the same \omterm{pb:b2:multint:1}{ball
