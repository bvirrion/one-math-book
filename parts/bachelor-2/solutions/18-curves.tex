...
function; since $P$ lies on the tangent line at $\gamma(s)$,
...
on a dense set, hence everywhere by continuity: $T$ is a
...
0 & \tau & 0\end{pmatrix}
\]
has continuous entries, so the linear system $F' = FA(s)$,
...
$\det F$ is continuous with values in $\{\pm1\}$ and equals
...
\textbf{1.} The system is linear in $(x, y)$ with determinant
...
$D_t$ is exactly the tangent line of the curve $E$ at $E(t)$.
...
\tfrac{t^2}2 :
\]
the envelope of the tangent lines of the parabola is the
...
characteristic point is $\gamma + \frac1\kappa N$, the center
of curvature, and the envelope of the normals is the evolute
...
0$, $-x\sin\theta + y\cos\theta = 0$ has determinant $1$ and
...
\end{align*}
And $(\cos^3t)^{2/3} + (\sin^3t)^{2/3} = 1$: the astroid.
...
y) \mapsto (y, x)$ of the astroid transport the cusp to $(-1,
0)$ and $(0, \pm1)$.
...
\tfrac32$, and by symmetry the total length is $4 \cdot
\tfrac32 = 6$.

\textbf{9.} $E(t) - P_t = (\cos^3 t - \cos t,\ \sin^3 t) =
\sin^2 t\,(-\cos t,\ \sin t) = \sin^2 t\,(Q_t - P_t)$. So the
contact point is the barycenter of $(P_t, \cos^2 t)$ and
...
astroid has area $\int_0^1 y\,\dd x$ with $x = \cos^3 t$
...
\]
the same curve: the envelope of the normals is the locus of
the centers of curvature, as question 4 promised.
...
the evolute sit at the extrema of curvature.
...
envelope}: the evolute is precisely the boundary between the
...
reflected ray is tangent to the caustic, as the envelope
...
the center --- the focal length of the spherical mirror.

\textbf{20.} $\norm{E'(\theta)} = \tfrac32\abs{\sin\theta}$,
so the length is $\tfrac32\int_0^{2\pi}\abs{\sin\theta}\,
...
each reflected ray). The Jacobian determinant
$\det(\partial_\theta\Phi, \partial_r\Phi) =
\det(P_\theta' + rv_\theta', v_\theta)$ is affine in $r$ and
...
dependent. Off the envelope the map is a local diffeomorphism,
...
value, so it blows up along the envelope: the caustic is the
...
$R(\pi) = 4$: half of the arch length $8$ computed in
...
2)$, at distance $4 = R(\pi)$, exactly the length of the
...
part of length $R(s_0) - \bigl(R(s_0) - R(s)\bigr) = R(s)$
...
(1, t) \neq 0$ everywhere --- smooth envelope (the parabola
itself). Sliding ladder: $\Delta = -1$, but $E' =
\tfrac32\sin 2t\,(-\cos t, \sin t)$ vanishes at the quadrant
ends --- the four cusps of the astroid. Normals of the
parabola and of the cycloid: $\Delta = \kappa \neq 0$, and
$E' = R'N$ vanishes exactly where the curvature is extremal
...
of the nephroid, the focal points of the mirror. Extrema of
curvature \emph{must} produce cusps on the envelope of the
