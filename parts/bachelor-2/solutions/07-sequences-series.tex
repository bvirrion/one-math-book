...
sum, as in \cref{ex:b2:series:sinn}): convergent (not \omterm{def:b2:series:def}{absolutely},
by the same $\cos^2$ trick).

$\sum \frac{(-1)^n}{\ln n}$ ($n \geq 2$): alternating test,
$\frac{1}{\ln n} \downarrow 0$: convergent; not \omterm{def:b2:series:def}{absolutely} ($\ln n
\leq n$).
...
\]
First series: alternating, convergent. Second: \omterm{def:b2:series:def}{absolutely}
convergent ($\frac{1}{n^{3/2}}$ scale). Third: \omterm{def:b2:series:def}{absolutely}
convergent. Total: convergent.
\end{solution}

\begin{solution}{exo:b2:series:2}
\omterm{thm:b2:series:fubini}{Cauchy product} of $\sum_{m\geq0} z^m$ with itself (both \omterm{def:b2:series:def}{absolutely}
...
Let $B_n = \sum_{k \leq n} b_k \to B$. \omterm{thm:b2:series:abel}{Abel summation} with $a_k = k$:
\[
...
$\alpha > 1$: \omterm{def:b2:series:def}{absolutely} convergent (domination by $n^{-\alpha}$).
...
\omterm{def:b2:series:def}{absolutely}: $\abs{\sin n\theta} \geq \sin^2 n\theta = \frac{1 -
...
\omterm{def:b2:series:summable}{Summability}: the finite partial sums of
...
\omterm{def:b2:series:summable}{Summability}: bounded by $\zeta(2)^2$ as a product family
...
the \omterm{def:b2:series:summable}{summable family} accordingly (a partition of the index set ---
legitimate for \omterm{def:b2:series:summable}{summable} families by
\cref{thm:b2:series:rearrangement}/\cref{thm:b2:series:fubini}
applied to the partition into \omterm{def:b2:structures:countable}{countably} many classes):
\[
...
\infty$: \omterm{def:b2:series:def}{absolutely} convergent. But the partial sums $S_N =
...
\omterm{def:b2:metric:complete}{Completeness} is exactly what \cref{thm:b2:nvs:absoluteconvergence}
needs.
\end{solution}
...
\omterm{def:b2:metric:continuity}{continuous}, and decreasing for large $t$ (its logarithm has
