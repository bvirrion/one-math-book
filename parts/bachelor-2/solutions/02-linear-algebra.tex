...
Write $V(a_0, \dots, a_n)$ for the determinant and induct on $n$;
$V(a_0) = 1$ starts. Fix $a_0, \dots, a_{n-1}$ and view $D(T) =
V(a_0, \dots, a_{n-1}, T)$, the determinant with last column $(1,
...
determinant, by the permutation formula, only picks the diagonal
...
\]
whose determinant is $\det(A - BD^{-1}C)\det D = \det\bigl((A -
BD^{-1}C)D\bigr) = \det(AD - BD^{-1}CD)$. Since $CD = DC$,
$BD^{-1}CD = BC$: the determinant is $\det(AD - BC)$.
...
\textbf{10.} Take the dual basis $(\varphi_1^*, \dots,
...
\lambda\,\mathrm{id}_E)^{\mathsf T}$ (transposition is linear and
...
T}\psi_y = \psi_{A^{\mathsf T}y}$: the transpose is the
...
equal $x_{k_0}$; propagating around the cycle, $x$ is constant.
...
\mathcal{S}_n^\circ$ (identifying the dual with matrices).
...
annihilator, whose dimension is $n^2 - (n^2 - 1) = 1$: equality.
