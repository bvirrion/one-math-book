...
probabilistic mission: to an $\N$-valued \omterm{def:b2:randomvar:law}{random variable} we attach
the power series with coefficients $\P(X = n)$. This
\emph{\omterm{ex:b2:powerseries:fibonacci}{generating function}} converts sums of \omterm{def:b2:proba:independence}{independent} variables
...
approximation of rare \omterm{def:b2:proba:space}{events}, and the extinction criterion for
...
Let $X$ be an $\N$-valued \omterm{def:b2:randomvar:law}{random variable}, $p_n = \P(X = n)$. The
\emph{probability generating function}\index{probability generating
function} of $X$ is the sum of the
power series
\[
...
\omterm{ex:b2:powerseries:fibonacci}{generating functions} at the end of the chapter.
\end{example}

\begin{proposition}[Radius and first properties]\label{prop:b2:genfun:radius}
The series defining $G_X$ has \omterm{def:b2:powerseries:radius}{radius of convergence} $\geq 1$;
$G_X$ is defined and \omterm{def:b2:metric:continuity}{continuous} on $\intcc{-1}{1}$,
$\mathcal{C}^\infty$ on $\intoo{-1}{1}$, with $G_X(1) = 1$ and
$\abs{G_X(t)} \leq 1$ there. Moreover $G_X$ determines the \omterm{def:b2:randomvar:law}{law} of
$X$:
\[
p_n = \frac{G_X^{(n)}(0)}{n!} .
\]
...
\omterm{def:b2:series:def}{absolutely} ($\sum p_n = 1$ dominates); better, on the whole
interval $\intcc{-1}1$,
\[
\sup_{\abs t\leq1}\,\abs{p_nt^n} = p_n
\quad\text{with}\quad \sum_np_n < \infty :
\]
the series converges \emph{\omterm{def:b2:funcseq:series}{normally}} on $\intcc{-1}1$, so
its sum is \omterm{def:b2:metric:continuity}{continuous} there
...
being recoverable, two variables with the same \omterm{ex:b2:powerseries:fibonacci}{generating function}
have the same \omterm{def:b2:randomvar:law}{law}.
\end{proof}
...
= \frac1{k+1}$ and term-by-term integration (\omterm{def:b2:funcseq:series}{normal
...
\cref{ex:b2:randomvar:transferex}. The \omterm{ex:b2:powerseries:fibonacci}{generating function}
is a two-way instrument: differentiate at $1$ for the
moments $\E(X)$, $\E(X(X-1))$, integrate over $\intcc01$
for $\E\bigl(\frac1{1+X}\bigr)$ --- one \omterm{def:b2:powerseries:analytic}{analytic} object,
...
probability \omterm{def:b2:randomvar:law}{law} by Basel's identity
(\cref{ex:b2:fourier:basel}). Its \omterm{ex:b2:powerseries:fibonacci}{generating function}
$G(t) = \frac6{\pi^2}\sum_{k\geq1}\frac{t^k}{k^2}$ has \omterm{def:b2:powerseries:radius}{radius
...
\]
$G$ is \omterm{def:b2:metric:continuity}{continuous} on $\intcc{-1}1$, smooth inside, but its
derivative blows up at $1^-$ --- the graph arrives at the
point $(1, 1)$ with a vertical tangent. Heavy tails are
visible \emph{geometrically} on the \omterm{ex:b2:powerseries:fibonacci}{generating function}, at
...
$X$ has an \omterm{def:b2:randomvar:expectation}{expectation} if and only if $G_X$ is \omterm{def:b2:diffcalc:differential}{differentiable} at
$1^-$ (left derivative, finite), and then $\E(X) = G_X'(1)$.
Similarly $X$ has a second moment iff $G_X$ is twice
\omterm{def:b2:diffcalc:differential}{differentiable} at $1^-$, and then
\[
...
\omterm{def:b2:diffcalc:differential}{differentiable} at $1^-$ with $G_X'(1) = \sum np_n = \E(X)$ (by
...
a second moment. The \omterm{def:b2:randomvar:variance}{variance} formula then follows from
K\"onig--Huygens:
\[
...
to qualitative facts about a discrete \omterm{def:b2:randomvar:law}{law} --- no \omterm{ex:b2:powerseries:fibonacci}{generating
function} needed, but the coefficients \emph{are} the
\omterm{ex:b2:powerseries:fibonacci}{generating function}, read term by term.
\end{example}

\section{Sums of independent variables}

\begin{theorem}[Multiplicativity]\label{thm:b2:genfun:product}
If $X$ and $Y$ are \omterm{def:b2:proba:independence}{independent} $\N$-valued \omterm{def:b2:randomvar:law}{random variables}, then
\[
G_{X + Y}(t) = G_X(t)\,G_Y(t)
\qquad (\abs t \leq 1),
\]
and by induction $G_{X_1 + \dots + X_n} = \prod_i G_{X_i}$ for
\omterm{def:b2:proba:independence}{independent} $X_1, \dots, X_n$.
\end{theorem}

\begin{proof}
Two proofs, both instructive. \emph{Via \omterm{def:b2:randomvar:expectation}{expectations}:} $t^X$ and
$t^Y$ are \omterm{def:b2:proba:independence}{independent} bounded variables, so
...
\]
\emph{Via \omterm{thm:b2:series:fubini}{Cauchy products}:} the \omterm{def:b2:randomvar:law}{law} of $X + Y$ is the convolution
$\P(X + Y = n) = \sum_{k=0}^n \P(X = k)\P(Y = n - k)$, and the
\omterm{thm:b2:series:fubini}{Cauchy product} theorem for \omterm{def:b2:series:def}{absolutely} convergent series
...
\omterm{def:b2:proba:independence}{Independent} binomials with the same $p$ add: $(1 - p + pt)^m(1 -
p + pt)^n = (1 - p + pt)^{m+n}$, so $\mathcal{B}(m, p) +
\mathcal{B}(n, p) = \mathcal{B}(m + n, p)$ --- in particular a sum
of $n$ \omterm{def:b2:proba:independence}{independent} Bernoulli variables is binomial, re-proving the
\omterm{def:b2:randomvar:law}{law} of the number of successes. \omterm{def:b2:proba:independence}{Independent} Poissons add:
...
\]
the triangular \omterm{def:b2:randomvar:law}{law} of dice sums ($7$ is the mode, with
...
counting arguments; the \omterm{ex:b2:powerseries:fibonacci}{generating function} does them all
simultaneously, because multiplying polynomials \emph{is}
convolving coefficients. This mechanical translation ---
\omterm{def:b2:randomvar:law}{laws} to coefficients, sums to products --- is the whole
...
Which \omterm{def:b2:randomvar:law}{law} has $G(t) = \dfrac1{2 - t}$? Expand into a power
series:
\[
...
genuine \omterm{def:b2:randomvar:law}{law}, $\P(X = k) = 2^{-(k+1)}$ on $\N$ --- a
geometric \omterm{def:b2:randomvar:law}{law} starting at $0$. By uniqueness
(\cref{prop:b2:genfun:radius}), no other \omterm{def:b2:randomvar:law}{law} shares this
$G$. Recognizing \omterm{def:b2:randomvar:law}{laws} from their \omterm{ex:b2:powerseries:fibonacci}{generating functions} is a
skill worth drilling: it is how the critical branching
iterate $G_n(t) = \frac{n - (n-1)t}{n+1 - nt}$ of the
weekend problem is unmasked as a geometric \omterm{def:b2:randomvar:law}{law} conditioned
on survival.
\end{example}

\begin{remark}
Stability goes one way only: sums of \omterm{def:b2:proba:independence}{independent} Poissons are
Poisson, but differences are not --- $X - Y$ takes negative
values, so it has no \omterm{ex:b2:powerseries:fibonacci}{generating function} at all, and its \omterm{def:b2:randomvar:law}{law}
(the Skellam \omterm{def:b2:randomvar:law}{distribution}) lies outside this chapter's
...
Let $(X_k)_{k\geq1}$ be \omterm{def:b2:proba:independence}{independent} $\N$-valued variables with the
same \omterm{def:b2:randomvar:law}{law} and \omterm{ex:b2:powerseries:fibonacci}{generating function} $G_X$, and let $N$ be an
$\N$-valued variable \omterm{def:b2:proba:independence}{independent} of the $X_k$, with \omterm{ex:b2:powerseries:fibonacci}{generating
function} $G_N$. Then the random sum $S = X_1 + \dots + X_N$ (with
$S = 0$ when $N = 0$) has \omterm{ex:b2:powerseries:fibonacci}{generating function}
\[
G_S = G_N \circ G_X .
\]
In particular, if $N$ and $X_1$ have \omterm{def:b2:randomvar:expectation}{expectations}, $\E(S) =
\E(N)\,\E(X_1)$.
\end{theorem}
...
= G_N\bigl(G_X(t)\bigr),
\]
using multiplicativity for each fixed $n$ and the \omterm{def:b2:series:summable}{summability} of
the whole double family ($\abs{G_X(t)} \leq 1$). The exchange of
summations is Fubini for \omterm{def:b2:series:summable}{summable} families
...
\]
The \omterm{def:b2:randomvar:variance}{variance} involves the \emph{second} moment of a single
claim, not its \omterm{def:b2:randomvar:variance}{variance}: a compound Poisson sum feels the
occasional large claim twice --- once through how many, once
through how big. For $\lambda = 10$ claims of geometric \omterm{def:b2:randomvar:law}{law}
...
\cref{prop:b2:genfun:branching}: composition of \omterm{ex:b2:powerseries:fibonacci}{generating
functions} is the algebra of random populations.
\end{example}

\begin{remark}
The \omterm{def:b2:proba:independence}{independence} of $N$ from the summands is not decorative.
...
\]
the binomial \omterm{def:b2:randomvar:law}{law} of many rare \omterm{def:b2:proba:independence}{independent events} converges to the
Poisson \omterm{def:b2:randomvar:law}{law} of parameter $\lambda$.
\end{theorem}
...
\omterm{ex:b2:powerseries:fibonacci}{generating functions}: $G_{X_n}(t) = \bigl(1 +
\frac{\lambda_n(t-1)}{n}\bigr)^n \to e^{\lambda(t - 1)} =
G_{\mathcal{P}(\lambda)}(t)$ for each fixed $t \in [0, 1]$ ---
convergence of \omterm{ex:b2:powerseries:fibonacci}{generating functions}, which (for $\N$-valued
variables) is equivalent to convergence of each $\P(X_n = k)$;
see \cref{exo:b2:genfun:9}.
\end{proof}

\begin{remark}
This is why Poisson \omterm{def:b2:randomvar:law}{laws} model counts of rare \omterm{def:b2:proba:space}{events} --- typos per
...
\omterm{def:b2:proba:space}{no-event} probability is exactly $\P(X_n = 0) = (1 - 2/n)^n$:
\[
n = 10:\ 0.107, \qquad
n = 20:\ 0.122, \qquad
n = 50:\ 0.130, \qquad
...
limit \omterm{def:b2:randomvar:law}{law} mercifully depends on nothing else.
\end{example}

\section{Branching processes}

Consider a population starting from one ancestor; each individual,
independently, has a random number of children with \omterm{def:b2:randomvar:law}{law} $(p_k)_{k
\in \N}$ and \omterm{ex:b2:powerseries:fibonacci}{generating function} $G$ (the \emph{offspring
\omterm{def:b2:randomvar:law}{distribution}}). Let $Z_n$ be the size of generation $n$ ($Z_0 =
1$), and let $m = G'(1) = \E(Z_1)$ be the mean offspring number.

\begin{proposition}\label{prop:b2:genfun:branching}
The \omterm{ex:b2:powerseries:fibonacci}{generating function} of $Z_n$ is the $n$-th iterate
...
members of generation $n$, with counts \omterm{def:b2:proba:independence}{independent} of each other
...
G\bigl(G_{Z_n}(0)\bigr) = G(q_n)$. The \omterm{def:b2:proba:space}{events} $\{Z_n = 0\}$ increase (extinct populations stay
extinct), so $q_n \uparrow q = \P\bigl(\bigcup_n\{Z_n = 0\}\bigr)$
by monotone \omterm{def:b2:metric:continuity}{continuity} (\cref{thm:b2:proba:continuity}), and
\omterm{def:b2:metric:continuity}{continuity} of $G$ on $[0, 1]$ turns $q_{n+1} = G(q_n)$ into $q =
G(q)$ at the limit.
\end{proof}

\begin{example}[Watching extinction converge]
\label{ex:b2:genfun:cobwebnumerics}
For the offspring \omterm{def:b2:randomvar:law}{law} $(p_0, p_1, p_2) = (\tfrac14,
...
\begin{proof}
$G$ is \omterm{def:b2:affine:convex}{convex} on $\intcc{0}{1}$ (power series with nonnegative
...
some interval $\intoo{1 - \delta}{1}$: the \omterm{def:b2:metric:continuity}{continuous} function
...
subcritical offspring \omterm{def:b2:randomvar:law}{law} --- the convex curve stays above the
diagonal, the iteration climbs to the unique fixed point $1$.
Right: a supercritical \omterm{def:b2:randomvar:law}{law} --- the curve crosses the diagonal at
...
Offspring \omterm{def:b2:randomvar:law}{law}: no child, one child, two children with
...
probability from \cref{ch:b2:proba} (monotone \omterm{def:b2:metric:continuity}{continuity}
proves $q_n \uparrow q$) and \cref{ch:b2:randomvar} ($G_X =
\E(t^X)$ is an \omterm{def:b2:randomvar:expectation}{expectation}, multiplicativity is the product
...
(i) \omterm{ex:b2:powerseries:fibonacci}{Generating functions} apply to $\N$-valued variables only:
...
means an algebra slip, not a new \omterm{def:b2:randomvar:law}{law}. (iii) In random sums,
...
of items). (iv) Multiplicativity needs \omterm{def:b2:proba:independence}{independence} and
...
The \omterm{ex:b2:powerseries:fibonacci}{generating function} is a fitting final object for this book:
it is simultaneously a power series (\cref{ch:b2:powerseries}), a
tool of \omterm{def:b2:series:summable}{summable} families (\cref{ch:b2:series}), an \omterm{def:b2:randomvar:expectation}{expectation}
...
Compute the \omterm{ex:b2:powerseries:fibonacci}{generating function} of the uniform \omterm{def:b2:randomvar:law}{law} on $\{1, 2,
...
Using \omterm{ex:b2:powerseries:fibonacci}{generating functions}, recover $\E$ and $V$ for the binomial
and geometric \omterm{def:b2:randomvar:law}{laws} (\cref{thm:b2:genfun:moments}).
\end{exercise}
...
Let $X_1, X_2, \dots$ be \omterm{def:b2:proba:independence}{independent} Bernoulli $\mathcal{B}(p)$
and $N \sim \mathcal{P}(\lambda)$ \omterm{def:b2:proba:independence}{independent} of them. Show,
...
Compute also the \omterm{def:b2:randomvar:law}{law} of the discarded count and admire: it is
...
\omterm{def:b2:proba:independence}{independent} geometric variables, deduce
\[
...
For the offspring \omterm{def:b2:randomvar:law}{law} $p_0 = \frac18$, $p_1 = \frac38$, $p_2 =
...
(Total progeny) In a subcritical \omterm{pb:b2:genfun:1}{branching process} ($m < 1$), let
$Y = \sum_{n\geq0} Z_n$ be the total number of individuals ever
born. Show $\E(Y) = \sum_n m^n = \frac{1}{1 - m}$ (justify the
exchange of summations), and prove that the \omterm{ex:b2:powerseries:fibonacci}{generating function}
$H = G_Y$ satisfies the functional equation $H(t) = t\,G(H(t))$.
\emph{(The ancestor, plus the total progenies of each of its
children, which are \omterm{def:b2:proba:independence}{independent} copies of $Y$.)}
\end{exercise}

\begin{exercise}[$\star\star\star$]\label{exo:b2:genfun:8}
Let $X$ have \omterm{ex:b2:powerseries:fibonacci}{generating function} $G$ with \omterm{def:b2:powerseries:radius}{radius of convergence} $>
...
(\omterm{def:b2:metric:continuity}{Continuity} theorem, elementary case) Let $X, X_1, X_2, \dots$ be
...
consider $\frac{G(t) - \P(X = 0)}{t}$, the \omterm{ex:b2:powerseries:fibonacci}{generating function} of
a shifted \omterm{def:b2:randomvar:law}{law}.)}
\end{exercise}
...
\omterm{ex:b2:powerseries:fibonacci}{generating function}
\[
\frac{t + t^2 + \dots + t^6}{6}
= \frac{t\,(1 + t)(1 + t + t^2)(1 - t + t^2)}{6},
\]
and show that the two dice with faces $\{1, 2, 2, 3, 3, 4\}$
and $\{1, 3, 4, 5, 6, 8\}$ have \omterm{ex:b2:powerseries:fibonacci}{generating functions}
...
the \omterm{ex:b2:powerseries:fibonacci}{generating functions} from the states ``no current head''
...
the chapter's notation: offspring \omterm{def:b2:randomvar:law}{law} $(p_k)$ with pgf $G$,
...
exactly.} Let the offspring \omterm{def:b2:randomvar:law}{law} be geometric on $\N$: $p_k =
...
\begin{enumerate}[resume]
  ...
  \item Critical case ($p = \tfrac12$): compute $\sigma^2 =
        ...
        like $\frac1n$ --- neither geometric nor \omterm{def:b2:series:summable}{summable}.
...
\begin{enumerate}[resume]
  ...
  \item With the central binomial asymptotics
        ...
        order suffice).
  \item Compare with the fair \omterm{pb:b2:proba:1}{random walk} (the weekend
...
\infty$.
\begin{enumerate}[resume]
  \item Show that $G''$ extends \omterm{def:b2:metric:continuity}{continuously} to $\intcc01$
  ...
  \item Telescope along the iteration $q_{j+1} = G(q_j)$:
        ...
        \omterm{pb:b2:genfun:1}{branching process} dies at the universal rate $1/n$,
        with only the constant remembering the offspring \omterm{def:b2:randomvar:law}{law}.
...
\begin{enumerate}[resume]
  \item (Epidemics, chain reactions) For a Poisson offspring
        \omterm{def:b2:randomvar:law}{law} $\mathcal P(\lambda)$ --- each case infects
  ...
  \item (Conditioning a supercritical process on extinction)
        ...
        of a bona fide offspring \omterm{def:b2:randomvar:law}{law}, with mean $\widehat m =
  ...
  \item Synthesis: draw up the trichotomy table --- for $m <
        ...
        Yaglom's exponential limit \omterm{def:b2:randomvar:law}{law}).
\end{enumerate}
\end{problem}
