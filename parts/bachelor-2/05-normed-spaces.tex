...
When the \omterm{def:b2:metric:def}{metric space} is a vector space and the distance comes from
a \omterm{def:b2:nvs:norm}{norm}, topology and linear algebra begin to interact: linear maps
are \omterm{def:b2:metric:continuity}{continuous} exactly when they are bounded on the unit ball,
finite dimension forces all \omterm{def:b2:nvs:norm}{norms} to agree, and \omterm{def:b2:metric:complete}{completeness} turns
...
\omterm{def:b2:metric:continuity}{continuous} (estimates $\norm{(x + y) - (x' + y')} \leq \norm{x -
x'} + \norm{y - y'}$, etc.).
\end{definition}
...
\]
($\norm\cdot_2$ is a \omterm{def:b2:nvs:norm}{norm} by Cauchy--Schwarz, Year 1 volume). On
$C(\intcc{a}{b})$:
\[
...
\]
the last two being \omterm{def:b2:nvs:norm}{norms} thanks to strict positivity of the
integral and integral Cauchy--Schwarz (Year 1 volume). On matrices:
any \omterm{def:b2:nvs:norm}{norm} on $\mathcal M_n(K) \simeq K^{n^2}$; the \omterm{thm:b2:nvs:continuouslinear}{operator norms}
below are the structurally important ones.
\end{example}

\begin{definition}[Equivalent norms]\label{def:b2:nvs:equivalent}
Two \omterm{def:b2:nvs:norm}{norms} $N_1, N_2$ on $E$ are \emph{equivalent}\index{equivalent
norms} when there are constants $c, C > 0$ with
\[
c\,N_1 \leq N_2 \leq C\, N_1 .
\]
Equivalent \omterm{def:b2:nvs:norm}{norms} have the same \omterm{def:b2:metric:topology}{open} sets, the same convergent and
Cauchy sequences, the same \omterm{def:b2:metric:compact}{compact} and \omterm{def:b2:metric:complete}{complete} subsets: the same
analysis.
\end{definition}
...
$\norm\cdot_1$ but not for $\norm\cdot_\infty$: the two \omterm{def:b2:nvs:norm}{norms}
disagree about convergence itself.
\end{example}

\begin{example}[Explicit constants in dimension $n$]\label{ex:b2:nvs:constants}
On $K^n$ the three classical \omterm{def:b2:nvs:norm}{norms} are \omterm{def:b2:nvs:equivalent}{equivalent} with sharp
constants:
\[
\norm x_\infty \leq \norm x_2 \leq \norm x_1
\leq \sqrt n\,\norm x_2 \leq n\,\norm x_\infty ,
...
{\small The unit balls of the three classical \omterm{def:b2:nvs:norm}{norms} of $\R^2$,
nested as the inequalities of \cref{ex:b2:nvs:constants}
dictate: smaller ball, larger \omterm{def:b2:nvs:norm}{norm}. Roundness matters: the flat
...
following are \omterm{def:b2:nvs:equivalent}{equivalent}:
\begin{enumerate}
  \item $u$ is \omterm{def:b2:metric:continuity}{continuous};
  \item $u$ is \omterm{def:b2:metric:continuity}{continuous} at $0$;
...
F)$ of \omterm{def:b2:metric:continuity}{continuous} linear maps a normed space, with
\[
...
\begin{proof}
(1 $\Rightarrow$ 2) trivial. (2 $\Rightarrow$ 3): \omterm{def:b2:metric:continuity}{continuity} at $0$
...
\omterm{def:b2:nvs:norm}{Norm} axioms for $\vertiii\cdot$: homogeneity and separation are
...
$f \mapsto f(0)$ has \omterm{thm:b2:nvs:continuouslinear}{operator norm} $1$; integration $f \mapsto
\int_0^1 f$ has \omterm{def:b2:nvs:norm}{norm} $1$; the map $f \mapsto \int_0^1 t f(t)\dd t$
has \omterm{def:b2:nvs:norm}{norm} $\int_0^1 t\,\dd t = \frac12$ (upper bound by the triangle
inequality for integrals; attained at $f \equiv 1$). But
differentiation, from $(C^1, \norm\cdot_\infty)$ to $(C^0,
\norm\cdot_\infty)$, is \emph{not} \omterm{def:b2:metric:continuity}{continuous}: $\norm{\sin(nx)}_\infty
= 1$ while the derivative has sup \omterm{def:b2:nvs:norm}{norm} $n$. Linear does not imply
\omterm{def:b2:metric:continuity}{continuous} in infinite dimension.
\end{example}
...
\]
one sequence, three \omterm{def:b2:nvs:norm}{norms}, three behaviours --- convergence to
zero, no convergence (\omterm{def:b2:nvs:norm}{norms} stabilize at $\frac1{\sqrt2}$ but
...
sequence: it is a question about a sequence \emph{and} a \omterm{def:b2:nvs:norm}{norm}.
\end{example}
...
\emph{any} of the \omterm{def:b2:nvs:norm}{norms} $\norm\cdot_1, \norm\cdot_2,
...
reasonable \omterm{def:b2:nvs:norm}{norms} tell the same story, the largest stretch
factor. Everything difficult about \omterm{thm:b2:nvs:continuouslinear}{operator norms} is about
\emph{non}-diagonal behaviour --- which is why the adapted
\omterm{def:b2:nvs:norm}{norms} of this chapter's weekend problem (question 22) work by
...
On $(\R^n, \norm\cdot_1)$, the \omterm{thm:b2:nvs:continuouslinear}{operator norm} of a matrix $A$ is
...
a law: \omterm{def:b2:linalg:transpose}{transpose} the matrix entries asymmetrically and the two
\omterm{def:b2:nvs:norm}{norms} part company. Rows for $\norm\cdot_\infty$, columns for
$\norm\cdot_1$: the mnemonic is that each \omterm{def:b2:nvs:norm}{norm}'s unit vectors
(sign patterns, resp.\ basis vectors) pick out the matching
sums.
\end{example}

\begin{proposition}[Bilinear maps]\label{prop:b2:nvs:bilinear}
A bilinear map $b \colon E \times F \to G$ is \omterm{def:b2:metric:continuity}{continuous} iff
$\norm{b(x,y)} \leq C\norm x\,\norm y$ for some $C$; then it is
\omterm{def:b2:metric:continuity}{Lipschitz} on bounded sets. (Same proof pattern; the product
...
C\norm{x_0}\norm{y - y_0}$: \omterm{def:b2:metric:continuity}{continuity} at $(x_0, y_0)$, and a
\omterm{def:b2:metric:continuity}{Lipschitz} bound where $\norm x, \norm y \leq R$. Conversely,
\omterm{def:b2:metric:continuity}{continuity} at $(0,0)$ gives $\delta$ with $\norm{b(x,y)} \leq 1$ on
...
On a finite-dimensional space, \emph{all \omterm{def:b2:nvs:norm}{norms} are
\omterm{def:b2:nvs:equivalent}{equivalent}}.\index{equivalence of norms} Consequently, in finite
dimension: convergence, openness, \omterm{def:b2:metric:compact}{compactness}, \omterm{def:b2:metric:complete}{completeness} are
norm-independent notions; \omterm{def:b2:metric:compact}{compact} $=$ closed and bounded;
the space is \omterm{def:b2:metric:complete}{complete}; and every linear (or multilinear) map
\emph{from} a finite-dimensional space is \omterm{def:b2:metric:continuity}{continuous}.
\end{theorem}

\begin{proof}
Fix a basis and identify $E \simeq K^n$; it suffices to compare any
\omterm{def:b2:nvs:norm}{norm} $N$ with $\norm\cdot_\infty$.

\emph{One direction is algebra:} $N(x) = N(\sum x_i e_i) \leq
\sum \abs{x_i} N(e_i) \leq C \norm x_\infty$ with $C = \sum N(e_i)$.
This also shows $N$ is \omterm{def:b2:metric:continuity}{continuous} on $(K^n, \norm\cdot_\infty)$
...
\norm\cdot_\infty)$, hence \omterm{def:b2:metric:compact}{compact}
(\cref{thm:b2:metric:compactprops}~(2), valid for $\C^n \simeq
\R^{2n}$). The \omterm{def:b2:metric:continuity}{continuous} function $N$ attains its minimum $c$ on
...
\begin{proof}
It is \omterm{def:b2:metric:complete}{complete} for the induced \omterm{def:b2:nvs:norm}{norm}
(\cref{thm:b2:nvs:finitedim}), and \omterm{def:b2:metric:complete}{complete} subsets are closed
...
The closed unit ball of a normed space $E$ is \omterm{def:b2:metric:compact}{compact} \emph{if and
...
is not \omterm{def:b2:metric:compact}{compact}.
\end{proof}
...
the ball is not \omterm{def:b2:metric:compact}{compact}, so $\dim C(\intcc01) = \infty$ by
\cref{thm:b2:nvs:riesz}. \omterm{def:b2:metric:compact}{Compactness} of the unit ball is a
...
A \emph{Banach space}\index{Banach space} is a \omterm{def:b2:metric:complete}{complete} normed
...
\omterm{def:b2:metric:continuity}{continuous} functions). Non-example: $\bigl(C(\intcc{0}{1}),
...
\int_0^x f(t)\,\dd t$ (an endomorphism: $T(f)$ is \omterm{def:b2:metric:continuity}{continuous}).
...
In a \omterm{def:b2:nvs:banach}{Banach space}, if $\sum \norm{u_n} < \infty$ then $\sum u_n$
converges, and $\norm{\sum u_n} \leq \sum\norm{u_n}$. (The full
...
for the real series of \omterm{def:b2:nvs:norm}{norms}): $(S_N)$ is Cauchy, hence convergent.
...
The \emph{matrix exponential}\index{matrix exponential}:
$\mathcal{M}_n(K)$ with any submultiplicative \omterm{def:b2:nvs:norm}{norm} ($\vertiii{AB}
...
0\end{psmallmatrix}$: $\vertiii A < 1$ in any \omterm{thm:b2:nvs:continuouslinear}{operator norm}
built on \cref{ex:b2:nvs:examples}'s \omterm{def:b2:nvs:norm}{norms}, and $A^2 = 0$, so
...
\omterm{def:b2:structures:sn}{cycle} with period four, and the series splits into even and odd
parts:
\[
...
scalar ones, once a submultiplicative \omterm{def:b2:nvs:norm}{norm} certifies absolute
convergence.
\end{example}
...
\to 0$ on $\intco{0}{1}$ and $f_n(1) = 1$; in \omterm{def:b2:nvs:norm}{norm},
...
about this one \omterm{def:b2:nvs:norm}{norm}; keeping the dictionary in mind halves
that chapter.
\end{example}

\begin{remark}[Common pitfalls]\label{rem:b2:nvs:pitfalls}
(i) An \omterm{thm:b2:nvs:continuouslinear}{operator norm} depends on \emph{both} chosen \omterm{def:b2:nvs:norm}{norms}: the
same matrix has $\vertiii\cdot_\infty$ given by row sums
(\cref{exo:b2:nvs:4}) and a different $\vertiii\cdot_1$ (column
sums); quoting ``the'' \omterm{def:b2:nvs:norm}{norm} of a matrix without naming the
underlying \omterm{def:b2:nvs:norm}{norms} is meaningless. (ii) $\vertiii{AB} \leq
\vertiii A\,\vertiii B$ is an inequality, usually strict ---
powers can shrink much faster than the bound $\vertiii A^k$
suggests, which is the whole point of adapted \omterm{def:b2:nvs:norm}{norms} (this
chapter's weekend problem, question 22). (iii) ``Linear implies
\omterm{def:b2:metric:continuity}{continuous}'' is a finite-dimensional privilege: differentiation
on polynomials is linear and unbounded
(\cref{ex:b2:nvs:operatornorms}). (iv) Absolute convergence of
$\sum u_n$ only helps when the space is \omterm{def:b2:metric:complete}{complete}
...
a \omterm{def:b2:nvs:banach}{Banach space}, absolutely convergent series converge, so the
...
dominate. With \cref{ch:b2:fourier}: the \omterm{def:b2:nvs:norm}{norms}
...
\omterm{thm:b2:nvs:continuouslinear}{Operator norms} and the geometric series drive the perturbation
arguments of \cref{ch:b2:diffcalc} (inverse function theorem) and
the \omterm{ex:b2:nvs:matrixexp}{matrix exponential} of \cref{ch:b2:diffeq}; \omterm{thm:b2:nvs:finitedim}{equivalence of
norms} silently authorizes every ``choose your favourite \omterm{def:b2:nvs:norm}{norm}''
...
Is $N(f) = \abs{f(0)} + \norm{f'}_\infty$ a \omterm{def:b2:nvs:norm}{norm} on
$C^1(\intcc{0}{1})$? Compare it with $\norm{f}_\infty$: one
inequality holds, the other fails (exhibit).
\end{exercise}

\begin{exercise}[$\star$]\label{exo:b2:nvs:3}
Compute the \omterm{thm:b2:nvs:continuouslinear}{operator norm} of $u(f) = \int_0^1 f(t)\,\eu^t\,\dd t$
...
On $(\R^n, \norm\cdot_\infty)$, prove that the \omterm{thm:b2:nvs:continuouslinear}{operator norm} of a
...
Prove that $GL_n(K)$ is \omterm{def:b2:metric:topology}{open} in $\mathcal{M}_n(K)$ and that $A
\mapsto A^{-1}$ is \omterm{def:b2:metric:continuity}{continuous} on it. \emph{Hint: for openness, if
...
(\cref{thm:b2:nvs:absoluteconvergence}); for \omterm{def:b2:metric:continuity}{continuity}, bound
...
$\varphi$ is \omterm{def:b2:metric:continuity}{continuous} if and only if $\ker\varphi$ is closed.
...
\omterm{def:b2:metric:complete}{complete}: show that the functions $f_n$, affine ramps from $0$ to
$1$ over $\bigl[\frac12 - \frac1n, \frac12\bigr]$ (value $0$
before, $1$ after), form a Cauchy sequence with no \omterm{def:b2:metric:continuity}{continuous}
$\norm\cdot_1$-limit.
\end{exercise}
...
\]
Prove that $\varphi$ is a well-defined \omterm{def:b2:metric:continuity}{continuous} linear form with
$\vertiii\varphi = 1$, but that the supremum defining
$\vertiii\varphi$ is \emph{not attained} on the closed unit ball.
\emph{(Upper bound: triangle inequality. \omterm{def:b2:nvs:norm}{Norm} $= 1$: build
\omterm{def:b2:metric:continuity}{continuous} $f_K$ with $\norm{f_K}_\infty \leq 1$ and $f_K(\frac1n)
= (-1)^n$ for $n \leq K$ --- the points $\frac1n$ are isolated from
each other. Non-attainment: equality would force $f(\frac1n) =
(-1)^n$ for every $n$, incompatible with \omterm{def:b2:metric:continuity}{continuity} of $f$ at $0$
since $\frac1n \to 0$.)}
\end{exercise}

\begin{exercise}[$\star\star\star$]\label{exo:b2:nvs:9}
Let $E$ be a normed space in which the closed unit ball is \omterm{def:b2:metric:compact}{compact}.
...
linear form on $E$ is \omterm{def:b2:metric:continuity}{continuous} if and only if $\dim E < \infty$.
...
up to a constant: the three \omterm{def:b2:nvs:norm}{norms} are pairwise \omterm{def:b2:nvs:equivalent}{non-equivalent}.
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:b2:nvs:11}
(Distance to a hyperplane) Let $\varphi$ be a nonzero \omterm{def:b2:metric:continuity}{continuous}
...
$N_1$ and $N_2$ are \emph{not} \omterm{def:b2:nvs:equivalent}{equivalent}; deduce that the
identity $(E, N_2) \to (E, N_1)$ is a \omterm{def:b2:metric:continuity}{continuous} linear bijection
whose inverse is discontinuous. Show finally that $(E, N_1)$ is
not \omterm{def:b2:metric:complete}{complete} \emph{(Taylor partial sums of $\eu^x$)}. All three
...
the answer belongs to this chapter: \omterm{def:b2:metric:compact}{compactness} in finite
...
smallest sup \omterm{def:b2:nvs:norm}{norm} on $\intcc{-1}{1}$ is the (normalized) Chebyshev
polynomial, of \omterm{def:b2:nvs:norm}{norm} $2^{1-n}$: \emph{Chebyshev's extremal
theorem}. The problem proves both sides, then measures how badly
\omterm{def:b2:metric:compact}{compactness} fails in infinite dimension: the unit ball of
...
\label{pb:b2:nvs:1}
\omterm{def:b2:nvs:norm}{Norms} without subscript are sup \omterm{def:b2:nvs:norm}{norms} on the indicated segment.
...
\begin{enumerate}
  \item Let $F$ be a finite-dimensional subspace of a normed
        ...
        \cref{thm:b2:nvs:finitedim})}.
  \item A \omterm{def:b2:nvs:norm}{norm} is \emph{strictly convex} when $\norm u = \norm v
        ...
        $n \geq 2$.
  \item Prove that for a strictly convex \omterm{def:b2:nvs:norm}{norm}, the best
...
\begin{enumerate}[resume]
  \item Let $g_k(x) = x^{2^k}$. Show $\norm{g_k}_\infty = 1$ and
        ...
        \omterm{def:b2:metric:compact}{compact}, by bare hands.
...
\medskip
\noindent\textbf{Part V --- \omterm{def:b2:nvs:norm}{Norms} at work on matrices, and
synthesis.}
\begin{enumerate}[resume]
  \item Prove that every \omterm{def:b2:reduction:eigen}{eigenvalue} $\lambda$ of $A \in
        \mathcal{M}_n(\C)$ satisfies $\abs\lambda \leq
        \vertiii A$ for every \omterm{thm:b2:nvs:continuouslinear}{operator norm}; apply
        \cref{exo:b2:nvs:4} to bound the \omterm{def:b2:reduction:eigen}{eigenvalues} of
        $\begin{psmallmatrix}1 & -2\\ 3 & 1\end{psmallmatrix}$
        and compare with their true modulus.
  \item (Adapted \omterm{def:b2:nvs:norm}{norms}) Let $A$ be \omterm{def:b2:reduction:diag}{diagonalizable}, $A =
        P\,\mathrm{diag}(\lambda_1, \dots, \lambda_n)\,P^{-1}$.
        Show $N_P(x) = \norm{P^{-1}x}_\infty$ is a \omterm{def:b2:nvs:norm}{norm} whose
        \omterm{thm:b2:nvs:continuouslinear}{operator norm} satisfies $\vertiii A_{N_P} =
        \max_i\abs{\lambda_i}$.
  \item Deduce: for \omterm{def:b2:reduction:diag}{diagonalizable} $A$, $A^k \to 0$ if and only
        if all \omterm{def:b2:reduction:eigen}{eigenvalues} satisfy $\abs{\lambda_i} < 1$ ---
        \omterm{thm:b2:nvs:finitedim}{equivalence of norms} makes the conclusion
  ...
  \item (Equivalence constants blow up) On $\R_n[X]$, compare
        ...
        $\norm{P}_{\intcc{0}{1}}$: both are \omterm{def:b2:nvs:norm}{norms}, so they are
        \omterm{def:b2:nvs:equivalent}{equivalent} for each fixed $n$; but show, using the monic
        minimizer of question 13 on $\intcc{0}{1}$, that the
        best constant $C_n$ in $N_c \leq
        C_n\norm\cdot_{\intcc{0}{1}}$ satisfies $C_n \geq
        2^{2n-1}$. Conclude in one sentence why ``all \omterm{def:b2:nvs:norm}{norms} are
        \omterm{def:b2:nvs:equivalent}{equivalent}'' dies in infinite dimension.
  \item (Synthesis) One sentence each: where \omterm{def:b2:metric:compact}{compactness} of
        ...
        spaces, Year 3 volume, where \omterm{def:b2:metric:complete}{completeness} replaces
