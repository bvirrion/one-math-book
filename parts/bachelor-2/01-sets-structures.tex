...
The closing insight: countability proofs are often
...
$\overline\Q$ of algebraic numbers is countable: polynomials of
...
$\intcc{0}{1}$ and $\intcc{0}{1}^2$ are equipotent --- dimension
...
Take $G = \mathfrak{S}_3$ (order $6$) and $H = A_3 =
...
Two immediate dividends. \emph{Groups of prime order are cyclic:}
...
subgroup per divisor of $12$ --- orders $1, 2, 3, 4, 6, 12$,
generated respectively by $\overline 0$, $\overline 6$, $\overline
4$, $\overline 3$, $\overline 2$, $\overline 1$. The closing
caution: the \emph{converse} of Lagrange fails in general --- $A_4$
has order $12$ but no subgroup of order $6$, as we prove in this
chapter's weekend problem (\cref{pb:b2:structures:1}, question 14).
Lagrange restricts the possible orders; it does not promise them.
\end{example}
...
Write $d = \gcd(k, n)$. For any $m \geq 1$, the order
...
How many permutations of $\mathfrak{S}_9$ have the cycle type
$(4, 3, 2)$ --- one $4$-cycle, one $3$-cycle, one transposition?
...
1, 2$. \emph{Via cycles:} $1 \mapsto 3 \mapsto 4 \mapsto 1$ and $2
...
transpositions:} $\sigma = (1\,4)(1\,3)(2\,5)$, three factors,
...
\]
\emph{By ideals:} the ideal $(X^3 - 1)\Q[X] + (X^2 - 1)\Q[X]$ is
...
monic generator is $X - 1$. The closing insight: the ideal
viewpoint identifies the gcd \emph{without dividing} --- common
roots locate the ideal, and Euclid merely certifies it.
\end{example}
...
= 8$ elements. Is it cyclic? Compute orders using the Chinese
isomorphism $(\Z/15\Z)^* \simeq (\Z/3\Z)^* \times (\Z/5\Z)^*$
(a unit mod $15$ is a pair of units): the factors have orders
$2$ and $4$, so every element's order divides
...
\]
orders $4, 2, 4, 2, 2$ and never $8$. Contrast with
\cref{exo:b2:structures:10}: $(\Z/p\Z)^*$ \emph{is} cyclic for
...
principal ideal, exactly as \cref{thm:b2:structures:principal}
predicts, generated by the monic $X^2$ of least degree in the
kernel --- the \emph{minimal polynomial} of $A$, star of
\cref{ch:b2:reduction}. The image is the two-dimensional
commutative algebra $\{aI + bA\}$: evaluation morphisms shrink
the infinite-dimensional $\R[X]$ onto small, computable algebras.
\end{example}
...
coprime-and-commuting statement, and disjoint cycles the
correct permutation version. (iii) Countability survives
countable \emph{unions} and finite \emph{products}, but not
countable products: $\{0,1\}^{\N}$ is uncountable
...
and the principal ideals of $K[X]$ produce minimal polynomials and
the kernel decompositions of \cref{ch:b2:reduction}; countability
is the stage on which \cref{ch:b2:proba} performs (probability on
countable spaces) and the reason topology keeps producing countable
...
countable subset, and deduce that a set $E$ is infinite if and only
if it is equipotent to a proper subset of itself. \emph{(For the
direct implication, shift a countable subset by one step; for the
...
\begin{enumerate}[resume]
  ...
  \item Show that exactly half of all configurations satisfy $I =
        ...
        composing with one fixed transposition of two other
        cells.)}
...
\begin{enumerate}[resume]
  ...
  \item (The grand tour) Verify that
        ...
        = 14$ for its cycle order, check that the reverse
...
\begin{enumerate}[resume]
  \item (Lemma A) Let $s$ and $t$ be $3$-cycles whose supports
        ...
        generated subgroup), the product $st$ is a double
        transposition; show that $A_4$ contains no subgroup of
        order $6$ \emph{(a subgroup of index $2$ contains every
...
\begin{enumerate}[resume]
  ...
  \item (A poor board) Now let the board be a single cycle of $n
        ...
        classes are the orbits of a cyclic group of order
