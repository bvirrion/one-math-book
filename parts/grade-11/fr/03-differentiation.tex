...
À quelle vitesse une fonction change-t-elle~? La réponse en un point
unique est la \emph{dérivée}, le nombre le plus utile de toutes les
mathématiques appliquées. Ce chapitre la construit à partir des taux de
variation moyens, la calcule pour les fonctions de référence, et utilise
son signe pour lire les variations d'une fonction. Tout ce qui suit est
...
Soit $f$ définie sur un intervalle $I$, et $a \neq b$ dans $I$. Le
\emph{taux de variation moyen}\index{taux de variation moyen} de $f$
entre $a$ et $b$ est
\[
\frac{f(b) - f(a)}{b - a}.
\]
C'est le coefficient directeur de la \emph{sécante} passant par les
points $\bigl(a, f(a)\bigr)$ et $\bigl(b, f(b)\bigr)$ de la courbe.
\end{definition}

\begin{example}
Une voiture parcourt $140$~km entre 14~h et 16~h~: sa vitesse moyenne est
...
la dérivée rend précise.
\end{example}

\section{La dérivée en un point}

\begin{definition}[Dérivée en un point]\label{def:g11:deriv:derivative}
Soit $f$ définie sur un intervalle contenant $a$. Pour $h \neq 0$, former
le taux de variation moyen entre $a$ et $a + h$~:
\[
...
$a$}\index{dérivée}~; ce nombre est la \emph{dérivée de $f$ en $a$},
notée $f'(a)$~:
\[
...
= 2 + h .
\]
Lorsque $h$ tend vers $0$, cela tend vers $2$~: la fonction $x^2$ est
dérivable en $1$ et $f'(1) = 2$. Noter la stratégie~: \emph{simplifier
...
n'est pas dérivable en $0$. Sa courbe a un \emph{coin} là.
\end{example}

\section{La tangente}

\begin{definition}[Tangente]\label{def:g11:deriv:tangent}
Si $f$ est dérivable en $a$, la \emph{tangente}\index{tangente} à la
courbe au point $A = \bigl(a, f(a)\bigr)$ est la droite passant par $A$
de coefficient directeur $f'(a)$. Son équation est
\[
y = f(a) + f'(a)(x - a).
\]
\end{definition}

La tangente est la position limite des sécantes par $A$~: lorsque
$h \to 0$, le second point $\bigl(a+h, f(a+h)\bigr)$ glisse le long de la
courbe vers $A$, et le coefficient directeur de la sécante
...
{\small La sécante par $A$ et $B = \bigl(a+h, f(a+h)\bigr)$ a pour
coefficient directeur $\frac{f(a+h)-f(a)}{h}$. Lorsque $B$ glisse vers
$A$, la sécante bascule dans la tangente, dont le coefficient directeur
est $f'(a)$.}
\end{omfigure}

\begin{example}\label{ex:g11:deriv:tangenteq}
Pour $f(x) = x^2$ en $a = 1$~: $f(1) = 1$ et $f'(1) = 2$
(\cref{ex:g11:deriv:atpoint}), donc la tangente est
\[
y = 1 + 2(x - 1) = 2x - 1 .
\]
...
$h \to 0$. Fixer $a$ dans l'ensemble de définition.
...
Soient $u$ et $v$ dérivables sur $I$ et $\lambda \in \R$. Alors
$u + v$, $\lambda u$, $uv$ sont dérivables sur $I$, de même que
$\frac{u}{v}$ là où $v \neq 0$, et
\[
(u+v)' = u' + v', \qquad
...
\begin{enumerate}
  \item Si $f' \geq 0$ sur $I$, alors $f$ est croissante sur $I$.
  \item Si $f' \leq 0$ sur $I$, alors $f$ est décroissante sur $I$.
  \item Si $f' > 0$ sur $I$ (sauf éventuellement en un nombre fini de
        points où elle s'annule), alors $f$ est strictement croissante
        sur $I$.
\end{enumerate}
\end{theorem}

\begin{proof}
L'intuition est claire --- une courbe dont toutes les tangentes pointent
...
Si $f'$ change de signe en $a$ de $+$ à $-$, alors $f$ a un maximum
local en $a$~; de $-$ à $+$, un minimum local. En un tel point,
$f'(a) = 0$ et la tangente est horizontale.
\end{proposition}

\begin{proof}
Si $f' \geq 0$ sur un intervalle se terminant en $a$ et $f' \leq 0$ sur
un intervalle commençant en $a$, alors $f$ croît jusqu'à $f(a)$ et
...
\begin{enumerate}
  \item Calculer $f'(x)$ et le simplifier --- idéalement sous forme
        factorisée.
  \item Déterminer le signe de $f'(x)$ sur chaque intervalle (pour un
        $f'$ du second degré, utiliser le \cref{thm:g11:quad:sign}).
  \item Enregistrer les résultats dans un \emph{tableau de variations}~:
...
$f'(x) = 3x^2 - 3 = 3(x-1)(x+1)$~: un trinôme de racines $-1$ et $1$,
positif à l'extérieur. Donc $f$ est croissante sur
$\intoc{-\infty}{-1}$, décroissante sur $\intcc{-1}{1}$, croissante sur
$\intco{1}{+\infty}$, avec un maximum local $f(-1) = -1 + 3 + 1 = 3$ et
un minimum local $f(1) = 1 - 3 + 1 = -1$.
\end{example}
...
(\cref{ex:g11:deriv:study})~: tangentes horizontales (orange) au
maximum local $(-1, 3)$ et au minimum local $(1, -1)$, où $f'$ change de
signe.}
\end{omfigure}
...
Donner l'équation de la tangente à la courbe de $f$ au point d'abscisse
$a$~:
\[
...
Étudier les variations de $f(x) = x^3 - 6x^2 + 9x - 2$ sur $\R$ (tableau
...
Soit $f(x) = x^2 - 4x + 5$.
\begin{enumerate}
  \item Trouver le point de la courbe où la tangente est horizontale.
  \item Trouver le ou les points où la tangente est parallèle à la
        droite $y = 2x + 1$.
\end{enumerate}
\end{exercise}
...
clôture (trois côtés). Exprimer l'aire en fonction de la largeur $x$, et
utiliser la dérivée pour trouver les dimensions d'aire maximale.
...
étudiant la fonction $f(x) = \frac{x+1}{2} - \sqrt x$ sur
$\intoo{0}{+\infty}$.
\end{exercise}
...
$\pi r^2 h = 500$. Exprimer $S$ en fonction de $r$ seul, et montrer que
...
Combien de tangentes à la parabole $y = x^2$ passent par le point
extérieur $P(1, -3)$~? Déterminer leurs équations. (Soit $a$ l'abscisse
du point de contact et exprimer que la tangente en $a$ passe par $P$.)
\end{exercise}

\begin{exercise}[$\star\star\star$]\label{exo:g11:deriv:12}
Soit $f(x) = x^3 - 3x + 1$ (voir l'\cref{ex:g11:deriv:study}). À l'aide
du tableau de variations, déterminer le nombre de solutions de
l'équation $f(x) = k$ selon la valeur du réel $k$.
\end{exercise}
