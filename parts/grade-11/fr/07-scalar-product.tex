...
Le produit scalaire attache un \emph{nombre} à une paire de vecteurs, et
...
derrière la loi des cosinus, les équations de cercles, et les calculs de
...
Le \emph{produit scalaire}\index{produit scalaire} de deux vecteurs
...
- \norm{\vec v}^2\right),
\]
où $\norm{\vec u}$ désigne la longueur (\emph{norme}) de $\vec u$.
\end{definition}

\begin{theorem}[Les quatre faces du produit scalaire]\label{thm:g11:scal:faces}
Soient $\vec u\,(x, y)$ et $\vec v\,(x', y')$ des vecteurs non nuls, et
$\theta$ l'angle entre eux. Alors
\begin{enumerate}
  \item \emph{(coordonnées)} $\vec u \cdot \vec v = xx' + yy'$~;
  \item \emph{(cosinus)} $\vec u \cdot \vec v =
        \norm{\vec u}\,\norm{\vec v}\cos\theta$~;
  \item \emph{(projection)} $\vec u \cdot \vec v = \vec u \cdot \vec v'$,
        où $\vec v'$ est la projection orthogonale de $\vec v$ sur la
  ...
  \item \emph{(normes)} la formule définissante de la
        \cref{def:g11:scal:dot}.
\end{enumerate}
\end{theorem}

\begin{proof}[Démonstration de 1 et 2]
\emph{1.} En coordonnées, $\norm{\vec u}^2 = x^2 + y^2$ (Pythagore) et
$\vec u + \vec v$ a pour coordonnées $(x + x', y + y')$, donc
\begin{align*}
2\,\vec u \cdot \vec v
...
\emph{2.} Choisir le repère pour que $\vec u$ pointe le long de l'axe des
$x$ positifs~: $\vec u\,(\norm{\vec u}, 0)$ et
$\vec v\,(\norm{\vec v}\cos\theta,\ \norm{\vec v}\sin\theta)$
...
$\vec u \cdot \vec v = 6 - 4 = 2$. Les normes sont
...
Pour tous vecteurs et tout $\lambda \in \R$~:
\[
...
\begin{proof}
Tout suit de la formule en coordonnées~: par exemple
...
en utilisant les trois premières règles --- le produit scalaire se
comporte exactement comme un produit ordinaire, donc les identités
algébriques usuelles s'appliquent aux vecteurs.
\end{proof}

\section{Orthogonalité}

\begin{definition}[Vecteurs orthogonaux]\label{def:g11:scal:orthogonal}
Deux vecteurs sont \emph{orthogonaux}\index{vecteurs orthogonaux}, noté
$\vec u \perp \vec v$, si $\vec u \cdot \vec v = 0$.
\end{definition}

\begin{remark}
Pour des vecteurs non nuls, la formule~2 du \cref{thm:g11:scal:faces}
montre $\vec u \cdot \vec v = 0 \iff \cos\theta = 0 \iff$ les directions
sont perpendiculaires. Le vecteur nul est orthogonal à tout.
\end{remark}

\begin{example}
$\vec u\,(3, 2)$ et $\vec v\,(-4, 6)$~: $\vec u \cdot \vec v = -12 + 12 =
0$~: orthogonaux. En général, $\vec u\,(a, b)$ est toujours orthogonal à
...
développer avec la \cref{prop:g11:scal:rules}~:
\[
a^2 = \norm{\vect{BC}}^2
= \norm{\vect{AC}}^2 - 2\,\vect{AC}\cdot\vect{AB} + \norm{\vect{AB}}^2
= b^2 + c^2 - 2\,\vect{AB}\cdot\vect{AC}.
\]
Par la formule du cosinus,
...
soit $O$ le milieu de $[AB]$ et $r = \frac{AB}{2}$. Alors
...
Le vecteur $\vec n\,(a, b)$ est orthogonal à tout vecteur directeur de la
...
exactement lorsque leurs vecteurs normaux (ou directeurs) sont
orthogonaux.
\end{proposition}
...
vecteur directeur de $d$ est $(1, 3)$, qui sert de vecteur
\emph{normal} de la perpendiculaire. Équation~: $x + 3y + c = 0$ avec
...
Pour quelle valeur de $t$ les vecteurs $\vec u\,(3, t)$ et
$\vec v\,(t - 8, 1)$ sont-ils orthogonaux~? Et pour quelle(s) valeur(s)
de $t$ le vecteur $\vec u\,(t, 2)$ est-il orthogonal à lui-même moins
...
Donner une équation du cercle de centre $\Omega(-2, 3)$ et de rayon $5$.
...
\cref{prop:g11:scal:diameter}, trouver une équation du cercle de
diamètre $[AB]$, et vérifier que l'origine $O(0,0)$ y est.
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:g11:scal:9}
Trouver une équation de la droite passant par $P(1, 3)$ perpendiculaire
à $d : 2x + y - 7 = 0$, et les coordonnées du pied $H$ de la
perpendiculaire (l'intersection des deux droites). En déduire la
distance de $P$ à $d$.
\end{exercise}
...
Soient $A$ et $B$ deux points avec $AB = 4$, et $I$ le milieu de
$[AB]$.
\begin{enumerate}
