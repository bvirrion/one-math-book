...
\section{Problème~: les deux cadeaux du produit scalaire}

\begin{problem}[{Devoir du week-end --- les formules d'addition
tombent d'un produit scalaire, la distance à une droite d'un autre,
et les trois hauteurs se rencontrent en prime}]
\label{pb:g11:scal:1}
Le \omterm{def:g11:scal:dot}{produit scalaire} a des airs de comptabilité --- on multiplie, on
additionne --- et pourtant il livre, presque gratuitement, deux
trésors qui ont résisté à des siècles de chasse aux angles~: les
\emph{formules d'addition} de la trigonométrie (calculer
$\cos 15^\circ$ exactement~!) et la \emph{distance d'un point à une
droite} en une seule fraction bien nette. En cadeau d'adieu, il
démontre que les trois hauteurs de tout triangle passent par un même
point. Les quatre visages du \cref{thm:g11:scal:faces} seront tous
appelés au service.

\medskip
\noindent\textbf{Partie I --- Aisance.}
\begin{enumerate}
  \item Pour $\vec u(3, 4)$ et $\vec v(-1, 2)$~: calculer
        $\vec u \cdot \vec v$, les deux \omterm{def:g11:scal:dot}{normes} et l'angle entre les
        \omterm{def:g11:vect:vector}{vecteurs} (au dixième de degré).
  \item Déterminer $k$ pour que $(2, k)$ et $(3, -4)$ soient
        \omterm{def:g11:scal:orthogonal}{orthogonaux} (\cref{def:g11:scal:orthogonal}).
  \item Visage physique~: on tire une luge sur $10$ m avec une force
        de $50$ N faisant un angle de $60^\circ$ avec le sol.
        Calculer le travail $W = \vec F \cdot \vec d$.
  \item Loi des \omterm{def:g11:trigo:cossin}{cosinus} (\cref{thm:g11:scal:cosines})~: deux côtés
        de longueurs $5$ et $7$ encadrent un angle de $60^\circ$~;
        déterminer exactement le troisième côté.
  \item Réciproquement~: un triangle a pour côtés $4$, $6$, $8$.
        Calculer le \omterm{def:g11:trigo:cossin}{cosinus} de l'angle opposé au côté de longueur
        $8$, classer cet angle, et le donner au dixième de degré.
\end{enumerate}

\medskip
\noindent\textbf{Partie II --- Premier cadeau~: les formules
d'addition.}
Soient $\vec u = (\cos a, \sin a)$ et $\vec v = (\cos b, \sin b)$~:
deux \omterm{def:g11:vect:vector}{vecteurs} unitaires d'angles $a$ et $b$.
\begin{enumerate}[resume]
  \item Calculer $\vec u \cdot \vec v$ de deux manières --- une fois
        avec les \omterm{def:g10:coordgeom:system}{coordonnées}, une fois avec les \omterm{def:g11:scal:dot}{normes} et l'angle
        entre les \omterm{def:g11:vect:vector}{vecteurs} --- et en déduire la formule mère~:
        \[
        \cos(a - b) = \cos a \cos b + \sin a \sin b .
        \]
  \item Remplacer $b$ par $-b$ (parités du \omterm{def:g11:trigo:cossin}{sinus} et du \omterm{def:g11:trigo:cossin}{cosinus}~!)
        pour obtenir $\cos(a + b)$.
  \item Obtenir $\sin(a + b)$ à partir de l'identité des angles
        \omterm{def:g10:proba:operations}{complémentaires} $\sin x = \cos\left(\frac\pi2 - x\right)$
        appliquée à $x = a + b$.
  \item Enfin des valeurs exactes~: calculer $\cos 15^\circ$ (sous
        la forme $\cos(45^\circ - 30^\circ)$) et $\sin 75^\circ$.
        Vérifier les deux numériquement.
  \item Poser $b = a$ pour établir les formules de duplication~:
        $\cos 2a$ (sous ses trois formes) et $\sin 2a$.
  \item À partir de $\cos 2a = 1 - 2\sin^2 a$, calculer
        $\sin^2 15^\circ$, puis $\sin 15^\circ$ sous forme de radical
        emboîté --- et vérifier que son carré s'accorde avec la
        valeur $\frac{\sqrt6 - \sqrt2}{4}$ que donne la méthode de
        la question 9.
\end{enumerate}

\medskip
\noindent\textbf{Partie III --- Deuxième cadeau~: la distance à une
droite.}
\begin{enumerate}[resume]
  \item Soit $d$ la droite $ax + by + c = 0$, de \omterm{def:g11:vect:vector}{vecteur} normal
        $\vec n(a, b)$ (\cref{prop:g11:scal:normal}), et soit
        $P(x_0, y_0)$ un point. En prenant un point $A$ quelconque
        de $d$, expliquer pourquoi la distance de $P$ à $d$ vaut
        ...
        \]
        Calculer la distance de $P(5, 6)$ à $3x + 4y - 12 = 0$.
  \item Écrire l'\omterm{def:g10:algebra:equation}{équation} du cercle de centre $(5, 6)$ tangent à
        cette droite.
  \item Recouper la formule~: calculer la distance de l'origine à
        $3x + 4y - 12 = 0$, puis la vérifier en calculant
        explicitement le pied de la perpendiculaire issue de
        l'origine.
  \item Triangle $A(0,0)$, $B(6,0)$, $C(2,5)$~: calculer son aire en
        prenant $[AB]$ pour base~; puis calculer la distance de $B$
        à la droite $(AC)$ et vérifier qu'elle redonne la même aire.
  \item Le cercle de centre $(2, 1)$ et de rayon $2$, face à la
        droite $x + y - 6 = 0$~: calculer la distance du centre à la
        droite et classer leur position relative (deux points
        communs, un seul, ou aucun).
\end{enumerate}

\medskip
\noindent\textbf{Partie IV --- La prime~: les hauteurs sont
concourantes.}
\begin{enumerate}[resume]
  \item Généraliser l'\cref{exo:g11:scal:11}~: pour deux points $A$
        et $B$ de \omterm{prop:g10:coordgeom:midpoint}{milieu} $I$, démontrer que
        \[
        \vect{MA} \cdot \vect{MB} = MI^2 - \frac{AB^2}{4}
        \quad\text{pour tout point } M .
        \]
  \item En déduire la \cref{prop:g11:scal:diameter} en une ligne~:
        l'ensemble des points $M$ tels que
        $\vect{MA} \cdot \vect{MB} = 0$ est le cercle de diamètre
        $[AB]$.
  \item Les hauteurs~: soit $H$ le point d'\omterm{def:g10:numbers:interunion}{intersection} des hauteurs
        issues de $A$ et de $B$ dans un triangle $ABC$ (de sorte que
        $\vect{HA} \cdot \vect{BC} = 0$ et
        $\vect{HB} \cdot \vect{CA} = 0$). Démontrer l'identité
        \[
        \vect{HA} \cdot \vect{BC} + \vect{HB} \cdot \vect{CA}
        + \vect{HC} \cdot \vect{AB} = 0
        \]
        --- tout \omterm{def:g10:algebra:expand}{développer} à partir de $H$ avec Chasles --- et en
        conclure que $H$ appartient aussi à la hauteur issue de
        $C$~: les trois hauteurs sont concourantes (en
        l'\emph{orthocentre}\index{orthocentre}).
  \item Pour finir~: quatre visages, trois trophées. Quel visage du
        \cref{thm:g11:scal:faces} a porté les formules d'addition,
        lequel la formule de distance, lequel l'\omterm{pb:g11:scal:1}{orthocentre} --- et
        pourquoi «~calculer deux fois la même quantité~» est-elle la
        devise de tout le chapitre~?
\end{enumerate}
\end{problem}
