...
La probabilité modélise des expériences dont le résultat est incertain~:
...
central de toute la théorie des probabilités~: la \emph{variable
aléatoire} et son \emph{espérance}. Ces notions sont reprises et
approfondies dans le \cref{ch:g12:randvar}, et les probabilités
...
(l'ensemble des issues) muni d'une \emph{probabilité} $\P$ attribuant à
...
L'événement $S$ = «~la somme est $7$~» contient les $6$ couples
$(1,6), (2,5), (3,4), (4,3), (5,2), (6,1)$, donc
$\P(S) = \frac{6}{36} = \frac16$. L'événement «~la somme est $12$~»
contient seulement $(6,6)$~: probabilité $\frac{1}{36}$. Choisir le
\emph{bon} univers --- couples ordonnés, pas non ordonnés --- est ce qui
rend le modèle uniforme applicable.
\end{example}

\begin{proposition}[Règles de la probabilité]\label{prop:g11:prob:rules}
Pour des événements $A, B \subseteq \Omega$~:
\[
...
\]
où $\bar A$ est le \emph{complémentaire} de $A$ (toutes les issues hors
de $A$). En particulier, $\P(A \cup B) = \P(A) + \P(B)$ lorsque $A$ et
$B$ sont \emph{incompatibles} ($A \cap B = \emptyset$).
\end{proposition}

\begin{proof}
L'événement vide ne contient aucune issue (somme vide), et $\Omega$ les
contient toutes (somme totale $1$). Chaque issue est dans exactement
l'un de $A$, $\bar A$, donc $\P(A) + \P(\bar A) = 1$. Pour la réunion~:
...
$20\,\%$ ne font ni l'un ni l'autre (complémentaire).
\end{example}
...
une fonction $X$ attribuant un nombre à chaque issue. Sa
\emph{loi}\index{loi} est le tableau de ses valeurs possibles
$x_1, \dots, x_k$ avec leurs probabilités
\[
...
$G = -2$~; l'issue $5$ donne $G = 3$~; l'issue $6$ donne $G = 4$. La loi
de $G$~:
\begin{center}
\begin{tabular}{c|ccc}
$g$ & $-2$ & $3$ & $4$\\
...
issues qui la produisent et additionner leurs probabilités~; vérifier
que la ligne des probabilités somme à $1$.
\end{method}

\section{Espérance}

\begin{definition}[Espérance]\label{def:g11:prob:expectation}
L'\emph{espérance}\index{espérance} (ou moyenne) de $X$ est la moyenne de
ses valeurs pondérées par leurs probabilités~:
\[
...
\begin{remark}
$\E(X)$ est ce que serait la moyenne d'un grand nombre de répétitions
...
l'analogie formelle avec la moyenne d'un jeu de données avec effectifs
(\cref{def:g11:stat:mean})~: les probabilités jouent le rôle des
fréquences relatives $\frac{n_i}{n}$.
\end{remark}
...
\]
En moyenne le joueur perd environ $17$ centimes par partie~: le jeu est
...
\begin{proof}
La variable $aX + b$ prend la valeur $a x_i + b$ avec la probabilité
$\P(X = x_i)$, donc
\[
...
valeurs autour de l'espérance $m = \E(X)$~:
\[
...
= \E(X^2) - 2m^2 + m^2 ,
\]
qui est $\E(X^2) - m^2$. C'est la version variable aléatoire de
...
\begin{proof}
Par linéarité, $aX + b$ s'écarte de son espérance $a m + b$ de
$(aX + b) - (am + b) = a(X - m)$~: chaque écart est multiplié par $a$,
donc chaque écart au carré par $a^2$, et leur moyenne pondérée aussi. Le
...
\sigma(G) \approx 2.6 .
\]
La perte de $0.17$ par partie en moyenne s'accompagne de fluctuations de
taille typique $2.6$ euros --- l'espérance sans l'écart type ne raconte
...
{\small La loi du gain $G$ de l'\cref{ex:g11:prob:game}, et son espérance
...
probabilité de tirer~: un cœur~; un roi~; un cœur ou un roi~; ni un cœur
ni un roi.
\end{exercise}
...
$X$ satisfait $\E(X) = 3$ et $\V(X) = 4$. Donner l'espérance, la variance
et l'écart type de $Y = 2X - 5$.
\end{exercise}
...
Donner la loi du gain $G$ (gains moins mise), calculer $\E(G)$, et
...
Construire la loi de $X$ (énumérer les $20$ tirages ordonnés, ou
utiliser un arbre), et calculer $\E(X)$.
\end{exercise}
...
probabilité $0.05$ le client réclame $1000$ euros, avec probabilité
$0.01$ il réclame $5000$ euros, sinon rien. Soit $B$ le bénéfice de la
compagnie sur une police. Donner la loi de $B$, calculer $\E(B)$, et
interpréter.
\end{exercise}
...
$25$ euros.
\begin{enumerate}
  \item Donner la loi du revenu total de l'organisateur moins les lots,
