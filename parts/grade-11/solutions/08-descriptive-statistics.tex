...
$n = 8$, sum $71$: mean $\bar x = \frac{71}{8} = 8.875$. Median: midpoint
of the $4$th and $5$th values, $\frac{8 + 9}{2} = 8.5$. Quartiles: a
quarter of $8$ is $2$, so $Q_1$ is the $2$nd value, $7$; three quarters is
$6$, so $Q_3$ is the $6$th value, $10$. Interquartile range:
$10 - 7 = 3$.
\end{solution}
...
= \frac{181}{50} = 3.62 .
\]
Median: the cumulative frequencies are $6, 15, 23, 33, \dots$, so the
$25$th and $26$th sorted outcomes are both $4$: the median is $4$.
\end{solution}
...
The deviations of $6, 8, 10, 12, 14$ from the mean $10$ are
$-4, -2, 0, 2, 4$, with squares summing to $40$:
$V = \frac{40}{5} = 8$ and $\sigma = 2\sqrt2 \approx 2.83$.
\end{solution}

\begin{solution}{exo:g11:stat:6}
By \cref{prop:g11:stat:affine} with $a = \frac12$, $b = 4$: new mean
$\frac{12.4}{2} + 4 = 10.2$; new standard deviation
$\frac{2.5}{2} = 1.25$.
\end{solution}

\begin{solution}{exo:g11:stat:7}
The pooled mean is the \emph{weighted} average
\[
\frac{20 \times 10 + 30 \times 15}{50} = \frac{650}{50} = 13,
\]
not $12.5$: group B contributes more values, so it pulls the pooled mean
towards its own.
\end{solution}

\begin{solution}{exo:g11:stat:8}
Both lines are nearly on target on average ($1.002$ and $1.001$ kg), but
line 2's standard deviation is three times larger: its bags vary much
more, producing far more noticeably under- and over-filled bags. Line 2
should be dealt with first --- consistency, not just the mean, is what
needs fixing.
\end{solution}
...
\emph{1.} With $23$: sum $50$, mean $5$; median = midpoint of $5$th and
$6$th values $= 3$. Without $23$: sum $27$, mean $3$; median (5th of 9
values) $= 3$.

\emph{2.} With $23$ ($n = 10$): $Q_1$ is the $3$rd value ($2$), $Q_3$ the
$8$th ($4$): interquartile range $2$. Without ($n = 9$): $Q_1$ is the
$3$rd value ($2$), $Q_3$ the $7$th ($4$): still $2$.

\emph{3.} The single outlier drags the mean from $3$ up to $5$ but leaves
the median and the interquartile range unchanged: those two are
\emph{robust}. When a dataset may contain aberrant values, report the
median and interquartile range rather than the mean and standard
deviation.
\end{solution}
