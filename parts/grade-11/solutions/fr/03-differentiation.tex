...
$f'(2) = 1$~; tangente $y = -2 + (x - 2) = x - 4$.

$f(x) = \frac1x$, $a = 1$~: $f(1) = 1$, $f'(1) = -1$~; tangente
$y = 1 - (x - 1) = 2 - x$.

$f(x) = \sqrt x$, $a = 4$~: $f(4) = 2$, $f'(4) = \frac{1}{2\sqrt4} =
\frac14$~; tangente $y = 2 + \frac14(x - 4) = \frac{x}{4} + 1$.
\end{solution}
...
croissante sur $\intoc{-\infty}{1}$, décroissante sur $\intcc{1}{3}$,
croissante sur $\intco{3}{+\infty}$, avec un maximum local
$f(1) = 1 - 6 + 9 - 2 = 2$ et un minimum local
$f(3) = 27 - 54 + 27 - 2 = -2$.
\end{solution}
...
$f'(x)$ a le signe de $x - 1$~: $f$ est décroissante sur
$\intoc{-1}{1}$ et croissante sur $\intco{1}{+\infty}$, avec minimum
$f(1) = \frac{4}{2} = 2$.
\end{solution}
...
\emph{1.} Tangente horizontale~: $f'(x) = 0$ en $x = 2$~; le point est
$(2, f(2)) = (2, 1)$ (le sommet de la parabole).
...
sur $\intoo{1}{+\infty}$. Donc $f$ décroît puis croît, avec minimum
...
négatif avant cette valeur et positif après, c'est un minimum.
\end{solution}

\begin{solution}{exo:g11:deriv:11}
La tangente à $y = x^2$ au point d'abscisse $a$ est
$y = a^2 + 2a(x - a) = 2ax - a^2$. Elle passe par $P(1, -3)$ lorsque
$-3 = 2a - a^2$, c'est-à-dire $a^2 - 2a - 3 = 0$, dont les racines sont
$a = 3$ et $a = -1$ ($\Delta = 16$). Il y a donc \emph{deux} tangentes
par $P$~:
\[
y = 6x - 9 \quad (a = 3),
\qquad
...
D'après l'\cref{ex:g11:deriv:study}, $f$ croît jusqu'au maximum local
$f(-1) = 3$, décroît jusqu'au minimum local $f(1) = -1$, puis croît à
...
droites horizontales $y = k$ sur le tableau de variations~:
\begin{itemize}
