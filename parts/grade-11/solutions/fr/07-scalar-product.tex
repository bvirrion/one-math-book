...
$(1,-4)\cdot(8,2) = 8 - 8 = 0$ (vecteurs orthogonaux).
...
Les vecteurs $\vect{AB}$ et $\vect{BC}$ font un angle de
...
orthogonaux, donc $O$ voit $[AB]$ sous un angle droit. (En complétant les
carrés~: centre $\left(-\frac32, 1\right)$, rayon
$\frac{\sqrt{13}}{2}$.)
\end{solution}

\begin{solution}{exo:g11:scal:9}
La perpendiculaire par $P$ a le vecteur directeur de $d$, à savoir
$(-1, 2)$, comme vecteur normal~: équation $-x + 2y + c = 0$~; par
$P(1,3)$~: $-1 + 6 + c = 0$, $c = -5$, donc $x - 2y + 5 = 0$.

Intersection avec $d$~: de $2x + y = 7$, $y = 7 - 2x$~; en substituant,
...
\emph{1.} Comme $I$ est le milieu, $\vect{IB} = -\vect{IA}$ et
$IA = 2$. Alors
\[
