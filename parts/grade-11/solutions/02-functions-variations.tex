...
$f$: need $3 - x \geq 0$, domain $\intoc{-\infty}{3}$.
...
$\sqrt{11} < \sqrt{13}$ since $\sqrt{}$ is increasing and $11 < 13$.

$(-2.1)^2 > (-2.05)^2$ since $x^2$ is decreasing on
$\intoc{-\infty}{0}$ and $-2.1 < -2.05$.

$\frac{1}{0.9} > \frac{1}{1.1}$ since $\frac1x$ is decreasing on
$\intoo{0}{+\infty}$ and $0.9 < 1.1$.

$(-1.01)^3 < (-0.99)^3$ since $x^3$ is increasing and $-1.01 < -0.99$.
\end{solution}
...
$-h(1)$: neither even nor odd.

$k(-x) = \frac{-x}{x^2+1} = -k(x)$: odd.
\end{solution}

\begin{solution}{exo:g11:func:4}
Read the variation table. On $\intcc{-3}{1}$, $f$ climbs from $-2$ to $4$
and crosses the value $1$ exactly once; on $\intcc{1}{5}$ it goes down
from $4$ to $0$ and crosses $1$ once more: the equation $f(x) = 1$ has
$2$ solutions. The maximum of $f$ is $f(1) = 4 < 5$, so $f(x) = 5$ has no
solution.
\end{solution}
...
$u \geq 2$ and $v > 2$. Hence $f(v) > f(u)$: $f$ is strictly increasing
on $\intco{2}{+\infty}$.
\end{solution}

\begin{solution}{exo:g11:func:6}
$f = \sqrt{u}$ with $u(x) = 5 - x$ decreasing and nonnegative on
$\intoc{-\infty}{5}$: $f$ is decreasing there
(\cref{prop:g11:func:transforms}).

$g = \frac1u$ with $u(x) = x^2 + 1$ increasing and positive on
$\intco{0}{+\infty}$: $g$ is decreasing there.

$h = 3 - 2\sqrt x$: $\sqrt x$ is increasing, multiplying by $-2 < 0$
reverses, adding $3$ changes nothing: $h$ is decreasing on
$\intco{0}{+\infty}$.
\end{solution}

\begin{solution}{exo:g11:func:7}
\emph{1.} $f$ is in canonical form: decreasing on $\intoc{-\infty}{2}$,
increasing on $\intco{2}{+\infty}$, minimum $f(2) = -3$.

\emph{2.} $g = \frac{1}{u}$ with $u(x) = (x-2)^2 + 1 = f(x) + 4$. The
shift by $4$ keeps the variations of $f$, and $u \geq 1 > 0$. Taking the
reciprocal reverses them: $g$ is increasing on $\intoc{-\infty}{2}$ and
decreasing on $\intco{2}{+\infty}$, with maximum
$g(2) = \frac{1}{1} = 1$.
\end{solution}
...
On $\intoo{-1}{+\infty}$, $u(x) = x + 1$ is increasing and positive, so
$\frac1u$ is decreasing, $-\frac1u$ is increasing, and adding $2$ changes
nothing: $f$ is strictly increasing on $\intoo{-1}{+\infty}$.
\end{solution}
...
\emph{2. False.} $f(x) = g(x) = x$ are increasing on $\R$, but their
product $x^2$ is not increasing on $\R$ (it decreases on
$\intoc{-\infty}{0}$). (The statement becomes true if both functions are
also nonnegative.)

\emph{3. False.} Same counterexample: $f(x) = x$ is increasing on $\R$
...
$uv < 1$, so $uv - 1 < 0$ and $f(v) < f(u)$: $f$ is strictly decreasing
on $\intoc{0}{1}$. If $1 \leq u < v$, then $uv > 1$ and $f(v) > f(u)$:
$f$ is strictly increasing on $\intco{1}{+\infty}$.
