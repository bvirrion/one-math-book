...
\emph{unit circle}, where cosine and sine become functions of an arbitrary
real number. This chapter installs the radian, the winding of the real
...
$\cos$ and $\sin$ as functions --- variations, derivatives --- is carried
...
Degrees and radians are proportional: multiply by $\frac{\pi}{180}$ to go
from degrees to radians, by $\frac{180}{\pi}$ the other way. For instance
...
To each real number $t$, associate the point $M(t)$ of $\mathcal C$
...
\emph{sine}\index{sine} of $t$ are the coordinates of the point $M(t)$ of
the unit circle:
\[
M(t) = (\cos t,\ \sin t).
\]
...
{\small The unit circle: the real number $t$, wound counterclockwise from
$I$, lands at the point $M(t)$ whose coordinates \emph{define} $\cos t$
and $\sin t$.}
\end{omfigure}
...
\begin{proof}
$M(t)$ lies on the unit circle, so its coordinates are between $-1$ and
$1$, and they satisfy the circle's equation $x^2 + y^2 = 1$, which is the
...
$\cos t = \pm\frac45$; in the second quadrant the abscissa is negative,
hence $\cos t = -\frac45$.
\end{example}
...
\begin{remark}
A memory aid: the sines read
$\frac{\sqrt0}{2}, \frac{\sqrt1}{2}, \frac{\sqrt2}{2}, \frac{\sqrt3}{2},
\frac{\sqrt4}{2}$, and the cosines are the same list reversed.
\end{remark}
...
$M(-t)$ is the reflection of $M(t)$ in the $x$-axis: the abscissa is
unchanged, the ordinate changes sign.

$M(\pi - t)$ is the reflection of $M(t)$ in the $y$-axis: walking
backwards from $\pi$ by $t$ lands opposite (horizontally) to walking
forwards from $0$ by $t$. The abscissa changes sign, the ordinate is
unchanged.
...
diagonal line $y = x$, which swaps the two coordinates.
\end{proof}
...
\begin{proof}
Two points of the unit circle have the same abscissa exactly when they are
...
$t = \pm a$ up to a multiple of $2\pi$. Similarly, equal ordinates mean
...
\begin{enumerate}
  ...
  \item Choose the integers $k$ that land inside the requested interval,
...
Convert to radians: $30^\circ$, $45^\circ$, $120^\circ$, $270^\circ$.
...
Solve on $\R$ the equation $\cos 2t = \cos t$. (Use
...
radians, then in degrees) does it turn when the bike advances by $1.5$ m?
...
abscissa is at most $\frac12$, and read off the interval of values of
$t$.)
\end{exercise}
