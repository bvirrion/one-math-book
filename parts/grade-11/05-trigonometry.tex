...
\section{Problem: The Ferris wheel and the tide}

\begin{problem}[{Weekend problem --- radians as rolled distance,
and the unit circle as the master clock of every periodic
phenomenon}]
\label{pb:g11:trigo:1}
A Ferris wheel carries you around a circle; the tide carries the
harbor's water up and down the same mathematical circle. Every
periodic phenomenon --- wheels, tides, sound, seasons --- reads
its schedule off the unit circle of this chapter
(\cref{def:g11:trigo:cossin}), and the \omterm{def:g10:algebra:equation}{equations} of
\cref{met:g11:trigo:solve} tell the exact times. This problem
converts, winds, models and solves --- and settles on the way why
mathematicians measure angles in \omterm{def:g11:trigo:radian}{radians}.

\medskip
\noindent\textbf{Part I --- \omterm{def:g11:trigo:radian}{Radians}, the rolled distance.}
\begin{enumerate}
  \item Convert to \omterm{def:g11:trigo:radian}{radians}: $30^\circ$, $135^\circ$,
        $300^\circ$; and to degrees: $\frac{3\pi}{4}$,
        $\frac{5\pi}{6}$ (\cref{met:g11:trigo:convert}).
  \item The whole point of \omterm{def:g11:trigo:radian}{radians}: an arc of angle $t$ (in
        \omterm{def:g11:trigo:radian}{radians}) on a circle of radius $r$ has length exactly
        $r\,t$. What arc does an angle of $2$ \omterm{def:g11:trigo:radian}{radians} cut on a
        circle of radius $3$ m? (No $\pi$ anywhere --- that is
        the point.)
  \item A wheel of radius $30$ cm rolls $1$ km without
        slipping. Through how many \emph{\omterm{def:g11:trigo:radian}{radians}} has it turned,
        and how many full turns is that?
  \item Place on the unit circle and evaluate exactly:
        $\cos\frac{2\pi}{3}$, $\sin\left(-\frac{\pi}{4}\right)$,
        and $\cos\frac{19\pi}{6}$ (reduce modulo $2\pi$ first;
        \cref{prop:g11:trigo:associated}).
  \item An angle $t \in \intoo{\frac\pi2}{\pi}$ satisfies
        $\sin t = \frac35$. Using
        $\cos^2 t + \sin^2 t = 1$ and the quadrant, find
        $\cos t$ and $\tan t$.
\end{enumerate}

\medskip
\noindent\textbf{Part II --- The Ferris wheel.}
A giant wheel has radius $60$ m, its hub $65$ m above the
ground, and turns once every $30$ minutes. You board at the
lowest point at time $t = 0$ (in minutes), so your height obeys
\[
h(t) = 65 - 60 \cos\!\left(\frac{\pi t}{15}\right).
\]
\begin{enumerate}[resume]
  \item Justify the formula: check the angle turned after $t$
        minutes, the height at $t = 0$, and explain the minus
        sign.
  \item Compute your height at $t = 7.5$, $t = 15$ and
        $t = 20$ minutes (exact values).
  \item At what time are you \emph{first} at $95$ m? (Solve
        $h(t) = 95$ with \cref{met:g11:trigo:solve}.)
  \item During which time \omterm{def:g10:numbers:interval}{interval} of the ride are you at least
        $95$ m high --- and for how long in total? (Compare
        \cref{exo:g11:trigo:11}.)
  \item Where does the height change \emph{fastest}? Compare the
        climb during the first minute
        ($h(1) - h(0)$) with the climb between minutes $7$ and
        $8$, and formulate the observation (the tool that makes
        it exact --- the \omterm{def:g11:deriv:derivative}{derivative} of the sinusoid --- arrives
        in grade 12).
  \item Engineer's turn: design the height formula of a wheel
        with maximal height $40$ m, minimal height $4$ m and
        period $12$ minutes, boarding at the bottom at
        $t = 0$.
\end{enumerate}
...
symmetries.}
In a harbor, the water depth in meters, $t$ hours after
midnight, is
\[
d(t) = 7 + 3 \sin\!\left(\frac{\pi t}{6}\right).
\]
\begin{enumerate}[resume]
  \item Give the maximal and minimal depths, the times at which
        they first occur, and the period of the tide.
  \item A cargo ship needs $8.5$ m of water. Solve
        $d(t) \geq 8.5$ on $\intco{0}{12}$: during which window
        can it enter, and how long is the window? When does the
        next window open?
  \item Re-derive two associated-angle identities directly from
        circle symmetries --- $\sin(\pi - t) = \sin t$ (mirror
        across the vertical axis) and $\cos(\pi + t) = -\cos t$
        (half-turn) --- and state the complementary identity
        $\cos\left(\frac\pi2 - t\right) = \sin t$.
  \item Parity (\cref{pb:g11:func:1} vocabulary): classify
        $\sin$, $\cos$, and $t \mapsto \sin t + t^3$ as even or
        odd, with one-line proofs from the circle.
  \item Solve on $\intco{0}{2\pi}$:
        $2\sin^2 t - \sin t - 1 = 0$. (A quadratic in
        disguise: factor it as in \cref{pb:g11:quad:1}, then
        read the circle.)
\end{enumerate}
...
\begin{enumerate}[resume]
  \item How many degrees is $1$ \omterm{def:g11:trigo:radian}{radian}? Then a calculator trap:
        which is larger, $\sin(1)$ (\omterm{def:g11:trigo:radian}{radian} mode) or
        $\sin(1^\circ)$ --- and by roughly what factor? Moral
        for calculator settings?
  \item Solve $\cos t = \sin t$ on $\intco{0}{2\pi}$, using the
        circle (where are \omterm{def:g10:coordgeom:system}{abscissa} and \omterm{def:g10:coordgeom:system}{ordinate} equal?).
  \item For small angles, the arc and its \omterm{def:g11:trigo:cossin}{sine} almost coincide:
        compare $\sin(0.1)$ with $0.1$ (five decimals), and
        give the relative error. This ``small-angle
        \omterm{def:g10:numbers:approx}{approximation}'' steers ships and pendulums; the theorem
        behind it ($\frac{\sin t}{t} \to 1$) is a star of the
        grade-12 limits chapter.
  \item Finale --- the unit circle as master clock, one sentence
        per item: \omterm{def:g11:trigo:radian}{radians} turn angles into honest numbers (arc
        $=$ radius $\times$ angle); \omterm{def:g11:trigo:cossin}{cosine} and \omterm{def:g11:trigo:cossin}{sine} are the
        clock hand's shadow and height; the circle's symmetries
        are the identities; solving $\cos t = c$ reads times
        off the clock; and every periodic phenomenon --- wheel,
        tide, sound --- is this clock wearing a costume.
\end{enumerate}
\end{problem}
