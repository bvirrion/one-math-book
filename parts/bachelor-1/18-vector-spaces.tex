...
Linear algebra begins here: the axioms of \omterm{def:b1:vspaces:def}{vector spaces} isolate what
$\R^2$, $\R^3$, \omterm{def:b1:poly:def}{polynomial} spaces and function spaces have in common
--- one can add, and scale. Two chapters build the theory
(\cref{ch:b1:findim} adds dimension); the language they \omterm{def:b1:logic:sets}{set} up ---
\omterm{def:b1:vspaces:span}{span}, \omterm{def:b1:vspaces:free}{free family}, \omterm{def:b1:vspaces:free}{basis}, \omterm{def:b1:vspaces:sum}{direct sum} --- is the daily bread of every
...
A \emph{$K$-vector space}\index{vector space} is a \omterm{def:b1:logic:sets}{set} $E$ with an
addition making $(E, +)$ an \omterm{def:b1:structures:group}{abelian group} (zero written $0_E$ or
...
cancel $0x$ in the \omterm{def:b1:structures:group}{group} $(E, +)$. For $\lambda\,0_E$: same
...
scalars form a \omterm{def:b1:structures:field}{field}) and use the two axioms
...
follows --- and the last one is exactly where \omterm{def:b1:structures:field}{fields} are needed:
over the scalars $\Z$, the ``space'' $\Z/2\Z$ would violate it
with $2\,x = 0$.
\end{proof}

\begin{example}\label{ex:b1:vspaces:examples}
$K^n$ (coordinatewise operations); the \omterm{def:b1:poly:def}{polynomials} $K[X]$; the
functions $\mathcal{F}(A, K)$ from any \omterm{def:b1:logic:sets}{set} $A$ to $K$ (pointwise
operations) --- containing \omterm{def:b1:continuity:continuous}{continuous} functions, sequences
...
x + \lambda y \in F .
\]
A subspace is itself a \omterm{def:b1:vspaces:def}{vector space}. Any intersection of subspaces
is a subspace; a union almost never is (same proof as
\cref{exo:b1:structures:6}).
\end{definition}

\begin{example}\label{ex:b1:vspaces:subspaces}
In $\mathcal{F}(\R, \R)$: the \omterm{def:b1:continuity:continuous}{continuous} functions, the
\omterm{def:b1:derivative:def}{differentiable} ones, the \omterm{def:b1:poly:def}{polynomials} of degree $\leq n$ (written
...
\begin{itemize}
  \item $\{u : u \text{ bounded}\}$ \emph{is} a \omterm{def:b1:vspaces:subspace}{subspace}: $0$ is
  ...
  \item $\{u : u \text{ monotone}\}$ is \emph{not}: $u_n = n$
        ...
        that fails, even though the \omterm{def:b1:logic:sets}{set} contains $0$ and all
...
($\lambda_i \in K$). The \omterm{def:b1:logic:sets}{set} of all of them is the
\emph{span}\index{span} $\operatorname{Vect}(x_1, \dots, x_p)$: it
is a \omterm{def:b1:vspaces:subspace}{subspace}, the smallest one containing the family.
\end{definition}
...
x_i$; the zero combination shows $0$ belongs: the \omterm{def:b1:vspaces:span}{span} is a
\omterm{def:b1:vspaces:subspace}{subspace}. Minimality: let $H$ be any \omterm{def:b1:vspaces:subspace}{subspace} containing $x_1,
...
$\operatorname{Vect}(x_1, \dots, x_p) \subseteq H$. So the \omterm{def:b1:vspaces:span}{span}
is contained in every \omterm{def:b1:vspaces:subspace}{subspace} containing the family: it is the
smallest one.
\end{proof}

\begin{definition}[Sum, direct sum]\label{def:b1:vspaces:sum}
For \omterm{def:b1:vspaces:subspace}{subspaces} $F, G$ of $E$:
\[
F + G = \{\,u + v : u \in F,\ v \in G\,\}
\]
is a \omterm{def:b1:vspaces:subspace}{subspace} (the smallest containing $F \cup G$). The sum is
...
G$, the \omterm{def:b1:vspaces:subspace}{subspaces} are \emph{supplementary}\index{supplementary
subspaces} in $E$.
\end{definition}
...
\omterm{prop:b1:vspaces:coordinates}{coordinates} force $a = b = 0$): the sum is direct, and $F \oplus
...
the desired identity at $x$ and $-x$; for \omterm{def:b1:poly:def}{polynomials} vanishing
...
odd functions $\mathcal{I}$ are \omterm{def:b1:vspaces:sum}{supplementary}: any $f$ writes
\[
...
$a$''. Note that $F$ is a large \omterm{def:b1:vspaces:subspace}{subspace} and $G$ a small one; a
\omterm{def:b1:vspaces:sum}{supplementary} pair need not be balanced in any sense.
\end{example}
...
$G' = \operatorname{Vect}\bigl((1,1)\bigr)$ are \omterm{def:b1:vspaces:sum}{supplementary} to
...
\omterm{def:b1:vspaces:sum}{supplementary} of $F$ in $\R^2$: supplementaries are plentiful,
and speaking of ``the'' \omterm{def:b1:vspaces:sum}{supplementary} is meaningless until an
...
(the $x$-axis): with \omterm{def:b1:vspaces:sum}{supplementary} $G$ (vertical drop) and with
\omterm{def:b1:vspaces:sum}{supplementary} $G'$ (oblique drop). The $F$-components differ:
...
$(e_1, \dots, e_n)$ is a \omterm{def:b1:vspaces:free}{basis} of $E$ if and only if every $x \in E$
is \emph{uniquely} a combination $x = \lambda_1 e_1 + \dots +
\lambda_n e_n$; the scalars $\lambda_i$ are the
\emph{coordinates}\index{coordinates} of $x$ in the \omterm{def:b1:vspaces:free}{basis}.
\end{proposition}
...
differences of \omterm{prop:b1:vspaces:coordinates}{coordinates} --- to vanish. Conversely, a nontrivial
null combination gives the two decompositions $0 = \sum \lambda_i
e_i = \sum 0\,e_i$.
\end{proof}

\begin{example}\label{ex:b1:vspaces:bases}
The \emph{canonical \omterm{def:b1:vspaces:free}{basis}} of $K^n$: $e_i = (0, \dots, 1, \dots, 0)$
($1$ in slot $i$). The monomials $(1, X, X^2, \dots, X^n)$: a \omterm{def:b1:vspaces:free}{basis}
of $K_n[X]$ (freeness: a null combination is the zero \omterm{def:b1:poly:def}{polynomial}, so
all coefficients vanish, \cref{def:b1:poly:def}). In
$\C$ over $\R$: the \omterm{def:b1:vspaces:free}{basis} $(1, \iu)$.
\end{example}

\begin{remark}[Coordinates are a team effort]
\label{rem:b1:vspaces:teameffort}
The first coordinate of $x$ in a \omterm{def:b1:vspaces:free}{basis} $(e_1, \dots, e_n)$
depends on \emph{all} the \omterm{def:b1:vspaces:free}{basis} vectors, not just $e_1$. In
$\R^2$: the vector $(3, 1)$ has first coordinate $3$ in the
canonical \omterm{def:b1:vspaces:free}{basis}, but first coordinate $2$ in the \omterm{def:b1:vspaces:free}{basis}
$\bigl((1,0), (1,1)\bigr)$ --- solve $(3,1) = a(1,0) + b(1,1)$:
$b = 1$, $a = 2$. Changing one \omterm{def:b1:vspaces:free}{basis} vector reshuffles
...
Is $\mathcal{F} = (1 + X,\ 1 + X^2,\ X + X^2)$ a \omterm{def:b1:vspaces:free}{basis} of
$\R_2[X]$? Write $u_1, u_2, u_3$ for the three \omterm{def:b1:poly:def}{polynomials}.
...
0$: $a = b = c = 0$, \omterm{def:b1:vspaces:free}{free}. \emph{Generating}: instead of solving
...
\]
The monomials lie in the \omterm{def:b1:vspaces:span}{span}, so everything does: $\mathcal{F}$
is a \omterm{def:b1:vspaces:free}{basis}. As a bonus, assembling the three displays gives the
\omterm{prop:b1:vspaces:coordinates}{coordinates} of any $P = \alpha + \beta X + \gamma X^2$:
\[
P = \frac{\alpha + \beta - \gamma}{2}\,u_1
+ \frac{\alpha - \beta + \gamma}{2}\,u_2
+ \frac{-\alpha + \beta + \gamma}{2}\,u_3 .
\]
(Sanity check with $P = X$: \omterm{prop:b1:vspaces:coordinates}{coordinates} $\bigl(\frac12,
...
generating half of this work will come \omterm{def:b1:vspaces:free}{free} of charge --- three
\omterm{def:b1:vspaces:free}{free} vectors of a $3$-dimensional space always form a \omterm{def:b1:vspaces:free}{basis}.
\end{example}

\begin{proposition}[Useful freeness criteria]\label{prop:b1:vspaces:freecriteria}
\begin{enumerate}
  \item A family of \emph{nonzero \omterm{def:b1:poly:def}{polynomials} of pairwise distinct
        degrees} is \omterm{def:b1:vspaces:free}{free}.
  \item Adding a vector to a \omterm{def:b1:vspaces:free}{free family} keeps it \omterm{def:b1:vspaces:free}{free} if and only
        if the vector is outside the \omterm{def:b1:vspaces:span}{span} of the family.
  \item Any subfamily of a \omterm{def:b1:vspaces:free}{free family} is \omterm{def:b1:vspaces:free}{free}; any family
...
family), and solving for $x$ puts it in the \omterm{def:b1:vspaces:span}{span}.

(3) Subfamily: a null combination of the subfamily is one of the
whole family with the missing coefficients \omterm{def:b1:logic:sets}{set} to $0$; freeness
...
\omterm{def:b1:vspaces:free}{basis} of $K_n[X]$. Freeness is
...
one reaches the zero \omterm{def:b1:poly:def}{polynomial}, and unwinding the subtractions
...
x})$ with $a_1 < \dots < a_p$ is \omterm{def:b1:vspaces:free}{free}: divide a null combination by
...
\]
the family is linked, and the \omterm{def:b1:vspaces:span}{span} is already generated by
$(1, \cos^2)$ alone ($\sin^2 = 1 - \cos^2$). That smaller family
is \omterm{def:b1:vspaces:free}{free}: $a + b\cos^2 x = 0$ for all $x$ gives, at $x = 0$ and
$x = \frac\pi2$: $a + b = 0$ and $a = 0$. So the \omterm{def:b1:vspaces:span}{span} is a
...
\emph{Union is not sum}: $F \cup G$ is almost never a \omterm{def:b1:vspaces:subspace}{subspace}
(\cref{def:b1:vspaces:subspace}); the smallest \omterm{def:b1:vspaces:subspace}{subspace}
...
two \omterm{def:b1:vspaces:subspace}{subspaces} are never disjoint (both contain $0$); the correct
...
\omterm{def:b1:vspaces:free}{free} in $\C$ seen as an $\R$-vector space, but linked in $\C$
seen as a $\C$-vector space ($\iu\cdot 1 + (-1)\cdot\iu = 0$).
Always know which \omterm{def:b1:structures:field}{field} is acting before declaring a family \omterm{def:b1:vspaces:free}{free}
...
Everything after this chapter speaks the language \omterm{def:b1:logic:sets}{set} up here.
\cref{ch:b1:findim} counts \omterm{def:b1:vspaces:free}{basis} vectors and turns ``\omterm{def:b1:vspaces:free}{free}'' and
``generating'' into inequalities on a single integer, the
dimension. \cref{ch:b1:linmaps} studies the \omterm{def:b1:logic:map}{maps} compatible with
the two operations; \omterm{def:b1:vspaces:sum}{direct sums} become projectors there.
\cref{ch:b1:matrices} encodes vectors by their \omterm{prop:b1:vspaces:coordinates}{coordinates} in a
\omterm{def:b1:vspaces:free}{basis} --- \cref{prop:b1:vspaces:coordinates} is the licence for
that encoding --- and \cref{ch:b1:euclid} adds lengths and angles
on top of the linear structure. In the Year~2 volume the same
axioms, verbatim, run over arbitrary \omterm{def:b1:structures:field}{fields} and in infinite
...
(\cref{ex:b1:vspaces:staircase}) reappears as the Newton \omterm{def:b1:vspaces:free}{basis}
in this chapter's weekend problem, as the binomial \omterm{def:b1:vspaces:free}{basis}
$(B_k)$ there, and as the \omterm{def:b1:poly:def}{polynomial} alternant trick in the
weekend problem of \cref{ch:b1:det}: one lemma, three
\omterm{def:b1:vspaces:free}{determinant-free} dividends.
...
\emph{\omterm{def:b1:vspaces:sum}{Direct sums}} (\cref{def:b1:vspaces:sum}) become
...
Which of the following are \omterm{def:b1:vspaces:subspace}{subspaces}?
\begin{enumerate}
...
Prove that $(1, X - 1, (X-1)^2, (X-1)^3)$ is a \omterm{def:b1:vspaces:free}{basis} of $\R_3[X]$,
and give the \omterm{prop:b1:vspaces:coordinates}{coordinates} of $X^3$ in it. \emph{(Taylor at $1$!)}
\end{exercise}
...
In the space of sequences, let $F$ be the \omterm{def:b1:logic:sets}{set} of convergent
...
Let $F, G, H$ be \omterm{def:b1:vspaces:subspace}{subspaces} of $E$. Prove that
\[
...
Prove that the following families of $\mathcal{F}(\R, \R)$ are \omterm{def:b1:vspaces:free}{free}:
\begin{enumerate}
...
Let $E$ be a $K$-vector space and $F, G, H$ \omterm{def:b1:vspaces:subspace}{subspaces} with $F + G =
...
In $\R[X]$, let $\mathcal P$ be the \omterm{def:b1:logic:sets}{set} of \emph{even} \omterm{def:b1:poly:def}{polynomials}
($P(-X) = P(X)$) and $\mathcal I$ the \omterm{def:b1:logic:sets}{set} of \emph{odd} ones
...
\omterm{def:b1:poly:def}{polynomials} are exactly the \omterm{def:b1:poly:def}{polynomials} in $X^2$.
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:b1:vspaces:11}
Let $(x_1, x_2, x_3)$ be a \omterm{def:b1:vspaces:free}{free family} of a real \omterm{def:b1:vspaces:def}{vector space} $E$.
Prove that $(x_1 + x_2,\ x_2 + x_3,\ x_3 + x_1)$ is \omterm{def:b1:vspaces:free}{free}. Is the
analogous family of four vectors $(x_1 + x_2,\ x_2 + x_3,\ x_3 +
x_4,\ x_4 + x_1)$ \omterm{def:b1:vspaces:free}{free} when $(x_1, x_2, x_3, x_4)$ is?
\end{exercise}

\begin{exercise}[$\star\star\star$]\label{exo:b1:vspaces:12}
Let $E$ be a \omterm{def:b1:vspaces:def}{vector space} over $\R$ (or $\C$) and $F_1, \dots,
F_k$ \emph{proper} \omterm{def:b1:vspaces:subspace}{subspaces} of $E$ (each $F_i \neq E$).
\begin{enumerate}
  ...
  \item Prove in general that $E \neq F_1 \cup \dots \cup F_k$: a
        \omterm{def:b1:vspaces:def}{vector space} over an infinite \omterm{def:b1:structures:field}{field} is never a finite
        union of proper \omterm{def:b1:vspaces:subspace}{subspaces}. \emph{(Take $k$ minimal, pick
...
This problem revisits \omterm{thm:b1:poly:lagrange}{Lagrange interpolation}
(\cref{thm:b1:poly:lagrange}) with the eyes of this chapter: the
space $\R_n[X]$ carries three natural bases --- Lagrange's,
Newton's, and (for equally spaced points) the binomial \omterm{def:b1:vspaces:free}{basis} ---
and each \omterm{def:b1:vspaces:free}{basis} makes one question easy. The road ends at a genuine
arithmetic theorem: P\'olya's characterization of the \omterm{def:b1:poly:def}{polynomials}
mapping $\Z$ into $\Z$.\index{Lagrange interpolation}
\index{divided differences}\index{integer-valued polynomial}

\medskip
\noindent\textbf{Part I --- The Lagrange \omterm{def:b1:vspaces:free}{basis}.} For $0 \leq i
\leq n$ set
\[
...
\begin{enumerate}
  \item Check that $\deg L_i = n$ and that $L_i(x_j) = 1$ if $j =
        i$, and $0$ if $j \neq i$.
  \item Prove that the family $(L_0, \dots, L_n)$ is \omterm{def:b1:vspaces:free}{free}.
  \item Prove that for every $P \in \R_n[X]$,
        \[
        P \;=\; \sum_{i=0}^{n} P(x_i)\, L_i ,
        \]
        and conclude that $(L_0, \dots, L_n)$ is a \omterm{def:b1:vspaces:free}{basis} of
  ...
  \item Deduce the interpolation theorem: for any values $y_0,
        ...
        \omterm{def:b1:vspaces:free}{basis}, what are the \omterm{prop:b1:vspaces:coordinates}{coordinates} of a \omterm{def:b1:poly:def}{polynomial} $P$?
...
\medskip
\noindent\textbf{Part II --- The Newton \omterm{def:b1:vspaces:free}{basis} and \omterm{pb:b1:vspaces:1}{divided
differences}.} Set $N_0 = 1$ and $N_k = (X - x_0)(X - x_1) \cdots
(X - x_{k-1})$ for $1 \leq k \leq n$. For a function $f$ defined
at the nodes, define the \emph{\omterm{pb:b1:vspaces:1}{divided differences}} by
$f[x_i] = f(x_i)$ and
\[
f[x_i, \dots, x_{i+k}] \;=\;
...
\begin{enumerate}[resume]
  \item Prove that $(N_0, N_1, \dots, N_n)$ is a \omterm{def:b1:vspaces:free}{basis} of
        $\R_n[X]$.
  \item Compute $f[x_0, x_1]$ and $f[x_0, x_1, x_2]$ in terms of
        the values of $f$, then compute all \omterm{pb:b1:vspaces:1}{divided differences} of
...
\omterm{def:b1:poly:def}{polynomial} $P$, we set
\[
\Delta P(X) = P(X + 1) - P(X),
\qquad
...
\begin{enumerate}[resume]
  ...
  \item Show that $(B_0, B_1, \dots, B_n)$ is a \omterm{def:b1:vspaces:free}{basis} of
...
\medskip
\noindent\textbf{Part IV --- \omterm{pb:b1:vspaces:1}{Integer-valued polynomials}.} A
\omterm{def:b1:poly:def}{polynomial} $P \in \R[X]$ is \emph{integer-valued} when $P(m) \in
\Z$ for every $m \in \Z$.
\begin{enumerate}[resume]
  ...
  \item Prove \emph{P\'olya's characterization}: $P \in \R_n[X]$
        is integer-valued if and only if its \omterm{prop:b1:vspaces:coordinates}{coordinates} in the
        \omterm{def:b1:vspaces:free}{basis} $(B_0, \dots, B_n)$ are integers.
  ...
  \item Let $P = \dfrac{X(X+1)(2X+1)}{6}$. Compute its Newton
        table at $0, 1, 2, 3$, write $P$ in the \omterm{def:b1:vspaces:free}{basis} $(B_k)$, and
...
\begin{enumerate}[resume]
  ...
  \item Expand $X^2$ and $X^3$ in the \omterm{def:b1:vspaces:free}{basis} $(B_k)$ and deduce
        closed formulas for $\sum_{j=0}^{m-1} j^2$ and
        $\sum_{j=0}^{m-1} j^3$; recover Nicomachus' identity
        $1^3 + \dots + m^3 = (1 + \dots + m)^2$.
  \item Take $n = 2$ and nodes $0, 1, 2$. Write the \omterm{prop:b1:vspaces:coordinates}{coordinates} of
        $X^2$ in the three bases of this problem: the monomial
        \omterm{def:b1:vspaces:free}{basis}, the Lagrange \omterm{def:b1:vspaces:free}{basis}, the Newton \omterm{def:b1:vspaces:free}{basis}. Check the
        three answers against questions 4 and 9.
  \item Synthesis. In four sentences: which vector-space concept
        makes question 4 automatic; why the Newton \omterm{def:b1:vspaces:free}{basis} computes
        \omterm{prop:b1:vspaces:coordinates}{coordinates} \emph{recursively} while the Lagrange \omterm{def:b1:vspaces:free}{basis}
        reads them off \emph{instantly}; which freeness criterion
        both bases share; and in what precise sense P\'olya's
        theorem says that integrality of a \omterm{def:b1:poly:def}{polynomial} is a
        property of its \omterm{prop:b1:vspaces:coordinates}{coordinates} \emph{in the right \omterm{def:b1:vspaces:free}{basis}}.
\end{enumerate}
\end{problem}
