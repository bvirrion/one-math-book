...
--- monotone convergence, \omterm{thm:b1:seq:adjacent}{adjacent sequences}, Bolzano--Weierstrass,
...
Given $\varepsilon > 0$, the \omterm{thm:b1:reals:archimedes}{Archimedean property} provides $N \geq
...
$n = 100$) --- divergence to $+\infty$ is a \omterm{def:b1:logic:statement}{statement} about the
...
$+\infty$. (Mirror \omterm{def:b1:logic:statement}{statements} for decreasing
...
{\small \omterm{thm:b1:seq:adjacent}{Adjacent sequences}: $(a_n)$ climbs, $(b_n)$ descends,
and the gap between them shrinks to $0$. Each \omterm{prop:b1:reals:intervals}{interval}
$\intcc{a_n}{b_n}$ contains all later ones, and the common limit
$\ell$ is the unique point left in every \omterm{prop:b1:reals:intervals}{interval} --- the
...
If $u_n \to \ell$ ($\ell \in \R$ or $\pm\infty$), every \omterm{def:b1:seq:subsequence}{subsequence}
tends to $\ell$. Consequently, a sequence with two \omterm{def:b1:seq:subsequence}{subsequences} of
...
\omterm{def:b1:seq:subsequence}{subsequences}, and that is the whole point.
\end{proof}

\begin{example}[Subsequential limits]\label{ex:b1:seq:sublimits}
For $u_n = (-1)^n \frac{n}{n+1}$: the even \omterm{def:b1:seq:subsequence}{subsequence} tends to
$1$, the odd one to $-1$, so the sequence diverges --- but it does
so in an organized way, clustering around the two values $\pm 1$.
For $u_n = \cos\frac{2\pi n}{3}$: the three \omterm{def:b1:seq:subsequence}{subsequences} of
indices $3k$, $3k + 1$, $3k + 2$ are constant, equal to $1$,
$-\frac12$, $-\frac12$; the \omterm{def:b1:logic:sets}{set} of subsequential limits is $\{1,
...
\omterm{def:b1:seq:subsequence}{subsequence}.\index{Bolzano--Weierstrass theorem}
\end{theorem}
...
It \emph{does} say: from boundedness alone, some \omterm{def:b1:seq:subsequence}{subsequence}
...
\omterm{def:b1:seq:subsequence}{subsequences} converging to $1$ and to $-1$, and the \omterm{def:b1:logic:sets}{set} of
subsequential limits can even be infinite
(\cref{ex:b1:seq:sublimits}, and all of the Cantor \omterm{def:b1:logic:sets}{set} in
\cref{pb:b1:topology:1}). It does not survive unboundedness:
$(n)$ has no convergent \omterm{def:b1:seq:subsequence}{subsequence} at all --- though one can
always extract a \omterm{def:b1:seq:subsequence}{subsequence} tending to $+\infty$ or $-\infty$
...
A sequence of reals converges if and only if it is a \omterm{def:b1:seq:cauchy}{Cauchy
...
\cref{thm:b1:seq:bw}, some \omterm{def:b1:seq:subsequence}{subsequence} $u_{\varphi(n)} \to \ell$.
...
$\sqrt 2$ form a \omterm{def:b1:seq:cauchy}{Cauchy sequence} of rationals with no rational
limit): completeness is a property of $\R$, equivalent to the \omterm{def:b1:reals:bounds}{upper
...
\begin{enumerate}
  \item \emph{Stable \omterm{prop:b1:reals:intervals}{interval}:} find an \omterm{prop:b1:reals:intervals}{interval} $I$ with $f(I)
  ...
  \item \emph{Monotonicity:} if $f$ is increasing on $I$, then
        ...
        study the two \omterm{def:b1:seq:subsequence}{subsequences} $(u_{2n})$ and $(u_{2n+1})$,
...
\omterm{def:b1:seq:subsequence}{subsequence} (\cref{thm:b1:seq:bw}). (iv) \emph{The fixed-point
...
$(H_n)$ is \emph{not} a \omterm{def:b1:seq:cauchy}{Cauchy sequence}, hence diverges (to
$+\infty$, being increasing).
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:b1:seq:6}
Suppose $(u_{2n})$, $(u_{2n+1})$ and $(u_{3n})$ all converge. Prove
that $(u_n)$ converges. \emph{(Find common \omterm{def:b1:seq:subsequence}{subsequences} to equate the
limits.)}
\end{exercise}
...
Let $(u_n)$ be bounded, such that every convergent \omterm{def:b1:seq:subsequence}{subsequence} of
...
Using the density of the \omterm{def:b1:structures:subgroup}{subgroup} $\Z + 2\pi\Z$ of $(\R, +)$
...
\begin{enumerate}[resume]
  ...
  \item Quantify the slowness: show that eventually $\sqrt{2/n}
        ...
        $30\,000$, by the asymptotics). Contrast with \omterm{ex:b1:seq:heron}{Heron's
        ...
        error-halving iterations need a slope of \omterm{def:b1:complex:field}{modulus} $< 1$.
