...
\bigl((1,0), (0,1)\bigr)$ and the free family $\bigl(f_1,
...
there been a third free vector $f_3$, no $g$ would remain to
absorb it --- which is precisely how the lemma forbids $3$ free
...
generating, and it is free: a nontrivial null combination would
...
The set $\C^2$ of pairs of complex numbers is a $\C$-vector space
of dimension $2$ (canonical basis $e_1, e_2$) --- and an
$\R$-vector space of dimension $4$, with basis
\[
(1, 0),\quad (\iu, 0),\quad (0, 1),\quad (0, \iu):
\]
every $(z, w) = (a + \iu b,\ c + \iu d)$ has real coordinates
$(a, b, c, d)$, uniquely. Dimension is not a property of the set
...
Complete the free family $\bigl((1,1,1)\bigr)$ into a basis of
...
of $(1,1,1)$ have equal coordinates) --- append it. Is $e_2 \in
\operatorname{Vect}\bigl((1,1,1), e_1\bigr)$? A combination
$\alpha(1,1,1) + \beta e_1$ has equal second and third
coordinates, and $e_2$ does not --- append it. The family
$\bigl((1,1,1), e_1, e_2\bigr)$ is free with $3$ vectors: stop,
it is a basis (\cref{prop:b1:findim:twoofthree} will make this
...
Is $\bigl((1,1,0), (0,1,1), (1,0,1)\bigr)$ a basis of $\R^3$?
...
0$: free, hence a basis, with the generating half of the
...
converts into exactly that saving, on every basis check for the
rest of the book.
\end{example}
...
\begin{enumerate}
  \item \emph{Parametrize, then read off a basis.} Solve the
        defining constraints, express the general element
        linearly in the surviving parameters, and check that the
        vectors multiplying the parameters are free: the
        dimension is the number of parameters. (Run below on a
        concrete subspace of $\R^4$.)
  \item \emph{Exhibit a bijective linear parametrization.} When
  ...
  \item \emph{Use the formulas.} Grassmann for intersections and
        sums, rank for spans, and later rank--nullity for
...
\]
The two vectors are free (look at the first two coordinates:
$(x, y) = (0,0)$), so they form a basis of $H$ and $\dim H = 2$.
The count was predictable --- two independent linear constraints
in $\R^4$ should each eat one dimension --- but the
parametrization proves it \emph{and} hands over a basis, which
...
reaches all of $W$, and is injective (a product is zero only if
$Q = 0$): $W$ is parametrized bijectively and linearly by
$\R_{n-2}[X]$, so
\[
\dim W = \dim \R_{n-2}[X] = n - 1 .
\]
A basis comes with the parametrization: the images of the
...
backbone of Lagrange interpolation
...
\frac13\bigr)$, dimension $2$ (the two polynomials have distinct
degrees: free). One linear condition --- whether an evaluation,
an integral, or any other linear recipe --- removes at most one
dimension, and exactly one as soon as the condition is not
identically zero. \cref{ch:b1:linmaps} will name such recipes
\emph{linear forms} and their solution sets \emph{hyperplanes};
the basis vectors found here reappear in
...
in $E$: a free family of $F$ is in particular free in $E$, and $E$
has a finite generating family). Among the free families of $F$
pick one of \emph{maximal} size $p$ --- possible since the sizes
are integers bounded by $\dim E$. It generates $F$: otherwise some
$x \in F$ would lie outside its span, and appending $x$ would give
a free family of $F$ of size $p + 1$
(\cref{prop:b1:vspaces:freecriteria}~(2)), contradicting
maximality. Being free and generating, it is a basis of $F$, and
$\dim F = p \leq \dim E$. If $p = \dim E = n$: a free family of $n$ vectors of
...
Rank of $\bigl((1,2,3), (2,3,4), (3,4,5), (1,1,1)\bigr)$ in
$\R^3$. The span is unchanged when one subtracts from a vector a
combination of the others (both families span the same
combinations): replace $(2,3,4)$ by $(2,3,4) - (1,2,3) = (1,1,1)$
and $(3,4,5)$ by $(3,4,5) - (1,2,3) = (2,2,2)$. The span is now
$\operatorname{Vect}\bigl((1,2,3), (1,1,1), (2,2,2),
(1,1,1)\bigr) = \operatorname{Vect}\bigl((1,2,3), (1,1,1)\bigr)$,
and these two vectors are not proportional: the rank is $2$. This
...
the pile by the rank algorithm; no new technique is ever needed.
\end{example}
...
When both subspaces come as solution sets, intersecting is just
...
subspace --- by equations, by generators --- each make one
...
only an actual parametrization or a rank computation decides.
\emph{Do not speak of the dimension of a non-subspace}: solution
sets of \emph{inhomogeneous} systems miss $0$; their ``dimension''
is that of the associated homogeneous solution space
(\cref{ch:b1:det} makes this precise).
\emph{Infinite dimension exists}: $K[X]$ contains free families
of every size (the monomials), so no finite generating family
can exist --- statements like the two-out-of-three rule are
strictly finite-dimensional and fail badly on $K[X]$
(\cref{cor:b1:linmaps:samedim} will show the same for
injectivity/surjectivity).
\emph{Rank is about the span, not the list}: repeating a vector,
reordering, or rescaling by nonzero constants leaves the rank
unchanged, and $\operatorname{rk} = p$ (the number of vectors)
is a \emph{property to prove} --- it is exactly freeness. A
family of $5$ vectors of rank $2$ carries three vectors' worth
...
computes ranks by row reduction; \cref{ch:b1:det} turns ``$n$
vectors of $K^n$ form a basis'' into one number being nonzero. The
weekend problem below shows dimension doing \emph{arithmetic}:
counting dimensions over the field $\Q$ proves irrationality
statements that look untouchable by hand. In the Year~3 volume the
same dimension counts, refined by group theory, decide which
...
\operatorname{rk} u$: what a linear map crushes plus what it
keeps always totals the source. \cref{ch:b1:det} refines this
into the structure of solution sets: $p$ unknowns minus
$\operatorname{rk} A$ pivots leaves the dimension of the
solution space, which Gaussian elimination exhibits as free
...
Let $E$ be the set of real sequences satisfying $u_{n+2} =
3u_{n+1} - 2u_n$ for all $n$.
\begin{enumerate}
  \item Show that $E$ is a subspace of the space of sequences, and
  ...
  \item Check that the constant sequence $(1)$ and the geometric
        sequence $(2^n)$ lie in $E$ and form a basis of $E$.
...
\begin{enumerate}
  \item If $F, G$ are subspaces with $\dim F + \dim G > n$, prove
        $F \cap G \neq \{0\}$. Illustrate: two subspaces of
        dimensions $51$ and $50$ of $\R^{100}$ always share a
        nonzero vector.
  \item If $H$ is a hyperplane and $F$ a subspace with $F \cap H =
        \{0\}$, prove $\dim F \leq 1$.
\end{enumerate}
\end{exercise}

\begin{exercise}[$\star\star\star$]\label{exo:b1:findim:12}
(Common supplementary) Let $F, G$ be subspaces of $E$ (finite
dimension) with $\dim F = \dim G$. Prove that $F$ and $G$ admit a
\emph{common} supplementary: there is a subspace $S$ with $E = F
...
the field axioms (\cref{def:b1:structures:field}): one may
therefore take $K = \Q$ and measure sets of \emph{real numbers}
with the yardstick of $\Q$-dimension. This problem computes the
dimension of $\Q(\sqrt2, \sqrt3)$, proves \emph{Dedekind's tower
...
\begin{enumerate}
  \item Check that $\Q$ is a field and that every definition and
        proof of Chapters 18--19 uses only the field axioms of the
        scalars; conclude that $\R$ is a $\Q$-vector space and
        that the exchange lemma, the basis theorems and
  ...
  \item Show that $(1, \sqrt2)$ is free over $\Q$ but linked over
...
\begin{enumerate}[resume]
  ...
  \item Let $M = \operatorname{Vect}_\Q(1, \sqrt2, \sqrt3,
        ...
        \emph{(a table of the products of basis vectors
        suffices)}.
  \item Write $K = \Q(\sqrt2)$. Show that $(1, \sqrt3)$ is free
        \emph{over the field $K$}, and deduce that $M = K +
        K\sqrt3$ is a $K$-vector space of dimension $2$ with
        basis $(1, \sqrt3)$.
  \item Prove that $(1, \sqrt2, \sqrt3, \sqrt6)$ is free over
...
\medskip
\noindent\textbf{Part III --- The tower law.} Let $\Q \subseteq K
\subseteq M \subseteq \R$ where $K$ and $M$ are subfields, $(e_1,
\dots, e_m)$ is a basis of $K$ as a $\Q$-vector space, and $(f_1,
\dots, f_n)$ a basis of $M$ as a $K$-vector space.
\begin{enumerate}[resume]
  \item Show that the $mn$ products $(e_i f_j)$ generate $M$ over
        $\Q$.
  \item Show that the family $(e_i f_j)$ is free over $\Q$.
        \emph{(Reorganize a null $\Q$-combination as $\sum_j
        \bigl(\sum_i \lambda_{ij} e_i\bigr) f_j$, whose inner
        coefficients live in $K$.)}
  \item Conclude with \emph{Dedekind's tower law}:
        \[
        \dim_\Q M \;=\; \dim_\Q K \,\cdot\, \dim_K M ,
        \]
        and check it on $\Q \subseteq \Q(\sqrt2) \subseteq M$
        against questions 7 and 8.
  \item (Fields for free) Let $A \subseteq \R$ be a
        finite-dimensional $\Q$-subspace containing $1$ and
        stable under multiplication, and let $u \in A$, $u \neq
        0$. Show that if $(a_1, \dots, a_d)$ is a basis of $A$,
        then $(u a_1, \dots, u a_d)$ is again a basis of $A$;
  ...
  \item Show that for every $x \in A$ (as in question 12, $\dim_\Q
        ...
        every element of $A$ is a root of a nonzero polynomial
...
Set $s = \sqrt2 + \sqrt3$.
\begin{enumerate}[resume]
  \item Compute the coordinates of $s^2$, $s^3$ and $s^4$ in the
        basis $(1, \sqrt2, \sqrt3, \sqrt6)$ of $M$.
  \item Show that $(1, s, s^2, s^3)$ is free over $\Q$, and deduce
        $\operatorname{Vect}_\Q(1, s, s^2, s^3) = M$: every
        element of $M$ is a rational polynomial in $s$. Express
        $\sqrt2$ and $\sqrt3$ as such polynomials.
  \item Verify $s^4 - 10s^2 + 1 = 0$, and show that $X^4 - 10X^2
        + 1$ is the \emph{monic polynomial of least degree}
...
\begin{enumerate}[resume]
  \item Show that $X^3 - 2$ has no rational root \emph{(the
        ...
        valuation count)}; deduce that $(1, t)$ is free over
        $\Q$.
  \item Show that $(1, t, t^2)$ is free over $\Q$. \emph{(If some
  ...
  \item Prove that $t \notin M$: otherwise $M$ would be a vector
        space over the field $A$, and the tower law would force
...
\begin{enumerate}[resume]
  \item Prove the divisibility of degrees: if $\Q \subseteq A
        \subseteq B \subseteq \R$ are subfields with $\dim_\Q B$
        finite, then $\dim_\Q A$ divides $\dim_\Q B$. What are
  ...
  \item Synthesis, in four sentences: why $\Q$-dimension is an
        ...
        tower law forbids (question 20). Name the theorem proved
        in Part III.
\end{enumerate}
\end{problem}
