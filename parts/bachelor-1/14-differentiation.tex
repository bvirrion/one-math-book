...
so $f$ is differentiable at $0$ with $f'(0) = 0$. At any $x_0
\neq 0$, $f$ is not even continuous: rational and irrational
sequences converging to $x_0$ send $f$ to $x_0^2 \neq 0$ and to
$0$ respectively (density, \cref{thm:b1:reals:density}). So
differentiability is a genuinely \emph{pointwise} notion: it can
hold at one point of $\R$ and nowhere else. The moral for
practice: statements like the monotonicity criterion or Rolle
require the derivative \emph{on an interval} --- possessing
...
for $f$ decreasing). The inverse remains continuous but is not
differentiable at $y_0$ --- its graph, the reflection of $f$'s
...
The theorem recomputes the classical derivatives with no limit
...
statement that the two variables live on opposite sides of the
bijection.
\end{example}
...
think of $x$ on $\intcc{0}{1}$, maximal at $1$ with derivative
$1$.)
\end{proof}
...
squares the error --- the general statement follows from the
Taylor bounds of \cref{ch:b1:taylor}. The closing insight:
where dichotomy (\cref{ex:b1:continuity:dichotomy}) uses only
continuity and gains one bit per step, Newton spends a
derivative to double the number of correct digits per step.
\end{example}
...
derivative evaluation. Likewise $\abs{\sin a - \sin b} \leq
...
``$f' = 0 \implies f$ constant'' is an \emph{interval} statement
...
well defined up to a constant \emph{on each interval of its
...
though its derivative vanishes at $0$: the criterion's clause
...
(\cref{prop:b1:counting:identities}), the boundary terms $j = 0$
...
x^2$, whose derivatives die quickly ($f' = 2x$, $f'' = 2$,
...
is a polynomial --- the sum then has only $\deg + 1$ terms, and
...
The function $f(t) = t\ln t$ is convex on $\intoo{0}{+\infty}$:
...
whose second derivative has a sign and write down what convexity
...
(i) \emph{A positive derivative at one point does not give
...
neighborhood of $0$ contains descents. Monotonicity needs $f'
\geq 0$ \emph{on an interval}
...
(no interior differentiability), $x$ on $\intcc{0}{1}$ (ends not
equal), and $x - \lfloor x\rfloor$ on $\intcc{0}{1}$ (continuity
fails at $1$) each break exactly one hypothesis and the
conclusion. (iii) \emph{Derivatives may be discontinuous, but
not arbitrarily}: $f'$ can oscillate
(\cref{ex:b1:derivative:notc1}) yet always satisfies the
intermediate value property (Darboux,
\cref{exo:b1:derivative:10}): a derivative never jumps --- if
you compute a one-sided ``derivative limit'' with a jump, you
have differentiated a non-differentiable function. (iv) \emph{The
...
Nearly every quantitative statement of the next chapters is this
...
number theory --- a repulsion inequality between algebraic
numbers and rationals, yielding the first transcendental number
...
inequalities of the Year 2 volume --- one concavity statement
about $\ln$, harvested for norms.
\end{example}

\begin{remark}[Perspectives inside this volume]
The derivative acquires three new lives before the volume ends.
In \cref{ch:b1:taylor} it iterates: $n$ derivatives at a point
compress into one polynomial plus a controlled error, and the
...
derivatives repeat this chapter twice over, and the tangent line
...
\begin{enumerate}
  \item Let $f, g$ be continuous on $\intcc{a}{b}$,
        differentiable on $\intoo{a}{b}$, with $g'$ never zero
...
\begin{problem}[{Weekend problem --- algebraic numbers repel
...
polynomial with integer coefficients, and \emph{transcendental}
otherwise. In 1844 Liouville produced the first number ever
\emph{proved} transcendental, and the engine of his proof is this
chapter's mean value theorem: an algebraic number of degree $d$
...
\begin{enumerate}[resume]
  \item Show that among the nonzero integer polynomials
...
\begin{enumerate}[resume]
  ...
  \item Generalize: replace each digit $1$ by an arbitrary
        ...
        many transcendental numbers of this shape.
\end{enumerate}
...
\begin{enumerate}[resume]
  \item Assemble the hierarchy from Parts I--III: rationals are
        ...
        least $2$; an algebraic number of degree $d$ to no order
  ...
  \item (Cantor, 1874) Prove that the set of algebraic numbers
        is countable: there are finitely many integer
        polynomials with degree plus sum of $\abs{\text{coefficients}}$
        bounded by $h$, each with at most $\deg$ roots; a
        countable union of finite sets is countable. Since no
        sequence exhausts $\R$ (\cref{pb:b1:reals:1}, question
        22), transcendental numbers exist --- in fact form an
        uncountable set. Compare the two proofs: what does
  ...
  \item Prove directly from question 2 that $\sqrt 2$ is
        ...
        no algebraic number is Liouville.
\end{enumerate}
...
\begin{enumerate}[resume]
  \item Let $x^\dagger$ be the value of the decimal string with
        ...
        (any exponent $> 2$ suffices, for every algebraic
        number) is Roth's theorem, far beyond this volume.
  \item Quantify Cantor: show that the algebraic numbers of
        degree $\leq d$ given by polynomials with coefficients
  ...
  \item Synthesis, one sentence each: (i) locate the single
        ...
        of the existence of transcendental numbers; (iv) name
        ...
        squeeze, with integrals in place of derivatives.
\end{enumerate}
\end{problem}
