...
1$. Isto dividir $ Q_n = X^{2n} + X^n + 1$ se $ j $ e $ j^2$ são raízes de
...
\begin{enumerate}
  \item Deixar $ p/q $ (termos mais baixos) seja uma raiz de \omterm{def:b1:poly:def}{Mônica} inteiro
        \omterm{def:b1:poly:def}{polinômio} $ X^3 + \dots + c_0$: limpando denominadores em
...
então $ a = -\frac32$, $ b = \frac72$. Mesmo \omterm{def:b1:poly:def}{polinômio}:
$ P = -\frac32 X^2 + \frac72 X + 1$. (Verificar $ P(2) = -6 + 7 + 1 = 2$.)
\end{solution}

\begin{solution}{exo:b1:poly:9}
$ P = X^{2n+1} - 1$: $ P' = (2n+1)X^{2n} \geq 0$, então o \omterm{def:b1:poly:def}{polinômio}
...
\begin{enumerate}
  ...
  \item $ T_n(\cos\theta) = 0$ se $ n\theta \equiv \frac\pi2 \pmod
        ...
        $\intoo{0}{\pi}$ onde $\cos $ é injetiva), todas as raízes de
  ...
  \item Para $ x = \cos\theta \in \intcc{-1}{1}$: $\abs{T_n(x)} =
        ...
        grau-$ n $ \omterm{def:b1:poly:def}{polinômio} da menor sup-norma em
...
infinito conjunto --- então $ P - T_n $ tem infinitas raízes e é o
zero \omterm{def:b1:poly:def}{polinômio} (\cref{cor:b1:poly:nroots}).
...
portanto, como \omterm{def:b1:poly:def}{polinômio} identidades pelo argumento da questão 3.
...
raízes distintas para um diferente de zero \omterm{def:b1:poly:def}{polinômio} de grau $\leq n - 1$,
...
raízes com \omterm{def:b1:poly:derivative}{multiplicidade}, por um \omterm{def:b1:poly:def}{polinômio} de grau $\leq n - 1$:
então $ D = 0$ e $ P = \widetilde T_n $. O minimizador é único.

\textbf{17.} O afim \omterm{def:b1:logic:map}{aplicação} $ t \mapsto x = \frac{a+b}2 +
\frac{b-a}2\,t $ é uma bijeção $\intcc{-1}1 \to \intcc ab $. Se $ P $
é \omterm{def:b1:poly:def}{Mônica} de grau $ n $, então $ Q(t) = P(x(t))$ é um \omterm{def:b1:poly:def}{polinômio} em
...
\frac{84.5 + 0.01}2 \approx 42$. Um \omterm{def:b1:poly:def}{polinômio} preso em
...
\textbf{24.} (i) Rigidez --- a \omterm{def:b1:poly:def}{polinômio} com mais raízes do que
seu grau é zero --- alimenta o princípio da singularidade
(questão 3), a transferência de identidades trigonométricas para
\omterm{def:b1:poly:def}{polinômio} identidades (questões 4, 7, 9, 11), e ambas
...
diferem dele por um baixo grau \omterm{def:b1:poly:def}{polinômio} forçado a mudar de sinal
