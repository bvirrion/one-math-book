...
\]
Computing the same quotient algebraically (multiply by the \omterm{def:b1:complex:field}{conjugate}):
\[
...
the quotient is $0 \in \iu\R$). The \omterm{def:b1:logic:sets}{set} is the circle of diameter
...
$1 - \iu$. Pairing \omterm{def:b1:complex:field}{conjugate} roots:
\[
...
\]
purely imaginary as claimed. The \omterm{def:b1:logic:map}{map} $t \mapsto \cos t/\sin t$ is
\omterm{def:b1:logic:inj}{injective} on $\intoo0\pi$ (it is strictly decreasing), so the
...
the cube \omterm{def:b1:complex:unity}{roots of unity}, \cref{prop:b1:complex:sumroots}); $j^{-1}
...
\text{constant},
\]
a \omterm{def:b1:logic:map}{map} of the form $Az + B$ with $A = \eu^{\iu(\theta+\theta')}$ of
\omterm{def:b1:complex:field}{modulus} $1$. By \cref{prop:b1:complex:similitude}, it is a rotation
...
identity \omterm{def:b1:logic:map}{map} as neutral element, the direct similarities form a
...
similarity-invariant \omterm{def:b1:logic:statement}{statement} for one well-placed triangle proves
...
\textbf{25.} The dictionary converts geometric \omterm{def:b1:logic:statement}{statements} into
