...
whose \omterm{def:g2:shapes:solids}{faces} are the flat shapes we already know.
...
\emph{polygon}\index{polygon}: triangle ($3$ sides), quadrilateral ($4$ sides:
...
Solids take up space. Their flat sides are \emph{faces}\index{face}:
\begin{itemize}
  \item the \emph{cube}\index{cube}: $6$ square faces (a die);
  \item the \emph{box} (rectangular prism): $6$ rectangle faces (a
        cereal packet);
  \item the \emph{cylinder}\index{cylinder}: two round faces and a rolled side (a
        can);
...
{\small The first solids. Run a finger over a \omterm{def:g2:shapes:solids}{cube}: you feel $6$ flat
\omterm{def:g2:shapes:solids}{faces} and sharp edges; a ball has none.}
\end{omfigure}
...
Sort into ``\omterm{def:g2:shapes:sort}{polygon}'' or ``not a \omterm{def:g2:shapes:sort}{polygon}'': a square; a circle; a
triangle; a hexagon.
\end{exercise}
...
How many \omterm{def:g2:shapes:solids}{faces} has a \omterm{def:g2:shapes:solids}{cube}? What shape is each \omterm{def:g2:shapes:solids}{face}? How many \omterm{def:g2:shapes:solids}{faces}
has a box?
\end{exercise}

\begin{exercise}[$\star$]\label{exo:g2:shapes:8}
Which solids can roll: the \omterm{def:g2:shapes:solids}{cube}, the box, the ball, the \omterm{def:g2:shapes:solids}{cylinder}?
Why?
\end{exercise}
