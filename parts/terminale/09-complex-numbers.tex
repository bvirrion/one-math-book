\chapter{Complex Numbers}\label{ch:tle:complex}

The equation $x^2 = -1$ has no real solution. Enlarging $\R$ with a single
new number $\iu$ whose square is $-1$ produces the field $\C$ of complex
numbers, in which \emph{every} polynomial equation has solutions --- and
...
\begin{definition}[Complex numbers]\label{def:tle:complex:def}
...
$\iu^2$ to $-1$. The reals $a$ and $b$ are the \emph{real part}
$\Rea(z)$ and \emph{imaginary part} $\Ima(z)$ of $z = a + \iu b$; this
expression is the \emph{algebraic form} of $z$, and it is unique:
...
\begin{definition}[Conjugate]\label{def:tle:complex:conjugate}
The \emph{conjugate}\index{conjugate} of $z = a + \iu b$ is
$\conj{z} = a - \iu b$.
\end{definition}

\begin{proposition}[Properties of conjugation]\label{prop:tle:complex:conjugate}
For all $z, w \in \C$:
\[
...
\begin{proof}
All are direct computations from the algebraic forms; for instance
$z\conj z = (a + \iu b)(a - \iu b) = a^2 - (\iu b)^2 = a^2 + b^2$.
\end{proof}

\begin{method}[Dividing complex numbers]\label{met:tle:complex:division}
To write a quotient in algebraic form, multiply numerator and denominator by
the conjugate of the denominator:
\[
\frac{1}{2 + \iu} = \frac{2 - \iu}{(2+\iu)(2-\iu)} = \frac{2 - \iu}{5}
= \frac25 - \frac15\iu .
\]
Every nonzero complex number therefore has an inverse: $\C$ is a
\emph{field}.
\end{method}

\section{The complex plane, modulus and argument}

\begin{definition}[Affix, modulus]\label{def:tle:complex:modulus}
In the plane with orthonormal coordinates, the point $M(a, b)$ is the
\emph{image} of $z = a + \iu b$, and $z$ is the \emph{affix} of $M$. The
...
{\small The point $M(a,b)$ of affix $z$: the modulus $\abs z$ is the
length $OM$, and the conjugate $\conj z$ is the reflection of $z$ in the
real axis.}
\end{omfigure}

\begin{proposition}[Properties of the modulus]\label{prop:tle:complex:modulus}
For $z, w \in \C$:
\[
...
\]
Moreover $\abs{z_B - z_A}$ is the distance between the points of affixes
$z_A$ and $z_B$.
\end{proposition}
...
formula applied to the coordinates of $z_B - z_A$.
\end{proof}

\begin{definition}[Argument, trigonometric and exponential forms]\label{def:tle:complex:argument}
...
{\small Exponential form: $z$ is determined by its distance $r$ to the
origin and the angle $\theta$ from the positive real axis.}
\end{omfigure}

\begin{theorem}\label{thm:tle:complex:expform}
...
\begin{corollary}[De Moivre's formula]\label{cor:tle:complex:demoivre}
...
\begin{proof}
Induction on $n \geq 0$ using \cref{thm:tle:complex:expform}; extend to
$n < 0$ by taking inverses.
\end{proof}

\begin{method}[Switching between forms]\label{met:tle:complex:forms}
...
before invoking an inverse cosine). From exponential to algebraic: expand
$r\cos\theta + \iu\, r\sin\theta$. Use the algebraic form for sums, the
exponential form for products, powers and quotients.
\end{method}
...
\begin{theorem}[Quadratic equations with real coefficients]\label{thm:tle:complex:quadratic}
Let $a, b, c \in \R$, $a \neq 0$, and $\Delta = b^2 - 4ac$. The equation
...
{\small The fifth roots of unity, $\omega = \eu^{2\iu\pi/5}$: the
vertices of a regular pentagon inscribed in the unit circle
(\cref{exo:tle:complex:10}).}
\end{omfigure}
...
\begin{proposition}[Geometric dictionary]\label{prop:tle:complex:geometry}
Let $A, B, C, D$ be points of affixes $a, b, c, d$ ($a \neq b$,
$c \neq d$).
\begin{enumerate}
  \item The translation by the vector of affix $t$ is $z \mapsto z + t$.
  \item The rotation of center $\omega$ (affix) and angle $\theta$ is
        $z \mapsto \omega + \eu^{\iu\theta}(z - \omega)$.
  \item The scaling (homothety) of center $\omega$ and ratio
        $k \in \R^*$ is $z \mapsto \omega + k(z - \omega)$.
  \item $\dfrac{d - c}{b - a}$ has modulus $\dfrac{CD}{AB}$ and argument the
        angle between the vectors $\vect{AB}$ and $\vect{CD}$. In
        particular, $AB \perp CD$ if and only if $\frac{d-c}{b-a}$ is purely
        imaginary, and $A, B, C$ are collinear ($C \ne A$, $C\ne B$) if and
...
arguments: it is the rotation of angle $\theta$ about the origin; conjugating
by the translation sending $\omega$ to $0$ gives the general center. For
(4), moduli and arguments of quotients subtract
(\cref{thm:tle:complex:expform}).
\end{proof}

\section{Exercises}

\begin{exercise}[$\star$]\label{exo:tle:complex:1}
Write in algebraic form:
$(3 - 2\iu)(1 + 4\iu)$, \quad $\dfrac{2 + \iu}{1 - \iu}$, \quad
$\iu^{2027}$.
\end{exercise}

\begin{exercise}[$\star$]\label{exo:tle:complex:2}
Solve in $\C$: (a) $z^2 - 4z + 13 = 0$; (b) $z^2 + z + 1 = 0$. Check in each
case that the two solutions are conjugates and compute their product.
\end{exercise}

\begin{exercise}[$\star$]\label{exo:tle:complex:3}
Put in exponential form: $z_1 = -1 + \iu$, $z_2 = \sqrt3 - \iu$, and compute
$\dfrac{z_1}{z_2}$ in exponential then algebraic form. Deduce the exact
value of $\cos\dfrac{11\pi}{12}$.
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:tle:complex:4}
Describe geometrically the set of points $M$ of affix $z$ such that:
(a) $\abs{z - 2} = \abs{z + \iu}$;\quad
(b) $\abs{z - 1 - \iu} = 3$;\quad
(c) $\dfrac{z - 1}{z + 1}$ is purely imaginary.
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:tle:complex:5}
...
\begin{exercise}[$\star\star$]\label{exo:tle:complex:6}
...
\begin{exercise}[$\star\star$]\label{exo:tle:complex:7}
Let $A$ and $B$ have affixes $a = 1 + \iu$ and $b = 3 + 2\iu$. Determine the
two points $C$ making the triangle $ABC$ equilateral, as the images of $B$
under the rotations of center $A$ and angles $\pm\frac{\pi}{3}$.
\end{exercise}

\begin{exercise}[$\star\star\star$]\label{exo:tle:complex:8}
Solve $z^4 = -4$ in $\C$ and plot the solutions. Factor
$z^4 + 4$ into two quadratic polynomials with real coefficients.
\end{exercise}

\begin{exercise}[$\star\star\star$]\label{exo:tle:complex:9}
For $\theta \in \intoo{0}{2\pi}$ and $n \in \N$, compute
\[
S = 1 + \cos\theta + \cos 2\theta + \dots + \cos n\theta
\]
by summing the geometric progression
$\sum_{k=0}^n \eu^{\iu k\theta}$ and taking real parts. (Answer:
$S = \dfrac{\sin\frac{(n+1)\theta}{2}}{\sin\frac{\theta}{2}}
\cos\frac{n\theta}{2}$.)
\end{exercise}

\begin{exercise}[$\star\star\star$]\label{exo:tle:complex:10}
