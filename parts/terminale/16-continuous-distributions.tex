\chapter{Continuous Random Variables}\label{ch:tle:contdist}

Waiting times, physical measurements, proportions: many random quantities
take a continuum of values, and no single value has positive probability.
Probabilities are then computed by integrating a \emph{density}. This
chapter studies the uniform, exponential and normal distributions, and uses
the normal law to quantify the fluctuations of polls and samples.

\section{Probability densities}

\begin{definition}[Density, continuous random variable]\label{def:tle:contdist:density}
A \emph{probability density}\index{density} on an interval $I$ is a
continuous, nonnegative function $f$ on $I$ with $\int_I f(t)\,\dd t = 1$
(the integral over an unbounded $I$ being understood as a limit of
integrals over growing bounded intervals). A random variable $X$ has
...
Probabilities are \emph{areas under the density curve}. In particular
$\P(X = a) = \int_a^a f = 0$ for every single value $a$: only intervals
carry probability, and $\P(a \leq X \leq b) = \P(a < X < b)$.

\begin{definition}[Expectation and variance]\label{def:tle:contdist:exp}
For $X$ with density $f$ on $I$:
\[
...
The rules of Chapters 14--15 (linearity of expectation, additivity of
variance for independent variables, Bienaymé--Chebyshev) remain valid;
their proofs, with integrals in place of sums, are admitted at this level.

\section{Uniform distribution}

\begin{definition}[Uniform distribution]\label{def:tle:contdist:uniform}
$X$ follows the \emph{uniform distribution}\index{uniform distribution}
$\mathcal U\intcc ab$ if its density is constant,
$f(t) = \frac{1}{b-a}$ on $\intcc{a}{b}$. Then for
$\intcc{c}{d} \subseteq \intcc ab$,
$\P(c \leq X \leq d) = \frac{d - c}{b - a}$: probability proportional to
length.
\end{definition}

\begin{proposition}\label{prop:tle:contdist:uniformmoments}
...
= \frac{b^2 - a^2}{2(b-a)} = \frac{a+b}{2}$. For the variance,
...
\begin{proposition}\label{prop:tle:contdist:expomoments}
...
\]
using $A\eu^{-\lambda A} \to 0$ (\cref{thm:tle:exp:growth}). A second
integration by parts gives $\E(X^2) = \frac{2}{\lambda^2}$, whence
$\V(X) = \frac{2}{\lambda^2} - \frac{1}{\lambda^2}$.
\end{proof}

\begin{theorem}[Memorylessness]\label{thm:tle:contdist:memoryless}
\index{memorylessness}
If $X \sim \mathcal E(\lambda)$, then for all $s, t \geq 0$:
\[
\pcond{X > s}{X > s + t} = \P(X > t) .
\]
The exponential distribution is the law of lifetimes \emph{without aging}
...
{\small The exponential density $\lambda\eu^{-\lambda x}$ (here
$\lambda = 1$): the tail area beyond $t$ (red) is $\eu^{-\lambda t}$,
and memorylessness says every tail looks like the whole distribution
rescaled.}
\end{omfigure}

\section{Normal distribution}

\begin{definition}[Normal distribution]\label{def:tle:contdist:normal}
$X$ follows the \emph{standard normal distribution}\index{normal
distribution} $\mathcal N(0, 1)$ if its density on $\R$ is
\[
...
{\small The normal density: about $68.3\%$ of the mass lies within
$\sigma$ of the mean, $95.4\%$ within $2\sigma$, $99.7\%$ within
$3\sigma$.}
\end{omfigure}

\begin{remark}
The factor $\frac{1}{\sqrt{2\pi}}$ makes the total area $1$ --- a famous
computation (the \emph{Gaussian integral}) done in licence. There is no
elementary formula for $\int \varphi$: normal probabilities are read from
tables or calculators.
\end{remark}

\begin{theorem}[De Moivre--Laplace, admitted]\label{thm:tle:contdist:tcl}
...
normal --- and (central limit theorem, licence) so is any sum of many small
independent effects.
\end{theorem}

\begin{method}[Fluctuation interval at $95\%$]\label{met:tle:contdist:fluctuation}
\index{confidence interval}
Since a normal variable falls within $1.96$ standard deviations of its mean
with probability $0.95$, for $n$ large a frequency
$F_n = \frac{X_n}{n}$ of a $\mathcal B(n,p)$ sample satisfies
\[
\P\left(p - 1.96\sqrt{\tfrac{p(1-p)}{n}} \leq F_n \leq
p + 1.96\sqrt{\tfrac{p(1-p)}{n}}\right) \approx 0.95 .
\]
\begin{itemize}
  \item \emph{Fluctuation interval} (testing): if a claimed $p$ puts the
        observed frequency outside this interval, reject the claim at the
        $5\%$ level.
  \item \emph{Confidence interval} (estimating): the simplified interval
        $\left[F_n - \frac{1}{\sqrt n},\ F_n + \frac{1}{\sqrt n}\right]$
        contains $p$ with probability at least $0.95$
        (using $1.96\sqrt{p(1-p)} \leq 1$, see \cref{exo:tle:sums:8}).
\end{itemize}
\end{method}

\begin{example}\label{ex:tle:contdist:poll}
A poll of $n = 1000$ voters gives a candidate $52\%$. The confidence
...
\begin{exercise}[$\star$]\label{exo:tle:contdist:1}
...
\begin{exercise}[$\star$]\label{exo:tle:contdist:2}
Check that $f(t) = \frac{3}{8}t^2$ is a density on $\intcc{0}{2}$, and
compute $\E(X)$ and $\P(X \geq 1)$ for a variable $X$ with this density.
\end{exercise}

\begin{exercise}[$\star$]\label{exo:tle:contdist:3}
...
\begin{enumerate}
  ...
  \item The component has already worked $3$ years. What is the
        probability it works at least $5$ more?
\end{enumerate}
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:tle:contdist:4}
The half-life of a radioactive atom whose lifetime follows
$\mathcal E(\lambda)$ is the median $m$: $\P(X > m) = \frac12$. Express
$m$ as a function of $\lambda$ and compare with the expectation. Which is
larger, and why does that make sense for a skewed distribution?
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:tle:contdist:5}
...
\begin{exercise}[$\star\star$]\label{exo:tle:contdist:6}
...
\begin{exercise}[$\star\star$]\label{exo:tle:contdist:7}
A die claimed to be fair is rolled $1200$ times and shows a six $260$
times (as in \cref{exo:tle:sums:7}).
\begin{enumerate}
  \item Compute the $95\%$ fluctuation interval for the frequency of sixes
        of a fair die over $1200$ rolls.
  \item Is the observed frequency inside? Compare the strength of this
        conclusion with the Bienaymé--Chebyshev analysis.
\end{enumerate}
\end{exercise}

\begin{exercise}[$\star\star\star$]\label{exo:tle:contdist:8}
Before an election, a poll of $n$ people will estimate the score $p$ of a
candidate by the observed frequency $F_n$.
\begin{enumerate}
  \item With the confidence interval of
        \cref{met:tle:contdist:fluctuation}, what sample size guarantees a
...
\begin{exercise}[$\star\star\star$]\label{exo:tle:contdist:9}
Let $X$ have density $f$ on $\intcc ab$ and let
$Y = \alpha X + \beta$ with $\alpha > 0$.
\begin{enumerate}
  \item Show that $\P(c \leq Y \leq d)
        = \P\left(\frac{c - \beta}{\alpha} \leq X \leq
        \frac{d-\beta}{\alpha}\right)$, and deduce that $Y$ has density
        $g(y) = \frac{1}{\alpha} f\left(\frac{y - \beta}{\alpha}\right)$.
  \item Deduce that if $Z \sim \mathcal N(0,1)$, then
        $\mu + \sigma Z$ has density
        $\frac{1}{\sigma\sqrt{2\pi}}\,
        \eu^{-(y - \mu)^2/(2\sigma^2)}$ --- the general normal density.
\end{enumerate}
\end{exercise}
