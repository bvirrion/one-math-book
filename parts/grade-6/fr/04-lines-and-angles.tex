...
chapitre fixe le vocabulaire --- points, segments, demi-droites,
droites, cercles ---, introduit les deux positions spéciales des
droites (parallèles et perpendiculaires), et enseigne à mesurer et
tracer des angles.
...
lorsqu'elles se croisent à angle droit~; on écrit $d_1 \perp d_2$ et on
marque l'angle droit d'un petit carré. Deux droites sont
\emph{parallèles}\index{droites parallèles} lorsqu'elles ne se
...
{\small Un croisement à angle droit (marqué du petit carré) et deux
parallèles~: même direction, pas de point de croisement.}
\end{omfigure}

\begin{proposition}[Deux faits utiles]\label{prop:g6:lines:facts}
\begin{enumerate}
  \item Si deux droites sont toutes deux perpendiculaires à une
        troisième, elles sont parallèles entre elles.
  \item Si deux droites sont parallèles, toute droite perpendiculaire
...
Pour tracer la perpendiculaire à une droite $d$ passant par un point
$P$~:
\begin{enumerate}
  \item placer un côté de l'angle droit de l'équerre le long de $d$~;
  \item glisser l'équerre le long de $d$ jusqu'à ce que l'autre côté
        atteigne $P$~;
  \item tracer la droite le long de ce côté, et marquer l'angle droit.
\end{enumerate}
Pour une parallèle par $P$~: tracer une perpendiculaire à $d$, puis la
perpendiculaire à \emph{cette} droite par $P$
...
Le \emph{cercle}\index{cercle} de centre $O$ et de rayon $r$ est
l'ensemble de tous les points situés à distance exactement $r$ de
$O$. Un segment du centre au cercle est un \emph{rayon}~; un segment
joignant deux points du cercle en passant par le centre est un
\emph{diamètre} --- sa longueur est $2r$~; un segment joignant deux
...
{\small Un cercle avec un rayon $[OM]$, un diamètre $[AB]$ (une corde
passant par le centre, deux fois plus longue que le rayon) et une
corde $[CD]$.}
\end{omfigure}

\begin{example}\label{ex:g6:lines:circle}
«~Tracer le cercle de centre $O$ passant par $A$~»~: ouvrir le
compas de $O$ à $A$ --- le rayon est la distance $OA$ --- et tourner.
...
\emph{sommet} (toujours la lettre du milieu~!). Les angles se mesurent
...
{\small Les quatre familles d'angles. L'angle droit ($90^\circ$) est
...
\begin{enumerate}
  \item Placer le centre du rapporteur exactement sur le sommet de
        l'angle~;
...
d'une feuille de papier est un peu plus de $90^\circ$~; la moitié d'un
angle droit est $45^\circ$~; le tiers d'un angle droit est $30^\circ$.
...
différentes~: le segment $[AB]$, la demi-droite $[CA)$, la droite
$(BC)$.
\end{exercise}

\begin{exercise}[$\star$]\label{exo:g6:lines:2}
Vrai ou faux~? «~$[AB]$ et $[BA]$ sont le même segment.~»
...
l'équerre~: la perpendiculaire à $d$ par $P$, puis la parallèle à $d$
par $P$. Décrire les étapes.
\end{exercise}

\begin{exercise}[$\star$]\label{exo:g6:lines:4}
Les droites $a$ et $b$ sont toutes deux perpendiculaires à une droite
$c$, et une quatrième droite $e$ est perpendiculaire à $a$. Que
peut-on dire de $a$ et $b$~? De $e$ et $c$~? Justifier avec
\cref{prop:g6:lines:facts}.
\end{exercise}

\begin{exercise}[$\star$]\label{exo:g6:lines:5}
Tracer un cercle de centre $O$ de rayon $3$~cm. Placer un point $M$
...
Un cercle a un diamètre de $9$~cm. Quel est son rayon~? Un autre a un
rayon de $2.6$~cm~: quel est son diamètre~?
\end{exercise}

\begin{exercise}[$\star$]\label{exo:g6:lines:7}
Nommer l'angle marqué en trois lettres, puis le classer (aigu, droit,
obtus, plat)~: un angle de $72^\circ$ au sommet $R$ entre les
demi-droites vers $S$ et $T$~; un angle de $148^\circ$ au sommet $B$
entre les demi-droites vers $A$ et $C$~; un angle de $90^\circ$ au
sommet $O$ entre les demi-droites vers $M$ et $N$.
\end{exercise}
...
Tracer un segment $[AB]$ de $6$~cm. Construire le point $C$ tel que
