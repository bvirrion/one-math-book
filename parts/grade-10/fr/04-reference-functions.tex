...
Une poignée de fonctions simples --- affines, carré, inverse, racine
carrée, cube --- apparaissent partout, seules ou combinées. Connaître
leurs graphiques et leurs variations par cœur transforme de nombreux
...
Une \emph{fonction affine}\index{fonction affine} est une fonction de la
forme
\[
f(x) = mx + p,
\]
où $m$ et $p$ sont des nombres réels fixes. Son graphique est une
...
$p = 0$ la fonction est \emph{linéaire}~: $f(x) = mx$ exprime la
proportionnalité.
\end{definition}
...
\begin{enumerate}
  \item Pour deux entrées distinctes $u \neq v$~:
        $m = \dfrac{f(v) - f(u)}{v - u}$ (le coefficient directeur est
        la variation de la sortie par unité de variation de l'entrée).
  \item Si $m > 0$, $f$ est strictement croissante sur $\R$~; si
        $m < 0$, strictement décroissante~; si $m = 0$, constante.
\end{enumerate}
\end{proposition}
...
$v - u > 0$~: $m$ positif donne $f(u) < f(v)$ (croissante), $m$ négatif
donne $f(u) > f(v)$ (décroissante).
\end{proof}
...
{\small Deux fonctions affines~: croissante pour $m = \frac12 > 0$,
décroissante pour $m = -1 < 0$. Avancer d'une unité vers la droite
change $y$ de $m$.}
\end{omfigure}

\begin{example}\label{ex:g10:reffunc:affine}
Trouver la fonction affine dont le graphique passe par $A(1, 3)$ et
$B(4, 9)$. Le coefficient directeur est
\[
...
La fonction $f(x) = x^2$, définie sur $\R$~:
\begin{enumerate}
  \item est strictement décroissante sur $\intoc{-\infty}{0}$ et
        strictement croissante sur $\intco{0}{+\infty}$, avec minimum $0$
        en $x = 0$~;
  \item satisfait $f(-x) = f(x)$ pour tout $x$~: son graphique, une
...
\]
car $v - u > 0$ et $v + u > 0$~: $f$ est strictement croissante sur
$\intco{0}{+\infty}$. Si $u < v \leq 0$, alors $v - u > 0$ mais
$v + u < 0$, donc $f(v) - f(u) < 0$~: strictement décroissante. Enfin
$x^2 \geq 0 = f(0)$ pour tout $x$.

2.~$(-x)^2 = x^2$~; donc les points $(x, x^2)$ et $(-x, x^2)$, images
l'un de l'autre par symétrie par rapport à l'axe vertical, sont tous
deux sur le graphique.
\end{proof}

\begin{remark}[Carrés et ordre]
Parce que la fonction carré décroît du côté négatif, élever au carré
...
La fonction $f(x) = \dfrac1x$, définie pour $x \neq 0$~:
\begin{enumerate}
  \item est strictement décroissante sur $\intoo{-\infty}{0}$ et
        strictement décroissante sur $\intoo{0}{+\infty}$~;
  \item satisfait $f(-x) = -f(x)$~: son graphique, une
...
\]
car $u - v < 0$ et $uv > 0$~: strictement décroissante sur
...
\begin{remark}
Attention~: $\frac1x$ n'est \emph{pas} décroissante sur tout son
ensemble de définition. De $u = -1$ à $v = 1$ la valeur saute de $-1$ à
...
La fonction $f(x) = \sqrt{x}$, définie sur $\intco{0}{+\infty}$, est
...
La fonction $f(x) = x^3$, définie sur $\R$, est strictement croissante,
et son graphique est symétrique par rapport à l'origine.
\end{proposition}
\admitted
...
Pour comparer $f(a)$ et $f(b)$ pour une fonction de référence $f$~:
\begin{enumerate}
  \item placer $a$ et $b$ sur un intervalle où les variations de $f$
        sont connues~;
  \item si $f$ croît là, les images sont dans le même ordre que les
        entrées~; si $f$ décroît, l'ordre est inversé~;
  \item si $a$ et $b$ ne sont pas dans le même intervalle de monotonie,
...
dans $\intoo{0}{+\infty}$, où la fonction inverse décroît, et
...
Pour chaque fonction affine, donner son coefficient directeur, son
ordonnée à l'origine, et dire si elle est croissante ou décroissante~:
\[
f(x) = 3x - 2, \qquad
...
Trouver la fonction affine dont le graphique passe par $(2, 1)$ et
...
En utilisant le graphique de la fonction carré, résoudre $x^2 = 16$,
...
Sachant que la fonction carré décroît sur $\intoc{-\infty}{0}$ et croît
...
\]
(Dans le cas (c), attention~: le minimum de $x^2$ n'est pas en une
extrémité.)
\end{exercise}
...
Modéliser chaque prix par une fonction affine de la distance $x$, tracer
...
\begin{enumerate}
  ...
  \item En déduire les variations de $f$ sur $\intoo{1}{+\infty}$ à
        partir de celles de la fonction inverse.
\end{enumerate}
\end{exercise}
