...
\section{Problem: Between any two numbers}

\begin{problem}[{Weekend problem --- rationals and irrationals
interlace: every interval, however small, contains infinitely many
of each, and no measurement can ever tell them apart}]
\label{pb:g10:numbers:1}
The rationals look like a crowd (all the fractions!) and the
\omterm{ex:g10:numbers:classify}{irrationals} like exotic exceptions ($\sqrt2$, $\pi$). This problem
reveals the true picture: the two families \emph{interlace} so
finely that every \omterm{def:g10:numbers:interval}{interval} of the number line, however
microscopic, contains infinitely many of each --- with strange
consequences, such as this: no physical measurement, however
precise, can ever decide whether a length is rational.

\medskip
\noindent\textbf{Part I --- The four kingdoms.}
\begin{enumerate}
  \item For each number, name the smallest of the sets $\N$,
        $\Z$, $\Q$, $\R$ containing it
        (\cref{def:g10:numbers:sets}): $-7$; \ $\frac{13}{4}$; \
        $\sqrt{16}$; \ $0.121212\ldots$ (recall
        the repeating-decimals weekend problem of the Middle School volume); \ $\sqrt8$; \ $\pi$ (admit its
        irrationality --- proved only in the university
        volumes).
  \item Prove that $\Q$ is stable under addition and
        multiplication: if $x = \frac pq$ and $y = \frac rs$ are
        rational, write $x + y$ and $xy$ as single fractions.
  \item Deduce by contradiction: (a) the sum of a rational and an
        \omterm{ex:g10:numbers:classify}{irrational} is \omterm{ex:g10:numbers:classify}{irrational}; (b) the product of a
        \emph{nonzero} rational and an \omterm{ex:g10:numbers:classify}{irrational} is \omterm{ex:g10:numbers:classify}{irrational}.
  \item Show that the set of \omterm{ex:g10:numbers:classify}{irrationals} is stable under
        \emph{neither} operation: exhibit two \omterm{ex:g10:numbers:classify}{irrationals} whose
        sum is rational, and two whose product is rational.
  \item Locate $\sqrt2 + \sqrt8$ and $\sqrt2 \times \sqrt8$ in
        the four kingdoms (simplify $\sqrt8$ first,
        the Middle School volume's method).
\end{enumerate}

\medskip
\noindent\textbf{Part II --- The rationals are dense.}
\begin{enumerate}[resume]
  \item Find a rational strictly between $3.47$ and $3.48$; then
        a second one; then describe how to produce as many as
        desired (the zoom of the no-next-number weekend problem of the Middle School volume, now with a
        proof in sight).
  \item The general theorem. Let $a < b$ be any two reals, with
        gap $g = b - a > 0$. Choose $n$ with
        $10^{-n} < g$, and consider the multiples of $10^{-n}$
        (the decimal grid of step $10^{-n}$). Explain why at
        least one grid point falls strictly between $a$ and $b$,
        and conclude: \emph{every \omterm{def:g10:numbers:interval}{interval} of positive length
        contains a \omterm{def:g10:numbers:sets}{rational number}}.
  \item Upgrade the conclusion: every such \omterm{def:g10:numbers:interval}{interval} contains
        \emph{infinitely many} rationals. (Apply question 7
        again, inside a smaller \omterm{def:g10:numbers:interval}{interval}.)
  \item Now the \omterm{ex:g10:numbers:classify}{irrationals}: given $a < b$, take a rational $r$
        strictly inside (question 7) and consider the numbers
        $r + \frac{\sqrt2}{10^k}$. Using question 3, show they
        are \omterm{ex:g10:numbers:classify}{irrational}, and that for $k$ large enough they still
        lie in the \omterm{def:g10:numbers:interval}{interval}: \emph{every \omterm{def:g10:numbers:interval}{interval} also contains
        infinitely many \omterm{ex:g10:numbers:classify}{irrationals}}.
  \item Two classic riddles settled: is there a smallest
        positive \omterm{def:g10:numbers:sets}{real number}? A \omterm{def:g10:numbers:sets}{real number} ``just after
        $3$''? Answer both with question 7's theorem, and
        salute the childhood version
        (the no-next-number weekend problem of the Middle School volume).
\end{enumerate}

\medskip
\noindent\textbf{Part III --- \omterm{def:g10:numbers:abs}{Absolute value}, the geometry of
$\R$.}
\begin{enumerate}[resume]
  \item Solve, and express the solutions as \omterm{def:g10:numbers:interval}{intervals} or \omterm{def:g10:numbers:interunion}{unions}
        (\cref{prop:g10:numbers:absinterval}):
        $\abs{x - 5} = 2$; \quad $\abs{x - 5} < 2$; \quad
        $\abs{x + 1} \geq 3$.
  \item A piston must be manufactured to $80$ mm within a
        tolerance of $0.05$ mm. Write the requirement with an
        \omterm{def:g10:numbers:abs}{absolute value}, then as an \omterm{def:g10:numbers:interval}{interval}. Two pistons measure
        $79.97$ and $80.06$ mm: verdicts?
  \item The triangle inequality on the line:
        $\abs{a + b} \leq \abs a + \abs b$. Check it on
        $(a, b) = (3, -5)$ and $(-2, -7)$, prove it when $a$ and
        $b$ have the same sign and when they have opposite
        signs, and state exactly when equality holds.
  \item Solve $\abs{x - 2} = \abs{x + 4}$ by reading it as an
        equality of distances on the line. Which point of
        the two-mirrors weekend problem of the Middle School volume have you just computed, one
        dimension down?
  \item Simplify $\sqrt{x^2}$ --- carefully. Test your formula
        on $x = 3$ and $x = -3$, then use it to solve
        $x^2 < 9$ with an \omterm{def:g10:numbers:abs}{absolute value}.
\end{enumerate}

\medskip
\noindent\textbf{Part IV --- What no measurement can decide.}
\begin{enumerate}[resume]
  \item A physicist measures a rod: $1.41421 \pm 0.00001$ m.
        Can any such measurement --- this one or a future, more
        precise one --- ever \emph{prove} that the rod's length
        is \omterm{ex:g10:numbers:classify}{irrational}? Or that it is rational? Use questions 7
        and 9 on the tolerance \omterm{def:g10:numbers:interval}{interval}, and explain why only
        \emph{proof} (as for the diagonal, the irrationality weekend problem of the Middle School volume)
        can settle irrationality.
  \item The truncations $1.4$, $1.41$, $1.414$, $1.4142,\dots$
        of $\sqrt2$ are all rational. What do they show about
        how close $\Q$ presses against every \omterm{ex:g10:numbers:classify}{irrational}?
        Formulate the general fact.
  \item \omterm{def:g10:numbers:interval}{Interval} arithmetic: measured values $a = 2.5 \pm 0.1$
        and $b = 1.2 \pm 0.1$. Bracket $a + b$ and $a \times b$
        between certain bounds. Which operation degrades the
        precision more?
  \item An engineer's rule says errors of independent
        measurements ``add'' for sums: express with the triangle
        inequality (question 13) why the error of $a + b$ is at
        most the sum of the errors --- the inequality is the
        engineering rule.
  \item Finale, in a short paragraph: assemble the picture of
        the real line established by this problem and its
        ancestors --- no next number
        (the no-next-number weekend problem of the Middle School volume), rationals $=$ repeating
        decimals (the repeating-decimals weekend problem of the Middle School volume), both families dense
        (questions 8 and 9), measurement forever undecided
        (question 16). End with the teaser this chapter cannot
        yet prove: in a precise sense there are \emph{vastly
        more} \omterm{ex:g10:numbers:classify}{irrationals} than rationals --- the university
        volumes count the infinities.
\end{enumerate}
\end{problem}
