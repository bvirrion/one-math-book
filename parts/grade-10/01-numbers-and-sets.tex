...
nested families, introduces intervals to describe portions of the number
line, and uses the absolute value to measure distances between numbers.
...
natural number is an integer, every integer is a rational number
(e.g.\ $-5 = \frac{-5}{1}$), and every rational number is real, so
\[
...
\begin{itemize}
  ...
  \item $-3.5 = -\frac{35}{10} = -\frac72$ is rational but not an integer.
...
The number $\sqrt 2$ is not rational: no fraction of integers has square
$2$.
\end{theorem}
...
\begin{enumerate}
  \item Suppose $\sqrt 2 = \frac{p}{q}$ where $p$ and $q$ are positive
        integers, and the fraction is fully simplified, so $p$ and $q$ are
        not both even.
  ...
  \item If $p$ were odd, say $p = 2k+1$, then
        ...
        be even: $p = 2k$ for some integer $k$.
  ...
  \item Now $p$ and $q$ are both even, contradicting step 1. The
        assumption was impossible: $\sqrt 2$ is irrational.
\end{enumerate}
\end{proof}

\begin{remark}
Rational numbers are exactly the numbers whose decimal expansion either
...
Irrational numbers like $\sqrt2 = 1.41421356\dots$ have decimal digits
...
Real numbers fill a line: choosing an origin $0$ and a unit of length,
every real number corresponds to exactly one point.

\begin{definition}[Interval]\label{def:g10:numbers:interval}
Let $a$ and $b$ be real numbers with $a < b$. An
\emph{interval}\index{interval} is the set of all real numbers between two
...
{\small Intervals on the number line: a filled dot for an included bound,
a hollow dot for an excluded one.}
\end{omfigure}

\begin{definition}[Intersection and union]\label{def:g10:numbers:interunion}
Let $I$ and $J$ be two sets of real numbers.
...
\begin{enumerate}
  ...
  \item shade the first interval above the line and the second below it;
  \item the intersection is where the shadings overlap, the union is where
        at least one shading is present;
  \item decide each bracket by testing whether the bound itself belongs to
        both sets (intersection) or to at least one (union).
\end{enumerate}
\end{method}
...
The \emph{absolute value}\index{absolute value} of a real number $x$ is
\[
\abs{x} =
\begin{cases}
x & \text{if } x \geq 0, \\
...
For all real numbers $a$ and $b$, the distance between the points $a$ and
...
which is again $\abs{b - a}$ by definition of the absolute value.
\end{proof}

\begin{proposition}[Absolute value and intervals]\label{prop:g10:numbers:absinterval}
Let $a$ be a real number and $r > 0$. Then
\[
\abs{x - a} \leq r
...
{\small The inequality $\abs{x - a} \leq r$ describes the interval of
...
most $2$ from $3$: the interval $\intcc{1}{5}$. Conversely, the interval
...
Irrational numbers, and even most fractions, cannot be written exactly
...
$3.142$ are both approximations of $\pi$ to within $10^{-3}$. The rounding
...
Write each statement with interval notation, then draw it on a number
...
Describe each interval by an inequality of the form
...
\begin{enumerate}
  \item The sum of two integers is an integer.
  \item The quotient of two integers is an integer.
  \item The sum of two rational numbers is rational.
  \item The sum of a rational number and an irrational number is
        irrational.
\end{enumerate}
\end{exercise}
...
irrational. (Replace ``even'' by ``multiple of $3$'': first check that if
