...
sharpens the two basic moves --- \omterm{def:g10:algebra:expand}{expanding} and \omterm{def:g10:algebra:expand}{factoring} --- and uses them
to solve \omterm{def:g10:algebra:equation}{equations} and inequalities systematically, with the \omterm{def:g10:algebra:signtable}{sign table} as
the central tool.
...
For all \omterm{def:g10:numbers:sets}{real numbers} $a$ and $b$:
\[
(a+b)^2 = a^2 + 2ab + b^2,
\qquad
(a-b)^2 = a^2 - 2ab + b^2,
...
Used forward (\omterm{def:g10:algebra:expand}{expanding}) and backward (\omterm{def:g10:algebra:expand}{factoring}):
\begin{align*}
(3x + 2)^2 &= 9x^2 + 12x + 4,
& x^2 - 25 &= (x+5)(x-5), \\
(x - 4)^2 &= x^2 - 8x + 16,
...
\begin{enumerate}
  ...
  \item \emph{Remarkable identity:} recognize $a^2 - b^2$, or
        ...
        (3x+4)(3x-4)$.
  \item Combine both, and check the result by \omterm{def:g10:algebra:expand}{expanding} it back.
\end{enumerate}
\end{method}
...
Two \omterm{def:g10:algebra:equation}{equations} are \emph{equivalent} when they have exactly the same
solutions. Adding the same number to both sides, or multiplying both sides
by the same \emph{nonzero} number, produces an equivalent \omterm{def:g10:algebra:equation}{equation}.
...
A product of \omterm{def:g10:numbers:sets}{real numbers} is zero if and only if at least one of its
factors is zero:
\[
A \times B = 0
...
To solve an \omterm{def:g10:algebra:equation}{equation} whose right-hand side is $0$ and whose left-hand
side can be factored:
\begin{enumerate}
  \item bring everything to the left so the \omterm{def:g10:algebra:equation}{equation} reads $E = 0$;
  \item factor $E$ into a product of first-degree factors;
  \item apply the zero-product rule and solve each small \omterm{def:g10:algebra:equation}{equation};
...
{\small The graph of $P(x) = (x-2)(3-x)$ confirms the \omterm{def:g10:algebra:signtable}{sign table}: positive
...
\begin{enumerate}
  ...
  \item factor the left-hand side, or put it over a common denominator if
        it is a quotient;
  \item build the \omterm{def:g10:algebra:signtable}{sign table} with one row per factor (for a quotient, mark
        forbidden values with a double bar in the last row);
  \item read off the \omterm{def:g10:numbers:interval}{intervals} where the requested sign holds, checking
...
Solve the inequalities and write the solution sets as \omterm{def:g10:numbers:interval}{intervals}:
\[
3x - 5 \leq 7, \qquad
-4x + 1 > 9, \qquad
2(x - 1) \geq 5x + 4 .
...
Using a \omterm{def:g10:algebra:signtable}{sign table}, solve
\[
(x + 2)(4 - x) > 0,
\qquad
\frac{2x - 6}{x + 1} \geq 0 .
...
Let $n$ be an \omterm{def:g10:numbers:sets}{integer}. Show that $(n + 1)^2 - n^2$ is always odd, and that
the difference between the squares of two consecutive odd numbers is
always a multiple of $8$.
\end{exercise}

\section{Problem: From Babylon to the golden ratio}

\begin{problem}[{Weekend problem --- sum-and-product problems,
completing the square the way al-Khwarizmi drew it, and the
equation $x^2 = x + 1$}]
\label{pb:g10:algebra:1}
Four thousand years ago, Babylonian scribes solved this on clay:
\emph{find two numbers knowing their sum and their product}. Their
trick --- look for the numbers symmetrically around the
half-sum --- still cracks every quadratic \omterm{def:g10:algebra:equation}{equation} today, under
its modern name, \emph{completing the square}. This problem
learns the trick, watches al-Khwarizmi literally complete a
square, and puts the method to work on the most celebrated
proportion of them all.

\medskip
\noindent\textbf{Part I --- The Babylonian problem.}
\begin{enumerate}
  \item Warm-up (\cref{thm:g10:algebra:identities}): factor
        $x^2 - 10x + 25$, then $9x^2 - 49$, then --- combining an
        identity with a difference of squares ---
        $x^2 + 6x + 9 - 4$.
  \item Solve $(x + 1)(x + 5) = 0$ and
        $(3x - 7)(3x + 7) = 0$
        (\cref{thm:g10:algebra:zeroproduct}).
  \item The scribe's problem: two numbers have sum $10$ and
        product $21$. Following Babylon, write them as $5 - t$
        and $5 + t$ (symmetric around the half-sum) and find $t$,
        then the two numbers.
  \item Same method: sum $14$, product $33$.
  \item The general recipe: for sum $s$ and product $p$, show
        ...
        \]
        and state the condition on $s$ and $p$ for the problem to
        have a solution.
\end{enumerate}
...
\begin{enumerate}[resume]
  \item Expand $(x - a)(x - b)$ and conclude: the solutions of
        $x^2 - sx + p = 0$ are exactly the pairs of numbers with
        sum $s$ and product $p$. (The scribes were solving
        quadratic \omterm{def:g10:algebra:equation}{equations} without knowing it.)
  \item Verify that $x^2 - 10x + 21 = (x - 5)^2 - 4$, and solve
        $x^2 - 10x + 21 = 0$ by \omterm{def:g10:algebra:expand}{factoring} the right side as a
        difference of squares. Compare with question 3.
  \item Solve $x^2 + 6x - 7 = 0$ by completing the square.
  \item Complete the square in $x^2 + 4x + 5$, and explain why
        the \omterm{def:g10:algebra:equation}{equation} $x^2 + 4x + 5 = 0$ has \emph{no} real
        solution. State the general criterion: once written as
        $(x + h)^2 + k$, when does a quadratic expression
        vanish?
  \item The name is literal. In ninth-century Baghdad,
        al-Khwarizmi drew $x^2 + 6x = 7$ as a square of side $x$
        with two $3 \times x$ rectangles glued to two of its
        sides, and \emph{completed the square} with the missing
        $3 \times 3$ corner. Draw the figure and explain how it
        turns the \omterm{def:g10:algebra:equation}{equation} into $(x + 3)^2 = 16$ --- then solve.
\end{enumerate}
...
promises.}
\begin{enumerate}[resume]
  \item Build the \omterm{def:g10:algebra:signtable}{sign table} of $(x - 3)(x + 2)$
        (\cref{def:g10:algebra:signtable}) and solve
        $(x - 3)(x + 2) < 0$.
  \item Solve $x^2 \leq 10x - 21$ (bring everything to one side
        and reuse question 7).
  \item Solve $\dfrac{x - 1}{x + 2} \geq 0$ with a \omterm{def:g10:algebra:signtable}{sign table},
        minding the forbidden value.
  \item An old promise kept: for the farmer of
        queen Dido's fence problem, in the Middle School volume, with $20$ m of fence, the area
        of an $x \times (10 - x)$ pen satisfies
        \[
        A(x) = x(10 - x) = 25 - (x - 5)^2 .
        \]
        Verify the identity, and read off it a \emph{proof} of
        what grade 6 could only conjecture: the maximum area,
        and the unique $x$ achieving it.
  \item Another: prove that $x + \dfrac1x \geq 2$ for every
        $x > 0$, with equality only at $x = 1$. (Multiply by
        $x$, gather, recognize a square --- the
        arithmetic--geometric inequality of
        the Euclid's-relations weekend problem of the Middle School volume in a new costume.)
\end{enumerate}

\medskip
\noindent\textbf{Part IV --- The golden section.}
Cut a segment of length $1$ into a large part $x$ and a small
part $1 - x$ so that
\emph{whole is to large as large is to small}:
$\frac{1}{x} = \frac{x}{1 - x}$.
\begin{enumerate}[resume]
  \item Show that the condition reads $x^2 + x - 1 = 0$.
  \item Solve by completing the square, and keep the root that
        is a length: $x = \frac{\sqrt5 - 1}{2} \approx 0.618$.
  \item The \emph{golden ratio} is
        $\varphi = \frac1x = \frac{2}{\sqrt5 - 1}$. Rationalize
        the denominator (multiply top and bottom by
        $\sqrt5 + 1$, an identity at work) and show
        $\varphi = \frac{\sqrt5 + 1}{2} \approx 1.618$.
  \item Verify \emph{exactly} the two magic identities of
        $\varphi$:
        \[
        \varphi^2 = \varphi + 1,
        \qquad
        ...
        \]
  \item Finale: explain why $\varphi$ is \omterm{ex:g10:numbers:classify}{irrational}
        (the irrationality weekend problem of the Middle School volume supplies the key fact about
        $\sqrt5$), then compute the ratios $\frac53$,
        $\frac85$, $\frac{13}{8}$, $\frac{21}{13}$ of
        consecutive Hemachandra--Fibonacci numbers
        (the rhythm-counting weekend problem of the Middle School volume) to three decimals. What do
        they seem to chase? (The proof that they catch it
        belongs to the chapters on sequences.)
\end{enumerate}
\end{problem}
