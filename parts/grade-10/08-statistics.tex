...
data (mean, median) and those measuring its \emph{spread} (range,
quartiles). A deeper treatment, including the standard deviation, comes in
\cref{ch:g11:stat}.
...
\hline
count $n$ & $1$ & $2$ & $4$ & $6$ & $5$ & $2$ \\
frequency $f$ & $0.05$ & $0.10$ & $0.20$ & $0.30$ & $0.25$ & $0.10$ \\
\end{tabular}
\end{center}
...
\begin{itemize}
  \item if $N$ is odd, the median is the middle value, in position
        $\frac{N+1}{2}$;
  \item if $N$ is even, take the midpoint of the two middle values, in
...
$180$, $210$, $240$, $900$. The median is the $4$th value, $180$. The mean
is $\frac{1960}{7} = 280$ --- larger than $6$ of the $7$ prices!
One extreme value ($900$) pulls the mean far more than the median: the
median is \emph{robust}, the mean is not.
\end{example}
...
\begin{itemize}
  ...
  \item the \emph{first quartile}\index{quartile} $Q_1$ is the smallest
        ...
        $\leq Q_3$;
  \item the \emph{interquartile range} $Q_3 - Q_1$ measures the spread of
...
\begin{enumerate}
  \item compute $\frac N4$; if it is not an integer round it \emph{up} to
        the next integer; $Q_1$ is the value in that position;
...
interquartile interval $\intcc{Q_1}{Q_3}$ and contains the central half of
...
\end{tabular}
\end{center}
Same median, very different spreads: class A is homogeneous (half its
...
\begin{remark}
Neither the mean nor the median is ``better'': the mean uses every value
(and is needed to compute totals), the median resists extreme values. A
...
Find the median, $Q_1$ and $Q_3$ of the sorted series
\[
...
The mean of $24$ test scores is $11.5$. A $25$th student takes the test
and scores $19$. What is the new mean of the class?
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:g10:stats:5}
A student has a mean of $12$ after $5$ tests, all with the same weight.
What score on the $6$th test would raise the mean to $13$? Is a mean of
$14$ reachable (scores are out of $20$)?
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:g10:stats:6}
Invent a series of $7$ nonnegative values whose median is $10$ and whose
mean is larger than $20$; then one whose median is $10$ and whose mean is
smaller than $6$. Why can the mean of such a series never be smaller than
...
\begin{enumerate}
  \item Compute the mean and the median salary.
  \item Which indicator best describes a ``typical'' salary here? Why?
\end{enumerate}
\end{exercise}

\begin{exercise}[$\star\star$]\label{exo:g10:stats:8}
A class of $12$ boys has a mean height of $168$ cm; the $18$ girls of the
same class have a mean height of $163$ cm. Compute the mean height of the
whole class. (Careful: it is \emph{not} the midpoint of $168$ and $163$
--- weight the means by the group sizes.)
\end{exercise}

\begin{exercise}[$\star\star\star$]\label{exo:g10:stats:9}
A series of $N$ values $x_1, \dots, x_N$ has mean $\bar x$. Each value is
transformed into $y_i = a x_i + b$ for fixed numbers $a$ and $b$.
\begin{enumerate}
  \item Show that the mean of the new series is $a \bar x + b$.
  \item The temperatures of a week, in degrees Celsius, have mean $20$.
        What is their mean in degrees Fahrenheit
        ($F = 1.8\,C + 32$)?
\end{enumerate}
\end{exercise}
